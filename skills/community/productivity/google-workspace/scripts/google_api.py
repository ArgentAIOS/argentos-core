#!/usr/bin/env python3
"""Google Workspace API CLI for Community Skills.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing community skills-facing JSON contract and falls back to the Python client
...
from _community_home import get_community_home
...
def _gws_binary() -> str | None:
    override = os.getenv("COMMUNITY_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Community Skills")
