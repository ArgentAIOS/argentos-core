...
description: Control Spotify — play music, search the catalog, manage playlists
  ...
  check what's playing. Assumes the community skills Spotify toolset is enabled and
  `community auth spotify` has been run.
version: 1.0.0
...
Control the user's Spotify account via the community skills Spotify toolset (7 tools). Setup guide: https://community-skills.nousresearch.com/docs/user-guide/features/spotify
...
**`401 Unauthorized` after a retry** — refresh token revoked. Tell the user to run `community auth spotify` again.
