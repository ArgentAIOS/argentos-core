...
5. Store seen GUIDs in a state file under `~/.argent/state/`.
6. Put the script in `~/.argent/scripts/` and schedule it with `cronjob`; cron `script` paths must be relative to `~/.argent/scripts/`, not absolute paths.
...
STATE_PATH = Path.home() / ".community" / "state" / "medium_seen.json"
...
cronjob(action="create",
  ...
  schedule="every 24h",
  script="medium_seen.py",  # relative to ~/.argent/scripts/
