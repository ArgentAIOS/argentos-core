---
name: community-github-code-review
...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.argent/.env ] && grep -q "^GITHUB_TOKEN=" ~/.argent/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.argent/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Community Skills\",
    \"comments\": [
...
```bash
source "${COMMUNITY_HOME:-$HOME/.community}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Community Skills. Code looks clean — good test coverage, no security concerns."
