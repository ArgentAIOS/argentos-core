...
- Give it a name like "community-skills"
- Select scopes:
...
- Give it a title like "community-skills-<machine-name>"
...
elif [ -f ~/.argent/.env ] && grep -q "^GITHUB_TOKEN=" ~/.argent/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.argent/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
