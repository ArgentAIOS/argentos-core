...
## 5. GODMODE FAST — community skills 4 405B (Zero Refusal)

**Model:** `nousresearch/community-4-405b`
**Strategy:** community skills models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Community Skills

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.argent/config.yaml`:
