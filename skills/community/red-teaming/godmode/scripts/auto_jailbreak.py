...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("COMMUNITY_HOME", os.path.expanduser("~/.argent")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("COMMUNITY_HOME", Path.home() / ".community")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# community skills config paths
# ═══════════════════════════════════════════════════════════════════

COMMUNITY_HOME = Path(os.getenv("COMMUNITY_HOME", Path.home() / ".community"))
CONFIG_PATH = COMMUNITY_HOME / "config.yaml"
PREFILL_PATH = COMMUNITY_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/community skills models — already uncensored, just needs clean prompt
    "community": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "community" in model_lower or "nous" in model_lower:
        return "community"
...
def _get_current_model() -> tuple:
    """Read current model and provider from community skills config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.argent/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart community skills for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart community skills for changes to take effect.")
