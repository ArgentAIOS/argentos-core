---
name: community-webhook-subscriptions
...
```bash
community gateway run
# Or if using systemd:
systemctl --user restart community-gateway
```
...
All management is via the `community webhook` CLI command:

### Create a subscription

```bash
community webhook subscribe <name> \
...
```bash
community webhook remove <name>
```

### Test a subscription

```bash
community webhook test <name>
community webhook test <name> --payload '{"key": "value"}'
```
...
```bash
community webhook subscribe github-issues \
  --events "issues" \
...
```bash
community webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
community webhook subscribe stripe-payments \
...
```bash
community webhook subscribe ci-builds \
  --events "pipeline" \
...
1. `community webhook subscribe` writes to `~/.argent/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status community-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.argent/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `community webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `community webhook test <name>` to verify the route works.
