...
metadata:
  { "argent": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${ARGENT_TMUX_SOCKET_DIR:-${OPENCLAW_TMUX_SOCKET_DIR:-${CLAWDBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/argent-tmux-sockets}}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/argent.sock"
SESSION=argent-python
...
- Use `ARGENT_TMUX_SOCKET_DIR` (legacy `OPENCLAW_TMUX_SOCKET_DIR` / `CLAWDBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$ARGENT_TMUX_SOCKET_DIR/argent.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `ARGENT_TMUX_SOCKET_DIR`).
