---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for ArgentOS deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, ArgentOS cron scheduling for periodic checks, or version status checks on a machine running ArgentOS (laptop, workstation, Pi, VPS).
---

# ArgentOS Host Hardening

## Overview

Assess and harden the host running ArgentOS, then align it to a user-defined risk tolerance without breaking access. Use ArgentOS security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim ArgentOS changes the host firewall, SSH, or OS updates; it does not.
...
5. ArgentOS gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if ArgentOS can access sensitive data.
...
### 2) Run ArgentOS security audits (read-only)

As part of the default read-only checks, run `argent security audit --deep`. Only offer alternatives if the user requests them:

1. `argent security audit` (faster, non-probing)
2. `argent security audit --json` (structured output)

Offer to apply ArgentOS safe defaults (numbered):

1. `argent security audit --fix`

Be explicit that `--fix` only tightens ArgentOS defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check ArgentOS version/update status (read-only)

As part of the default read-only checks, run `argent update status`.
...
- Credential hygiene notes (location of ArgentOS creds, prefer disk encryption)
...
- ArgentOS security audit (re-run)
...
After ArgentOS install or first hardening pass, run at least one baseline audit and version check:

- `argent security audit`
- `argent security audit --deep`
- `argent update status`

Ongoing monitoring is recommended. Use the ArgentOS cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. "Do you want me to schedule periodic audits (e.g., daily/weekly) via `argent cron add`?"

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `argent update status`
...
Before creating, `argent cron list` and match on exact `name`. If found, `argent cron edit <id> ...`.
If not found, `argent cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `argent update status` (preferred for source checkouts and channels)
2. `npm view argentos version` (published npm version)

## ArgentOS command accuracy

Use only supported commands and flags:

- `argent security audit [--deep] [--fix] [--json]`
- `argent status` / `argent status --deep`
- `argent health --json`
- `argent update status`
- `argent cron add|list|runs|run`

Do not invent CLI flags or imply ArgentOS enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by ArgentOS compaction:
