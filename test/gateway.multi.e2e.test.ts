...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `argent-e2e-${name}-`));
  const configDir = path.join(homeDir, ".argent");
  await fs.mkdir(configDir, { recursive: true });
  const configPath = path.join(configDir, "argent.json");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          ARGENT_SKIP_BROWSER_CONTROL_SERVER: "1",
          ARGENT_SKIP_CANVAS_HOST: "1",
        },
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        ARGENT_GATEWAY_TOKEN: inst.gatewayToken,
        ARGENT_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ARGENT_GATEWAY_PORT: String(gwA.port),
          ARGENT_GATEWAY_TOKEN: gwA.gatewayToken,
          ARGENT_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ARGENT_GATEWAY_PORT: String(gwB.port),
          ARGENT_GATEWAY_TOKEN: gwB.gatewayToken,
          ARGENT_GATEWAY_PASSWORD: "",
        }),
