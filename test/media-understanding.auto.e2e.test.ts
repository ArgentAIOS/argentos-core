...
import type { ArgentConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("argent-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("argent-bin-sherpa-");
      const modelDir = await makeTempDir("argent-sherpa-model-");
      ...
      const cfg: ArgentConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("argent-bin-whispercpp-");
      const modelDir = await makeTempDir("argent-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("argent-bin-gemini-");
      ...
      const cfg: ArgentConfig = { tools: { media: { image: {} } } };
