...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ARGENT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.ARGENT_STATE_DIR = path.join(base, ".argentos");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "argent-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".argentos", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
