...
- **VERIFIED (API)**: standalone `send_resend` test → Resend `accepted:true`, msg id `877eac8f-…` (sender domain verified, key works).
- **⚠️ DEPLOY GOTCHA (corrected later this session): the running gateway is the INSTALLED snapshot** `~/.argentos/lib/node_modules/argentos`, **not the repo** — so committing the resend fix + restarting the gateway did NOT make it live (the installed dist still had `sendgrid`). Made live only after a real build+install: `ARGENT_GIT_DIR=<repo> SKIP_FETCH=1 SKIP_UPDATE=1 bash scripts/out-of-sync-patch.sh` (builds repo HEAD → atomic rsync → gateway bounce). Post-deploy dist: resend ×5 / sendgrid ×0. **Lesson: code changes need an install (`out-of-sync-patch.sh`) to reach the live box; only config (`argent.json`/`service-keys.json`) is read live.** This same deploy shipped all of WR2 P4.
