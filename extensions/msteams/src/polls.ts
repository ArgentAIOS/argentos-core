...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["argentPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["argent", "pollId"]) ??
    readNestedString(value, ["argent", "poll", "id"]) ??
    readNestedString(value, ["data", "argentPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "argent", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "argent poll vote",
          displayText: "Vote recorded",
          value: { argentPollId: pollId, pollId },
        },
      },
    ],
  };
