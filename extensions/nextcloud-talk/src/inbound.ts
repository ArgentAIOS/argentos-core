...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as ArgentConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as ArgentConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as ArgentConfig);
