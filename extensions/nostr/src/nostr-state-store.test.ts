import type { PluginRuntime } from "argent/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.ARGENT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-nostr-"));
  process.env.ARGENT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.ARGENT_STATE_DIR?.trim() || env.ARGENT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".argent");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ARGENT_STATE_DIR = previous;
    }
