...
import type { ArgentPluginApi, ArgentPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "argent-lobster-plugin-") {
...
function fakeApi(overrides: Partial<ArgentPluginApi> = {}): ArgentPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<ArgentPluginToolContext> = {}): ArgentPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: ArgentPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
