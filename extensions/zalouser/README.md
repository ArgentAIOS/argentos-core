...
ArgentOS extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
argent channels login --channel zalouser
...
```bash
argent message send --channel zalouser --target <threadId> --message "Hello from ArgentOS!"
```
...
```bash
argent channels login --channel zalouser              # Login via QR
argent channels login --channel zalouser --account work
argent channels status --probe
argent channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
argent directory self --channel zalouser
argent directory peers list --channel zalouser --query "name"
argent directory groups list --channel zalouser --query "work"
argent directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
argent message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
argent message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `argent channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
argent channels login --channel zalouser --account work
argent message send --channel zalouser --account work --target <id> --message "Hello"
