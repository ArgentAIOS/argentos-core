...
metadata: { "argent": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == ArgentOS `sessions_spawn`
- **File I/O** == ArgentOS `read`/`write`
- **Remote fetch** == ArgentOS `web_fetch` (or `exec` with curl when POST is required)
