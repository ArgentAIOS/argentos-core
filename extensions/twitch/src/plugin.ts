/**
 * Twitch channel plugin for Argent.
 *
...
import { buildChannelConfigSchema } from "argent/plugin-sdk";
import type {
 ...
 * for Argent. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: ArgentConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: ArgentConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: ArgentConfig): boolean => {
