 ...
 */
function setTwitchAccount(cfg: ArgentConfig, account: Partial<TwitchAccountConfig>): ArgentConfig {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: ARGENT_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: ArgentConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var ARGENT_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.ARGENT_TWITCH_ACCESS_TOKEN?.trim();
