...
function resolveArgentRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.ARGENT_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set ARGENT_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveArgentRoot(), "dist", "extensionAPI.js");
