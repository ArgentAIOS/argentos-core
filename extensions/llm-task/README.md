...
`argent.invoke --each`) without adding new ArgentOS code per workflow.
...
This extension depends on ArgentOS internal modules (the embedded agent runner).
It is intended to ship as a **bundled** ArgentOS extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.argent/extensions` as a standalone plugin directory.
