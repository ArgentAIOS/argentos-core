...
// NOTE: This extension is intended to be bundled with Argent.
// When running from source (tests/dev), Argent internals live under src/.
...
export function createLlmTaskTool(api: ArgentPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via argent.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-llm-task-"));
