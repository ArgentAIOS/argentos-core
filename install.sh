...
STATE_DIR="$HOME/.argentos"
WORKSPACE_DIR="$HOME/argent"
...
add_to_path() {
  ...
  if ! grep -q 'HOME/bin' "$profile" 2>/dev/null; then
    ...
    ok "Added ~/bin to PATH in $(basename "$profile")"
  else
    ok "~/bin already in PATH ($(basename "$profile"))"
  fi
}
...
case "$SHELL_NAME" in
  ...
  *)    warn "Unknown shell ($SHELL_NAME). Add ~/bin to your PATH manually." ;;
esac
...
APP_DEST="/Applications/Argent.app"
APP_SOURCE=""
...
if ! is_truthy "$FULL_STACK_INSTALL"; then
  ...
  if ! command -v brew >/dev/null 2>&1; then
    ...
    mkdir -p "$ARGENT_REDIS_CONF_DIR" "$ARGENT_REDIS_DATA" "$(dirname "$ARGENT_REDIS_LOG")" "${HOME}/Library/LaunchAgents"
...
PLIST_PATH="$HOME/Library/LaunchAgents/ai.argent.gateway.plist"
UI_PLIST_PATH="$HOME/Library/LaunchAgents/ai.argent.dashboard-ui.plist"
API_PLIST_PATH="$HOME/Library/LaunchAgents/ai.argent.dashboard-api.plist"
...
if ! echo "$PATH" | tr ':' '\n' | grep -qx "$HOME/bin"; then
  warn "Restart your terminal (or run: export PATH=\"\$HOME/bin:\$PATH\")"
fi
