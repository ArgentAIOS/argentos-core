...
TOOL_SCOPE_KEYS = ("aos-airtable", "airtable")
...
def _candidate_keys(variable: str) -> tuple[str, ...]:
    normalized = variable.removeprefix("AOS_").removeprefix("AIRTABLE_").lower()
    keys = [variable, variable.lower(), normalized]
    aliases = {
        "AIRTABLE_API_TOKEN": ("api_token", "token", "access_token"),
        "AOS_AIRTABLE_API_TOKEN": ("api_token", "token", "access_token"),
        "AIRTABLE_BASE_ID": ("base_id", "base"),
        "AOS_AIRTABLE_BASE_ID": ("base_id", "base"),
        "AIRTABLE_TABLE_NAME": ("table_name", "table"),
        "AOS_AIRTABLE_TABLE_NAME": ("table_name", "table"),
        "AIRTABLE_WORKSPACE_ID": ("workspace_id", "workspace"),
        "AOS_AIRTABLE_WORKSPACE_ID": ("workspace_id", "workspace"),
        "AIRTABLE_API_BASE_URL": ("api_base_url", "base_url", "api_url"),
    }
...
def resolve_named_value(*names: str, ctx_obj: dict[str, Any] | None = None, default: str | None = None) -> dict[str, Any]:
    blocked_detail: dict[str, Any] | None = None
    for name in names:
        detail = service_key_details(name, ctx_obj)
        if detail["value"]:
            return detail
        if detail.get("blocked") and blocked_detail is None:
            blocked_detail = detail
            break
    if blocked_detail is not None:
        return blocked_detail
    if default is not None:
        return service_key_details(names[0] if names else "", ctx_obj, default=default)
    return {"value": "", "present": False, "usable": False, "source": "missing", "variable": names[0] if names else ""}
