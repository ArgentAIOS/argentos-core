...
TOOL_SCOPE_KEYS = ("aos-monday", "monday")
...
def _candidate_keys(variable: str) -> tuple[str, ...]:
    suffix = variable.removeprefix("MONDAY_").lower()
    keys = [variable, variable.lower(), suffix]
    aliases = {
        "MONDAY_TOKEN": ("token", "api_token", "api_key"),
        "MONDAY_API_URL": ("api_url", "base_url", "url"),
        "MONDAY_API_VERSION": ("api_version",),
        "MONDAY_WORKSPACE_ID": ("workspace_id", "workspace"),
        "MONDAY_BOARD_ID": ("board_id", "board"),
        "MONDAY_ITEM_ID": ("item_id", "item"),
        "MONDAY_COLUMN_ID": ("column_id", "column"),
    }
    ...
    for entry in keys:
        ...
        if value.startswith("enc:v1:"):
            return {
                ...
                "present": True,
                "usable": False,
                "source": "repo-service-key-encrypted",
                "variable": variable,
                "blocked": True,
                "reason": "encrypted service keys must be decrypted by the operator runtime",
            }
        return {
            "value": value,
            "present": True,
