...
TOOL_SCOPE_KEYS = ("aos-zapier", "zapier")
...
def _candidate_keys(variable: str) -> tuple[str, ...]:
    suffix = variable.removeprefix("ZAPIER_").lower()
    keys = [variable, variable.lower(), suffix]
    aliases = {
        "ZAPIER_API_URL": ("api_url", "base_url", "bridge_url", "url"),
        "ZAPIER_API_KEY": ("api_key", "token", "key"),
        "ZAPIER_WEBHOOK_BASE_URL": ("webhook_base_url", "webhook_url", "hooks_url"),
        "ZAPIER_WORKSPACE_NAME": ("workspace_name", "workspace"),
        "ZAPIER_ZAP_ID": ("zap_id", "zap"),
        "ZAPIER_ZAP_NAME": ("zap_name", "name"),
        "ZAPIER_ZAP_STATUS": ("zap_status", "status"),
    }
