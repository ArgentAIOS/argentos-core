...
def _first_present_env(*names: str) -> tuple[str | None, str | None, str | None]:
    for name in names:
        if not name:
            continue
        source = service_key_source(name)
        if source != "service-keys":
            continue
    ...
    for name in names:
        ...
        value = (service_key_env(name) or "").strip()
        if value:
            return name, value, service_key_source(name)
...
def resolve_runtime_values(ctx_obj: dict[str, Any]) -> dict[str, Any]:
    ...
    access_token_source, access_token, access_token_source_kind = _first_present_env(
        access_token_env,
        DEFAULT_ACCESS_TOKEN_ENV,
        LEGACY_ACCESS_TOKEN_ENV,
    )
    app_id_source, app_id, app_id_source_kind = _first_present_env(app_id_env, DEFAULT_APP_ID_ENV, LEGACY_APP_ID_ENV)
    webhook_source, webhook_secret, webhook_source_kind = _first_present_env(
        webhook_secret_env,
        DEFAULT_WEBHOOK_SECRET_ENV,
        LEGACY_WEBHOOK_SECRET_ENV,
    )
    portal_source, portal_id_env_value, portal_source_kind = _first_present_env(
        DEFAULT_PORTAL_ID_ENV,
        LEGACY_PORTAL_ID_ENV,
    )
    alias_source, account_alias_env_value, alias_source_kind = _first_present_env(
        DEFAULT_ACCOUNT_ALIAS_ENV,
        LEGACY_ACCOUNT_ALIAS_ENV,
    )
