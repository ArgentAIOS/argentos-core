...
TOOL_SCOPE_KEYS = ("aos-vip-email", "vip-email", "vip_email")
...
def _candidate_keys(variable: str) -> tuple[str, ...]:
    ...
    short = variable.removeprefix("VIP_EMAIL_").lower()
    aliases = {
        "GOOGLE_WORKSPACE_ACCOUNT": ("google_account", "account", "account_id"),
        "VIP_EMAIL_SENDERS": ("senders", "vip_senders", "sender_rules"),
        "VIP_EMAIL_ACCOUNTS": ("accounts",),
        "VIP_EMAIL_SCAN_CADENCE_SECONDS": ("scan_cadence_seconds", "cadence_seconds"),
        "VIP_EMAIL_LOOKBACK_DAYS": ("lookback_days",),
        "VIP_EMAIL_MAX_RESULTS": ("max_results",),
        "VIP_EMAIL_DEDUPE_WINDOW_SECONDS": ("dedupe_window_seconds", "dedupe_seconds"),
        "VIP_EMAIL_QUIET_HOURS": ("quiet_hours",),
        "VIP_EMAIL_ALERT_DESTINATIONS": ("alert_destinations", "destinations"),
    }
    return tuple(dict.fromkeys([variable, suffix, short, *aliases.get(variable, ())]))
