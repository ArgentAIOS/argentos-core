...
def _candidate_keys(variable: str) -> tuple[str, ...]:
    suffix = variable.removeprefix("HOOTSUITE_").lower()
    keys = [variable, variable.lower(), suffix]
    aliases = {
        "HOOTSUITE_ACCESS_TOKEN": ("access_token", "oauth_token", "token"),
        "HOOTSUITE_BASE_URL": ("base_url", "api_url", "url"),
        "HOOTSUITE_ORGANIZATION_ID": ("organization_id", "org_id", "organization"),
        "HOOTSUITE_SOCIAL_PROFILE_ID": ("social_profile_id", "profile_id", "social_profile"),
        "HOOTSUITE_TEAM_ID": ("team_id", "team"),
        "HOOTSUITE_MESSAGE_ID": ("message_id", "message"),
    }
