...
TOOL_SCOPE_KEYS = ("aos-slack-attention", "slack-attention", "slack_attention")
...
function readKeychainKey() {
  ...
  try {
    const hex = execFileSync("security", ["find-generic-password", "-s", "ArgentOS-MasterKey", "-a", "ArgentOS", "-w"], { encoding: "utf8", timeout: 5000, stdio: ["ignore", "pipe", "ignore"] }).trim();
...
def _candidate_keys(variable: str) -> tuple[str, ...]:
    short = variable.removeprefix("SLACK_ATTENTION_").removeprefix("SLACK_").lower()
    aliases = {
        "SLACK_BOT_TOKEN": ("bot_token", "token"),
        "SLACK_ATTENTION_CHANNELS": ("channels", "watched_channels"),
        "SLACK_ATTENTION_KEYWORDS": ("keywords", "keyword_watchlist"),
        "SLACK_ATTENTION_MENTION_USER_IDS": ("mention_user_ids", "user_ids"),
        "SLACK_ATTENTION_MENTION_NAMES": ("mention_names", "names"),
        "SLACK_ATTENTION_SCAN_CADENCE_SECONDS": ("scan_cadence_seconds", "cadence_seconds"),
        "SLACK_ATTENTION_LOOKBACK_MINUTES": ("lookback_minutes",),
        "SLACK_ATTENTION_MAX_MESSAGES": ("max_messages",),
        "SLACK_ATTENTION_DEDUPE_WINDOW_SECONDS": ("dedupe_window_seconds", "dedupe_seconds"),
        "SLACK_ATTENTION_QUIET_HOURS": ("quiet_hours",),
        "SLACK_ATTENTION_ALERT_DESTINATIONS": ("alert_destinations", "destinations"),
    }
    return tuple(dict.fromkeys([variable, variable.lower(), short, *aliases.get(variable, ())]))
