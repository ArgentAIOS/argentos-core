...
@click.pass_context
def file_list(ctx: click.Context, limit: int, folder_id: str | None, mime_type: str | None) -> None:
    _set_command(ctx, "file.list")
    require_mode(ctx, "file.list")
    _emit_success(ctx, "file.list", file_list_result(ctx.obj, limit=limit, folder_id=folder_id, mime_type=mime_type))


@file_group.command("get")
@click.argument("file_id", required=False)
@click.pass_context
def file_get(ctx: click.Context, file_id: str | None) -> None:
...
@click.option("--folder-id", default=None)
@click.pass_context
def folder_list(ctx: click.Context, limit: int, folder_id: str | None) -> None:
...
@click.argument("file_id", required=False)
@click.pass_context
def share_list(ctx: click.Context, file_id: str | None) -> None:
    _set_command(ctx, "share.list")
    require_mode(ctx, "share.list")
    _emit_success(ctx, "share.list", share_list_result(ctx.obj, file_id=file_id))
...
@click.argument("file_id", required=False)
@click.pass_context
def export_pdf(ctx: click.Context, file_id: str | None) -> None:
    _set_command(ctx, "export.pdf")
    require_mode(ctx, "export.pdf")
    _emit_success(ctx, "export.pdf", export_pdf_result(ctx.obj, file_id=file_id))


@export_group.command("docx")
@click.argument("file_id", required=False)
@click.pass_context
def export_docx(ctx: click.Context, file_id: str | None) -> None:
    _set_command(ctx, "export.docx")
    require_mode(ctx, "export.docx")
    _emit_success(ctx, "export.docx", export_docx_result(ctx.obj, file_id=file_id))
...
@click.option("--limit", default=25, show_default=True, type=int)
@click.pass_context
def search_query(ctx: click.Context, query_text: str | None, limit: int) -> None:
    _set_command(ctx, "search.query")
    require_mode(ctx, "search.query")
    _emit_success(ctx, "search.query", search_query_result(ctx.obj, query_text=query_text, limit=limit))
