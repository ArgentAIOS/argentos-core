...
def _candidate_keys(variable: str) -> tuple[str, ...]:
    suffix = variable.removeprefix("N8N_").lower()
    keys = [variable, variable.lower(), suffix]
    aliases = {
        "N8N_API_URL": ("api_url", "base_url", "url"),
        "N8N_API_KEY": ("api_key", "token", "key"),
        "N8N_WEBHOOK_BASE_URL": ("webhook_base_url", "webhook_url", "hooks_url"),
        "N8N_WORKSPACE_NAME": ("workspace_name", "workspace"),
        "N8N_WORKFLOW_ID": ("workflow_id", "workflow"),
        "N8N_WORKFLOW_NAME": ("workflow_name", "name"),
        "N8N_WORKFLOW_STATUS": ("workflow_status", "status"),
    }
