...
homepage: https://docs.argent.ai/hooks#command-logger
metadata:
  {
    "argent":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.argentos/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.argentos/logs/commands.log ~/.argentos/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/argent`:
   ```
   /home/username/.argent/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
