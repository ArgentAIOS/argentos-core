...
This directory contains hooks that ship with ArgentOS. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.argent.ai/hooks/soul-evil
...
homepage: https://docs.argent.ai/hooks#my-hook
metadata:
...
Hooks can be configured in `~/.argentos/argent.json`:
...
2. Restart gateway: `pkill -9 -f 'argent.*gateway' && pnpm argent gateway`
3. Enable the hook: `argent hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.argent.ai/hooks
