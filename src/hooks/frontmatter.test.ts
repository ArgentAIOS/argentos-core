...
`;
    ...
    expect(parsed.argent.emoji).toBe("💾");
    expect(parsed.argent.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.argent.emoji).toBe("📝");
    expect(parsed.argent.events).toEqual(["command"]);
    expect(parsed.argent.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.argent.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"argent": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"argent": {"events": ["test"]}}');
  });
...
describe("resolveArgentMetadata", () => {
  it("extracts argent metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveArgentMetadata(frontmatter);
  ...
  it("returns undefined when argent key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        argent: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Argent" },
            { id: "npm", kind: "npm", package: "@argent/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@argent/hook");
  });
...
homepage: https://docs.argent.ai/hooks#session-memory
metadata:
  {
    "argent":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Argent" }],
      },
  }
---
...
`;
    ...
    const argent = resolveArgentMetadata(frontmatter);
    expect(argent).toBeDefined();
    expect(argent?.emoji).toBe("💾");
    expect(argent?.events).toEqual(["command:new"]);
    expect(argent?.requires?.config).toEqual(["workspace.dir"]);
    expect(argent?.install?.[0].kind).toBe("bundled");
  });
