...
export class AcpGatewayAgent implements Agent {
  ...
  async newSession(params: NewSessionRequest): Promise<NewSessionResponse> {
    const mcp = normalizeAcpMcpServers(params.mcpServers);
    if (mcp.diagnostics.requested > 0) {
      this.log(
        `MCP session setup: requested=${mcp.diagnostics.requested} accepted=${mcp.diagnostics.accepted} ignored=${mcp.diagnostics.ignored.length}`,
      );
    }
    ...
    const session = this.sessionStore.createSession({
      ...
      cliMcpServers: mcp.cliMcpServers,
  ...
  async loadSession(params: LoadSessionRequest): Promise<LoadSessionResponse> {
    ...
    if (mcp.diagnostics.requested > 0) {
      this.log(
        `MCP session load: requested=${mcp.diagnostics.requested} accepted=${mcp.diagnostics.accepted} ignored=${mcp.diagnostics.ignored.length}`,
      );
    }
      ...
      this.gateway
        .request(
          "chat.send",
          {
            ...
            cliMcpServers: session.cliMcpServers,
            idempotencyKey: runId,
