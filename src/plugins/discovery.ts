...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @argent/voice-call -> voice-call).
...
export function discoverArgentPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ...
  if (workspaceDir) {
    ...
    const workspaceExtDirs = [path.join(workspaceRoot, ".argent", "extensions")];
  ...
  if (orgId) {
    const orgDir = path.join(resolveConfigDir(), "orgs", orgId, "extensions");
    discoverInDirectory({
      dir: orgDir,
      origin: "org",
      orgId,
