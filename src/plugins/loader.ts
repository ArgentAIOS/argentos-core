...
import { discoverArgentPlugins } from "./discovery.js";
import { initializeGlobalHookRunner } from "./hook-runner-global.js";
import { loadPluginManifestRegistry } from "./manifest-registry.js";
import { resolveOrgPluginScope } from "./org-scope.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ArgentPluginDefinition;
  register?: ArgentPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ArgentPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ArgentPluginDefinition;
...
export function loadArgentPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (orgScope.error) {
    registry.diagnostics.push({
      level: "error",
      message: orgScope.error,
      source: orgScope.allowlistPath,
    });
  }

  const discovery = discoverArgentPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "argent/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    if (candidate.origin === "org") {
      if (orgScope.allowlistStatus !== "ok") {
        record.status = "disabled";
        record.error = "organization allowlist unavailable (fail-closed)";
        registry.plugins.push(record);
        seenIds.set(pluginId, candidate.origin);
        continue;
      }
      if (!orgScope.allowlist.has(pluginId)) {
        record.status = "disabled";
        record.error = "not in organization allowlist";
        registry.plugins.push(record);
        seenIds.set(pluginId, candidate.origin);
        continue;
      }
    ...
    let mod: ArgentPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as ArgentPluginModule;
    } catch (err) {
  ...
  if (typeof memorySlot === "string" && memorySlot !== "none" && !memorySlotMatched) {
    registry.diagnostics.push({
      level: "warn",
