...
function resolveManifestCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ARGENT_PLUGIN_MANIFEST_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseManifestCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.ARGENT_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
export function loadPluginManifestRegistry(params: {
  ...
  for (const candidate of candidates) {
    ...
    if (!manifestRes.ok) {
      const orgScopedPrefix =
        candidate.origin === "org"
          ? `org-scoped plugin "${candidate.idHint}" manifest error${candidate.orgId ? ` (org: ${candidate.orgId})` : ""}: `
          : "";
      diagnostics.push({
        level: "error",
        message: `${orgScopedPrefix}${manifestRes.error}`,
