...
async function confirmGatewayReachable(port: number): Promise<boolean> {
  const token = process.env.ARGENT_GATEWAY_TOKEN?.trim() || undefined;
  const password = process.env.ARGENT_GATEWAY_PASSWORD?.trim() || undefined;
...
export function renderRestartDiagnostics(snapshot: GatewayRestartSnapshot): string[] {
  ...
  if (snapshot.staleGatewayPids.length > 0) {
    lines.push(`Stale listener candidates: ${snapshot.staleGatewayPids.join(", ")}.`);
  }
