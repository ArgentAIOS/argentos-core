...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  const runtimeRunning = service.runtime?.status === "running";
  const portBusy = status.port?.status === "busy";
  const rpcAuthLike =
    !rpc?.ok &&
    /(unauthor|auth|token|password|forbidden|permission|1008|scope|role)/i.test(
      String(rpc?.error ?? ""),
    );
  const healthClass = runtimeRunning
    ? rpc?.ok
      ? "running+healthy"
      : !portBusy
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("argent doctor")}" (or "${formatCliCommand("argent doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("argent gateway install --force")}\` from the same --profile / ARGENT_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  defaultRuntime.log(`${label("Gateway health:")} ${infoText(healthClass)}`);
  ...
  if (rpc) {
    if (rpc.ok) {
      ...
      if (rpc.diagnosis === "gateway-down-or-crashed") {
        defaultRuntime.error(
          `${label("Diagnosis:")} ${errorText("gateway down/crashed (service not accepting connections)")}`,
        );
      } else if (rpc.diagnosis === "auth-misconfiguration") {
        defaultRuntime.error(
          `${label("Diagnosis:")} ${errorText("auth misconfiguration (gateway.auth is invalid)")}`,
        );
      } else if (rpc.diagnosis === "token-mismatch-unauthorized") {
        defaultRuntime.error(`${label("Diagnosis:")} ${errorText("token mismatch unauthorized")}`);
      }
      if (rpc.diagnosisMessage) {
        defaultRuntime.error(`${label("Hint:")} ${errorText(rpc.diagnosisMessage)}`);
      }
      if (rpc.url) {
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.ARGENT_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("argent gateway install")}`),
    );
    spacer();
  }
  ...
  if (status.port) {
    ...
    if (
      ...
      const owned = status.port.listeners.some(
        (listener) => listener.pid === runtimePid || listener.ppid === runtimePid,
      );
      if (!owned && status.port.listeners.length > 0) {
        const owners = Array.from(
          new Set(status.port.listeners.map((listener) => listener.pid).filter(Boolean)),
        ).join(", ");
        defaultRuntime.error(
          errorText(
            `Listener ownership mismatch: runtime pid=${runtimePid}, but port ${status.port.port} is owned by pid(s): ${owners}.`,
          ),
        );
      }
    }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const env = service.command?.environment ?? process.env;
      const unit = resolveGatewaySystemdServiceName(env.ARGENT_PROFILE);
      defaultRuntime.error(
  ...
  if (runtimeRunning && portBusy && rpcAuthLike) {
    defaultRuntime.error(
      errorText(
        "Auth/config mismatch: service is listening but rejected CLI credentials. Check gateway.auth token/password and env overrides.",
      ),
    );
    spacer();
  }
  ...
  if (status.channelRuntime && Object.keys(status.channelRuntime).length > 0) {
    ...
    const channelIds = Object.keys(status.channelRuntime).toSorted();
