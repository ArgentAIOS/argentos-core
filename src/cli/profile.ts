...
export function applyCliProfileEnv(params: {
  ...
  env.ARGENT_PROFILE = profile;

  const stateDir = env.ARGENT_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.ARGENT_STATE_DIR?.trim()) {
    env.ARGENT_STATE_DIR = stateDir;
  }

  if (!env.ARGENT_CONFIG_PATH?.trim()) {
    env.ARGENT_CONFIG_PATH = path.join(stateDir, "argent.json");
  }

  if (profile === "dev" && !env.ARGENT_GATEWAY_PORT?.trim()) {
    env.ARGENT_GATEWAY_PORT = "19001";
  }
}
