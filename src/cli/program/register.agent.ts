...
${formatHelpExamples([
  ['argent agent --to +15555550123 --message "status update"', "Start a new session."],
  ['argent agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['argent agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'argent agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.argent.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.argent.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['argent agents set-identity --agent main --name "Argent" --emoji "🤖"', "Set name + emoji."],
  ["argent agents set-identity --agent main --avatar avatars/argent.png", "Set avatar path."],
  [
    "argent agents set-identity --workspace ~/.argentos/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "argent agents set-identity --identity-file ~/.argentos/workspace/IDENTITY.md --agent main",
