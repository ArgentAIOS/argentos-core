...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "argent.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("argent completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", ARGENT_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
...
function formatSourceStatusLine(update: Awaited<ReturnType<typeof checkUpdateStatus>>): string {
  ...
  return `${base} · not ready (${source.reason ?? "unknown"})`;
}
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const rows = [
    ...
    { Item: "Source", Value: formatSourceStatusLine(update) },
  ...
  defaultRuntime.log(theme.heading("Argent update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
  ...
  if (update.source && !update.source.ready) {
    defaultRuntime.log(
      theme.warn(
        `Install source is not ready (${update.source.reason ?? "unknown"}). Run ${replaceCliName(formatCliCommand("argent doctor"), CLI_NAME)} before updating.`,
      ),
    );
  }
}
...
function printResult(result: UpdateRunResult, opts: PrintResultOptions) {
  ...
  const statusColor =
    result.status === "ok" || (result.status === "skipped" && result.reason === "up-to-date")
      ? theme.success
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Argent..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    if (result.reason === "up-to-date") {
      defaultRuntime.log(theme.success("Already up to date. No changes applied."));
    }
    ...
    if (result.reason === "not-argent-root") {
      defaultRuntime.log(
        theme.warn(
          "Skipped: current git checkout is not an ArgentOS root. Falling back to package-manager update path is recommended.",
        ),
      );
    }
    if (result.reason === "not-git-install") {
      ...
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g argent@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g argent@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("argent gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("argent doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("argent gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `argent update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.argent.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["argent update status", "Show channel + version status."],
          ["argent update status --json", "JSON output."],
          ["argent update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.argent.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
