...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("argent node install"), formatCliCommand("argent node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("argent node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ARGENT_LOG_PREFIX: baseEnv.ARGENT_LOG_PREFIX ?? "node",
