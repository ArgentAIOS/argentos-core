...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "argent-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...
      ARGENT_SKIP_GMAIL_WATCHER: "1",
      ARGENT_SKIP_CRON: "1",
      ARGENT_SKIP_BROWSER_CONTROL_SERVER: "1",
      ARGENT_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "argent-entry-bootstrap.mjs");
