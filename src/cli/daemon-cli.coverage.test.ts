...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    ...
    ARGENT_GATEWAY_PORT: process.env.ARGENT_GATEWAY_PORT,
    ARGENT_PROFILE: process.env.ARGENT_PROFILE,
  };

  beforeEach(() => {
    process.env.ARGENT_STATE_DIR = "/tmp/argent-cli-state";
    process.env.ARGENT_CONFIG_PATH = "/tmp/argent-cli-state/argent.json";
  ...
  afterEach(() => {
    if (originalEnv.ARGENT_STATE_DIR !== undefined) {
      process.env.ARGENT_STATE_DIR = originalEnv.ARGENT_STATE_DIR;
    } else {
      delete process.env.ARGENT_STATE_DIR;
    }

    if (originalEnv.ARGENT_CONFIG_PATH !== undefined) {
      process.env.ARGENT_CONFIG_PATH = originalEnv.ARGENT_CONFIG_PATH;
    } else {
      delete process.env.ARGENT_CONFIG_PATH;
    }

    if (originalEnv.ARGENT_GATEWAY_PORT !== undefined) {
      process.env.ARGENT_GATEWAY_PORT = originalEnv.ARGENT_GATEWAY_PORT;
    } else {
      delete process.env.ARGENT_GATEWAY_PORT;
    }

    if (originalEnv.ARGENT_PROFILE !== undefined) {
      process.env.ARGENT_PROFILE = originalEnv.ARGENT_PROFILE;
    } else {
      delete process.env.ARGENT_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ARGENT_PROFILE: "dev",
        ARGENT_STATE_DIR: "/tmp/argent-daemon-state",
        ARGENT_CONFIG_PATH: "/tmp/argent-daemon-state/argent.json",
        ARGENT_GATEWAY_PORT: "19001",
      },
