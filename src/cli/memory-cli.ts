...
import { collapseOperationalProfileSnapshots } from "../memory/hygiene/profile-collapse.js";
...
function parseTimeoutMs(timeout: unknown, fallbackMs: number): number {
  ...
  const parsed =
    typeof timeout === "number" && Number.isFinite(timeout)
      ...
      : typeof timeout === "string" && timeout.trim().length > 0
        ? Number.parseInt(timeout.trim(), 10)
        : Number.NaN;
...
export function registerMemoryCli(program: Command) {
  const memory = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.argent.ai/cli/memory")}\n`,
    );

  memory
    .command("contemplation-once")
    .description("Trigger one contemplation cycle through the running gateway")
    .option("--agent <id>", "Agent id (default: default agent)")
    .option("--json", "Print JSON")
    .option("--timeout <ms>", "Gateway timeout in milliseconds", "300000")
    .action(async (opts: MemoryCommandOptions) => {
      try {
        const timeoutMs = parseTimeoutMs(opts.timeout, 300_000);
        const cfg = loadConfig();
        const payload = await callGateway<{
          ...
          config: cfg,
          method: "contemplation.runOnce",
          params: opts.agent ? { agentId: opts.agent } : {},
          timeoutMs,
        });
        if (opts.json) {
        ...
        if (payload.status === "ran") {
          defaultRuntime.log(
            `Contemplation cycle ran for ${payload.agentId}.${typeof payload.isOk === "boolean" ? ` isOk=${payload.isOk}` : ""}`,
          );
        } else {
          defaultRuntime.log(
            `Contemplation cycle skipped for ${payload.agentId}: ${payload.reason ?? "unknown"}.`,
          );
        }
        if (typeof payload.lastRunMs === "number") {
          defaultRuntime.log(`Last run: ${new Date(payload.lastRunMs).toISOString()}`);
        }
        if (typeof payload.nextDueMs === "number") {
          defaultRuntime.log(`Next due: ${new Date(payload.nextDueMs).toISOString()}`);
        }
      } catch (err) {
        defaultRuntime.error(`Contemplation trigger failed: ${formatErrorMessage(err)}`);
        process.exitCode = 1;
      }
    });
  ...
  memory
    .command("collapse-profiles")
    .description(
      "Collapse historical low-signal operational profile snapshots into canonical memories",
    )
    .option("--apply", "Apply changes (default is dry-run preview)", false)
    .option("--json", "Print JSON")
    .option(
      "--sample <n>",
      "How many duplicate groups to include in the preview/output",
      (value: string) => Number(value),
      20,
    )
    .option("--fuzzy", "Enable fuzzy semantic clustering (more aggressive)", false)
    .action(async (opts: { apply?: boolean; json?: boolean; sample?: number; fuzzy?: boolean }) => {
      const sampleRaw =
        typeof opts.sample === "number" && Number.isFinite(opts.sample) ? opts.sample : 20;
      const maxSampleGroups = Math.max(0, Math.round(sampleRaw));
      ...
      try {
        ...
        const report = collapseOperationalProfileSnapshots(store, {
          dryRun,
          maxSampleGroups,
          enableFuzzy: Boolean(opts.fuzzy),
        });
        if (opts.json) {
        ...
        const modeLabel = report.dryRun ? "dry-run" : "applied";
        defaultRuntime.log(
          [
            `Memory operational profile collapse (${modeLabel})`,
            `scanned profiles: ${report.scannedProfiles}`,
            `operational profiles: ${report.operationalProfiles}`,
            `unique signatures: ${report.uniqueSignatures}`,
            `duplicate groups: ${report.duplicateGroups}`,
            `  exact groups: ${report.exactDuplicateGroups}`,
            `  token groups: ${report.tokenDuplicateGroups}`,
            `  fuzzy groups: ${report.fuzzyDuplicateGroups}`,
            `duplicates found: ${report.duplicatesFound}`,
            `groups collapsed: ${report.groupsCollapsed}`,
            `duplicates removed: ${report.duplicatesRemoved}`,
            `reinforcements applied: ${report.reinforcementsApplied}`,
          ].join("\n"),
        );

        if (report.samples.length > 0) {
          defaultRuntime.log("");
          defaultRuntime.log("sample groups:");
          for (const sample of report.samples) {
            defaultRuntime.log(
              `- ${sample.groupSize} items | canonical=${sample.canonicalId} | transfer=${sample.reinforcementTransfer}`,
            );
            defaultRuntime.log(`  ${truncate(sample.canonicalSummary)}`);
          }
        }
      } catch (err) {
        const message = formatErrorMessage(err);
        defaultRuntime.error(`Profile collapse failed: ${message}`);
        process.exitCode = 1;
      } finally {
        const { closeMemuStore } = await import("../memory/memu-store.js");
        closeMemuStore();
      }
    });
