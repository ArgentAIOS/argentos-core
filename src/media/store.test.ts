...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "ARGENT_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "argent-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.ARGENT_STATE_DIR = path.join(home, ".argent");
    ...
    await fs.mkdir(path.join(home, ".argent"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.argent${path.sep}media`);
