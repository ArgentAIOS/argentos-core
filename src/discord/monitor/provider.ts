...
import type { ArgentConfig, ReplyToMode } from "../../config/config.js";
...
export async function monitorDiscordProvider(opts: MonitorDiscordOpts = {}) {
  ...
  // Track close-1005 events to detect stale RESUME loops.
  // Carbon doesn't recognize 1005 as a session-death code, so we must
  // manually clear state and force IDENTIFY. If repeated state clears
  // don't fix it (token invalid, Discord outage, etc.), give up entirely.
  let consecutive1005Count = 0;
  let total1005Count = 0;
  let identifyAttempts = 0;
  const MAX_CONSECUTIVE_1005 = 3;
  const MAX_TOTAL_1005 = 15; // Give up after this many total 1005s
  const MAX_IDENTIFY_ATTEMPTS = 3; // Max fresh IDENTIFY cycles before giving up
  const onGatewayDebug = (msg: unknown) => {
    const message = String(msg);

    // Detect stale RESUME loop: Discord keeps closing with 1005 because the
    // session is invalid, but Carbon doesn't recognize 1005 as a "clear session"
    // code. After MAX_CONSECUTIVE_1005 failures, wipe session state and force
    // a fresh IDENTIFY.
    if (message.includes("closed with code 1005")) {
      consecutive1005Count++;
      total1005Count++;

      // Hard cap: too many total 1005s means something is fundamentally broken
      // (bad token, Discord outage, intent misconfiguration). Stop retrying.
      if (total1005Count >= MAX_TOTAL_1005 || identifyAttempts >= MAX_IDENTIFY_ATTEMPTS) {
        runtime.error?.(
          danger(
            `discord gateway: ${total1005Count} total 1005 closes across ${identifyAttempts} IDENTIFY attempts — giving up. Check bot token and intents.`,
          ),
        );
        // Set maxAttempts to 0 so Carbon stops reconnecting.
        // Also clear the heartbeat interval — Carbon doesn't do this on
        // disconnect(), so the heartbeat timer fires and tries to reconnect
        // a dead connection, causing an uncaught "zombie reconnect" error.
        if (gateway) {
          gateway.options.reconnect = { maxAttempts: 0 };
      ...
      if (consecutive1005Count >= MAX_CONSECUTIVE_1005 && gateway) {
        identifyAttempts++;
        runtime.log?.(
          danger(
            `discord gateway: ${consecutive1005Count} consecutive 1005 closes (attempt ${identifyAttempts}/${MAX_IDENTIFY_ATTEMPTS}) — stale session, forcing fresh IDENTIFY`,
          ),
        );
        // Clear Carbon's internal session state so canResume() returns false.
    ...
    // Reset counters on any successful state change (READY, RESUMED, etc.)
