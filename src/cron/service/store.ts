...
export async function ensureLoaded(state: CronServiceState, opts?: { forceReload?: boolean }) {
  ...
  for (const raw of jobs) {
    ...
    const executionModeRaw =
      typeof raw.executionMode === "string" ? raw.executionMode.trim().toLowerCase() : "";
    if (executionModeRaw === "paper_trade" || executionModeRaw === "live") {
      raw.executionMode = executionModeRaw;
    } else {
      raw.executionMode = "live";
      mutated = true;
    }
