...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-cron-"));
  return {
...
describe("CronService", () => {
  ...
  it("paper_trade mode blocks isolated live execution and records simulation evidence", async () => {
    ...
    const job = await cron.add({
      enabled: true,
      name: "paper trade isolated",
      schedule: { kind: "at", at: new Date(atMs).toISOString() },
      sessionTarget: "isolated",
      executionMode: "paper_trade",
      wakeMode: "now",
      payload: { kind: "agentTurn", message: "place a trade now" },
      delivery: { mode: "announce" },
    });
    ...
    expect(updated?.state.lastGateDecision).toBe("simulated_paper_trade");
    expect(updated?.state.lastExecutionMode).toBe("paper_trade");
    expect(updated?.state.lastSimulationEvidence?.action).toBe("isolated_agent_turn");
    expect(updated?.state.lastSimulationEvidence?.policy).toBe("external_side_effect_gate");

    cron.stop();
    await store.cleanup();
  });

  it("execution mode transition paper_trade -> live allows isolated execution", async () => {
    const store = await makeStorePath();
    const runIsolatedAgentJob = vi.fn(async () => ({
      status: "ok" as const,
      summary: "executed in live mode",
    }));
    ...
    const job = await cron.add({
      ...
      payload: { kind: "agentTurn", message: "execute side effects" },
      delivery: { mode: "none" },
    });
    ...
    await cron.update(job.id, { executionMode: "live" });
    await cron.run(job.id, "force");
    expect(runIsolatedAgentJob).toHaveBeenCalledTimes(1);

    const jobs = await cron.list({ includeDisabled: true });
    const updated = jobs.find((j) => j.id === job.id);
    expect(updated?.state.lastGateDecision).toBe("allow_live");
    expect(updated?.state.lastExecutionMode).toBe("live");
    expect(updated?.state.lastSimulationEvidence).toBeUndefined();

    cron.stop();
    await store.cleanup();
  });

  it("retires one-shot isolated jobs after first error when deleteAfterRun is true", async () => {
    const store = await makeStorePath();
    const enqueueSystemEvent = vi.fn();
    const requestHeartbeatNow = vi.fn();
    const runIsolatedAgentJob = vi.fn(async () => ({
    ...
    expect(updated?.state.lastError).toMatch(/delivery target is missing/i);
    expect(runIsolatedAgentJob).toHaveBeenCalledTimes(1);
