...
declare const __ARGENT_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __ARGENT_VERSION__ === "string" && __ARGENT_VERSION__) ||
  process.env.ARGENT_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `argent --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.ARGENT_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.ARGENT_GATEWAY_BIND ??
    // CLAWDBOT_GATEWAY_BIND: backward-compat fallback for pre-ArgentOS installs
