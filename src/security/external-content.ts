...
export function wrapWebContent(
  ...
  // Agent-initiated web fetches get content markers only (not the full
  // security warning). The heavy warning is for untrusted *inbound* content
  // like emails and webhooks — not for URLs the agent chose to fetch.
  return wrapExternalContent(content, { source, includeWarning: false });
}
