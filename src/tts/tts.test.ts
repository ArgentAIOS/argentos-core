...
import { completeSimple } from "../agent-core/ai.js";
...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    it("skips auto-TTS when inbound audio gating is on and the message is not audio", async () => {
      const prevPrefs = process.env.ARGENT_TTS_PREFS;
      process.env.ARGENT_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      ...
      process.env.ARGENT_TTS_PREFS = prevPrefs;
    });
