...
const execHostEnforced = process.env.ARGENT_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed = process.env.ARGENT_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureArgentCliOnPath({ pathEnv: process.env.PATH ?? "" });
