...
var ArgentA2UIHost = class extends i$6 {
	static properties = {
		surfaces: { state: true },
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--argent-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--argent-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--argent-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--argent-a2ui-empty-top, var(--argent-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  `;
	connectedCallback() {
		...
		for (const eventName of ["argent:a2ui-action-status"]) globalThis.addEventListener(eventName, this.#statusListener);
		this.#syncSurfaces();
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		if (this.#statusListener) {
			for (const eventName of ["argent:a2ui-action-status"]) globalThis.removeEventListener(eventName, this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		globalThis.__argentLastA2UIAction = userAction;
		const handler = globalThis.webkit?.messageHandlers?.argentCanvasA2UIAction ?? globalThis.argentCanvasA2UIAction;
		if (handler?.postMessage) try {
			if (handler === globalThis.argentCanvasA2UIAction) handler.postMessage(JSON.stringify({ userAction }));
...
if (!customElements.get("argent-a2ui-host")) customElements.define("argent-a2ui-host", ArgentA2UIHost);
