...
const QMD_UPDATE_TIMEOUT_MS = 120_000;
const QMD_EMBED_TIMEOUT_MS = 120_000;
...
export class QmdMemoryManager implements MemorySearchManager {
  ...
  private readonly cfg: ArgentConfig;
  ...
  private constructor(params: { cfg: ArgentConfig; agentId: string; resolved: ResolvedQmdConfig }) {
    this.cfg = params.cfg;
  ...
  async search(
    ...
    try {
      if (collectionNames.length > 1) {
        ...
        const args = this.buildSearchArgs("query", trimmed, limit);
      ...
      if (this.shouldRunEmbed(force)) {
        try {
          await runWithQmdEmbedLock(async () => {
            await this.runQmd(["embed"], { timeoutMs: QMD_EMBED_TIMEOUT_MS });
          });
  ...
  private async runQmdUpdateWithRetry(reason: string): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        await this.runQmd(["update"], { timeoutMs: QMD_UPDATE_TIMEOUT_MS });
        return;
      } catch (err) {
  ...
  private ensureDb(): SqliteDatabase {
    ...
    this.db = openDatabase(this.indexPath, { readonly: true });
  ...
  private buildSearchArgs(command: "query", query: string, limit: number): string[] {
