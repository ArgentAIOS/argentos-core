...
describe("memory hybrid helpers", () => {
  ...
  it("supports optional mmr re-ranking when enabled", async () => {
    const merged = await mergeHybridResults({
      vectorWeight: 1,
      textWeight: 0,
      mmr: { enabled: true, lambda: 0 },
      vector: [
        {
          id: "a",
          path: "memory/a.md",
    ...
    expect(merged[1]?.path).toBe("memory/c.md");
  });

  it("supports optional temporal decay when enabled", async () => {
    const merged = await mergeHybridResults({
      vectorWeight: 1,
      textWeight: 0,
      temporalDecay: { enabled: true, halfLifeDays: 10 },
      nowMs: Date.UTC(2026, 1, 24),
      vector: [
        {
          id: "new",
    ...
    expect(merged).toHaveLength(2);
    expect(merged[0]?.path).toBe("memory/2026-02-23.md");
    expect((merged[1]?.score ?? 0) < (merged[0]?.score ?? 0)).toBe(true);
  });
});
