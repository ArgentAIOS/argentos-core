...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedArgentRelay({
    baseUrl,
    relayAuthHeader: "x-argent-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ARGENT_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
  });
  ...
  it("accepts authenticated argent relay probe responses", async () => {
    ...
    await withRelayServer(
      (req, res) => {
        ...
        const header = req.headers["x-argent-relay-token"];
        seenToken = Array.isArray(header) ? header[0] : header;
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ Browser: "Argent/extension-relay" }));
      },
      async ({ port }) => {
