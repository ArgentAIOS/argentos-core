...
describe("browser config", () => {
  ...
  it("derives default ports from ARGENT_GATEWAY_PORT when unset", () => {
    const prev = process.env.ARGENT_GATEWAY_PORT;
    process.env.ARGENT_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const argent = resolveProfile(resolved, "argent");
      expect(argent?.cdpPort).toBe(19012);
      expect(argent?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.ARGENT_GATEWAY_PORT;
      } else {
        process.env.ARGENT_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const argent = resolveProfile(resolved, "argent");
      expect(argent?.cdpPort).toBe(19022);
      expect(argent?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.ARGENT_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "argent");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        argent: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("argent");
  });
});
