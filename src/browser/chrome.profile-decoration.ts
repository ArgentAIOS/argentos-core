...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".argent-profile-decorated");
}
 ...
 */
export function decorateArgentProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ARGENT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ARGENT_BROWSER_COLOR).toUpperCase();
