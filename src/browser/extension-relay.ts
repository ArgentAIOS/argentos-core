...
const RELAY_AUTH_HEADER = "x-argent-relay-token";
...
function envMsOrDefault(names: string[], fallback: number): number {
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const extensionReconnectGraceMs = envMsOrDefault(
    ["ARGENT_EXTENSION_RELAY_RECONNECT_GRACE_MS"],
    DEFAULT_EXTENSION_RECONNECT_GRACE_MS,
  );
  const extensionCommandReconnectWaitMs = envMsOrDefault(
    ["ARGENT_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS"],
    DEFAULT_EXTENSION_COMMAND_RECONNECT_WAIT_MS,
  );

  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    let relayAuthToken = "";
    let relayTokenDerivedFromGateway = false;
    let relayAuthTokens = new Set<string>();
    try {
      const acceptedTokens = resolveRelayAcceptedTokensForPort(info.port);
      relayAuthToken = acceptedTokens[0] ?? "";
      relayAuthTokens = new Set(acceptedTokens);
      relayTokenDerivedFromGateway = relayAuthToken.length > 0;
    } catch {
      // Preserve legacy behavior when gateway token is unavailable.
      relayAuthToken = randomBytes(32).toString("base64url");
      relayAuthTokens = new Set([relayAuthToken]);
    }

    let extensionWs: WebSocket | null = null;
    const cdpClients = new Set<WebSocket>();
    const connectedTargets = new Map<string, ConnectedTarget>();
    const syntheticBrowserSessions = new Map<string, WebSocket>();
    ...
    const closeCdpClientsAfterExtensionDisconnect = () => {
      ...
      syntheticBrowserSessions.clear();
    ...
    const createSyntheticBrowserSession = (ws: WebSocket): string => {
      const sessionId = `cb-browser-${nextBrowserSessionId++}`;
      syntheticBrowserSessions.set(sessionId, ws);
      return sessionId;
    };

    const isSyntheticBrowserSession = (sessionId: string | undefined): boolean => {
      if (!sessionId) {
        return false;
      }
      return syntheticBrowserSessions.has(sessionId);
    };

    const dropSyntheticBrowserSession = (sessionId: string | undefined): boolean => {
      if (!sessionId) {
        return false;
      }
      return syntheticBrowserSessions.delete(sessionId);
    };

    const dropSyntheticBrowserSessionsForClient = (ws: WebSocket) => {
      for (const [sessionId, owner] of syntheticBrowserSessions) {
        if (owner === ws) {
          syntheticBrowserSessions.delete(sessionId);
        }
      }
    };
    ...
    const routeCdpCommand = async (ws: WebSocket, cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/Argent-Extension-Relay",
            revision: "0",
            userAgent: "Argent-Extension-Relay",
            jsVersion: "V8",
          };
        case "Browser.setDownloadBehavior":
          return {};
        case "Runtime.runIfWaitingForDebugger":
          if (isSyntheticBrowserSession(cmd.sessionId)) {
            return {};
          }
          break;
        case "Target.attachToBrowserTarget":
          // Playwright creates a browser-scoped CDP session before attaching to a page
          // target via BrowserContext.newCDPSession(). chrome.debugger can only attach
          // to tab targets, so we emulate the browser session locally inside the relay.
          return { sessionId: createSyntheticBrowserSession(ws) };
        ...
        case "Target.detachFromTarget": {
          ...
          if (dropSyntheticBrowserSession(detachedSessionId)) {
            return {};
          }
          if (isSyntheticBrowserSession(cmd.sessionId)) {
            // Playwright detaches child CDP sessions from the synthetic browser
            // session when it cleans up newCDPSession(). Keep the underlying tab
            // attached and treat that as a local no-op.
            return {};
          }
          break;
        }
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "Argent/extension-relay",
    ...
    server.on("upgrade", (req, socket, head) => {
      ...
      if (pathname === "/extension") {
        const token = getRelayAuthTokenFromRequest(req, url);
        // Backward compatibility: older extension builds do not include relay auth token yet.
        if (token && !relayAuthTokens.has(token)) {
    ...
    wssCdp.on("connection", (ws) => {
            ...
            if (targetId) {
              ...
              if (target) {
                ws.send(
                  JSON.stringify({
                    method: "Target.attachedToTarget",
                    ...(isSyntheticBrowserSession(cmd.sessionId)
                      ? { sessionId: cmd.sessionId }
                      : {}),
                    params: {
                      sessionId: target.sessionId,
      ...
      ws.on("close", () => {
        ...
        dropSyntheticBrowserSessionsForClient(ws);
      });
    });
    ...
    try {
      ...
      if (
        relayTokenDerivedFromGateway &&
        isAddrInUseError(err) &&
        (await probeAuthenticatedArgentRelay({
