...
function localArgentProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "argent",
...
function createLocalArgentResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localArgentProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveArgentUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "argent-reset-"));
    const profileDir = path.join(tempRoot, "argent");
    ...
    const ops = createLocalArgentResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveArgentUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "argent-reset-no-own-"));
    const profileDir = path.join(tempRoot, "argent");
    fs.mkdirSync(profileDir, { recursive: true });
