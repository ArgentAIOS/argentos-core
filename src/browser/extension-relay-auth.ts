...
const RELAY_TOKEN_CONTEXT = "argent-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const ARGENT_RELAY_BROWSER = "Argent/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken = process.env.ARGENT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or ARGENT_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedArgentRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === ARGENT_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
