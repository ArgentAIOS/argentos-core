...
const originalGatewayToken = process.env.ARGENT_GATEWAY_TOKEN;
...
describe("chrome extension relay server", () => {
  ...
  afterEach(async () => {
    ...
    if (originalGatewayToken === undefined) {
      ...
      process.env.ARGENT_GATEWAY_TOKEN = originalGatewayToken;
    }
  });
  ...
  it("allows legacy extension websocket auth when gateway token is configured", async () => {
    const port = await getFreePort();
    cdpUrl = `http://127.0.0.1:${port}`;
    process.env.ARGENT_GATEWAY_TOKEN = "test-gateway-token";
    await ensureChromeExtensionRelayServer({ cdpUrl });
  ...
  it("emulates browser-scoped CDP sessions for Playwright", async () => {
    const port = await getFreePort();
    cdpUrl = `http://127.0.0.1:${port}`;
    await ensureChromeExtensionRelayServer({ cdpUrl });
    ...
    cdp.send(JSON.stringify({ id: 1, method: "Target.attachToBrowserTarget" }));
    const browserSessionResponse = JSON.parse(await q.next()) as {
      id?: number;
      result?: { sessionId?: string };
    };
    expect(browserSessionResponse.id).toBe(1);
    expect(String(browserSessionResponse.result?.sessionId ?? "")).toMatch(/^cb-browser-\d+$/);
    const browserSessionId = String(browserSessionResponse.result?.sessionId ?? "");
    ...
    const attachResponse = received.find((m) => m.id === 2);
    expect(attachResponse?.result?.sessionId).toBe("cb-tab-1");

    const attachEvent = received.find((m) => m.method === "Target.attachedToTarget");
    expect(attachEvent?.sessionId).toBe(browserSessionId);
    expect(JSON.stringify(attachEvent?.params ?? {})).toContain("t1");
    ...
    const runIfWaiting = JSON.parse(await q.next()) as { id?: number; error?: unknown };
    expect(runIfWaiting.id).toBe(3);
    expect(runIfWaiting.error).toBeUndefined();
    ...
    const detachResponse = JSON.parse(await q.next()) as { id?: number; error?: unknown };
    expect(detachResponse.id).toBe(4);
