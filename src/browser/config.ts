import type { BrowserConfig, BrowserProfileConfig, ArgentConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_ARGENT_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_ARGENT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "argent" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ARGENT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ARGENT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Argent browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for argent/argent CDP).
...
export function resolveBrowserConfig(
  cfg: BrowserConfig | undefined,
  rootConfig?: ArgentConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ARGENT_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "argent";
