...
import {
  decorateArgentProfile,
  ensureProfileCleanExit,
  isProfileDecorated,
} from "./chrome.profile-decoration.js";
import { DEFAULT_ARGENT_BROWSER_COLOR, DEFAULT_ARGENT_BROWSER_PROFILE_NAME } from "./constants.js";
...
export function resolveArgentUserDataDir(profileName = DEFAULT_ARGENT_BROWSER_PROFILE_NAME) {
...
export async function launchArgentChrome(
  ...
  const userDataDir = resolveArgentUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_ARGENT_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateArgentProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🤖 argent browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`argent browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`argent browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🤖 argent browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
