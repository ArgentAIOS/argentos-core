...
import { getLastPwAiLoadError, getPwAiModule as getPwAiModuleBase } from "../pw-ai-module.js";
...
export async function requirePwAi(
  ...
  const loadErr = getLastPwAiLoadError();
  const detail = loadErr ? `\nLoad error: ${loadErr.message}` : "";
  const nodeVersion = process.version;
  jsonError(
    res,
    501,
    [
      `Playwright module failed to load; '${feature}' is unavailable.${detail}`,
      `Node ${nodeVersion} — if using Homebrew node, switch to nvm (v22.x) for native module compatibility.`,
      "Run: pnpm rebuild playwright-core && argent gateway restart",
