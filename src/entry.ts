...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.ARGENT_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.ARGENT_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.ARGENT_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[argent] ${parsed.error}`);
    process.exit(2);
  }
