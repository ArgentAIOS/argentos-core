...
function describeApprovalRequest(request: ExecApprovalRequest): {
  kind: "exec" | "tool";
  title: string;
  subjectLabel: string;
  subjectValue: string;
} {
  const rawCommand = request.request.command.trim();
  const toolPrefix = "[TOOL_APPROVAL] ";
  if (rawCommand.startsWith(toolPrefix)) {
    const summary = rawCommand.slice(toolPrefix.length).trim() || "tool";
    return {
      kind: "tool",
  ...
  return {
    ...
    subjectValue: request.request.command,
  };
}

function buildRequestMessage(request: ExecApprovalRequest, nowMs: number) {
  const approval = describeApprovalRequest(request);
  const lines: string[] = [approval.title, `ID: ${request.id}`];
  lines.push(`${approval.subjectLabel}: ${approval.subjectValue}`);
...
function buildResolvedMessage(resolved: ExecApprovalResolved, request: ExecApprovalRequest) {
  const approval = describeApprovalRequest(request);
  const noun = approval.kind === "tool" ? "Tool" : "Exec";
  const base = `✅ ${noun} approval ${decisionLabel(resolved.decision)}.`;
...
function buildExpiredMessage(request: ExecApprovalRequest) {
  const approval = describeApprovalRequest(request);
  const noun = approval.kind === "tool" ? "Tool" : "Exec";
  return `⏱️ ${noun} approval expired. ID: ${request.id}`;
}
...
export function createExecApprovalForwarder(
  ...
  const handleResolved = async (resolved: ExecApprovalResolved) => {
    ...
    const cfg = getConfig();
    const text = buildResolvedMessage(resolved, entry.request);
