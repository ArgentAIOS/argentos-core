...
const REMOTE_BIN_PROBE_TIMEOUT_MS = 15_000;
const REMOTE_BIN_PROBE_RETRY_TIMEOUT_MS = 30_000;
const REMOTE_BIN_PROBE_RETRY_DELAY_MS = 750;
...
function isTimeoutMessage(message?: string): boolean {
  ...
  return normalized.includes("invoke timed out") || normalized.includes("timeout");
}
...
export async function refreshRemoteNodeBins(params: {
  ...
  try {
    ...
    const baseTimeoutMs = params.timeoutMs ?? REMOTE_BIN_PROBE_TIMEOUT_MS;
    const buildInvokeRequest = (timeoutMs: number) =>
      canWhich
        ? {
    ...
    let res = await remoteRegistry.invoke(buildInvokeRequest(baseTimeoutMs));
    const timeoutCode = res.error?.code;
    const timeoutMessage = res.error?.message;
    if (!res.ok && (timeoutCode === "TIMEOUT" || isTimeoutMessage(timeoutMessage))) {
      await new Promise((resolve) => setTimeout(resolve, REMOTE_BIN_PROBE_RETRY_DELAY_MS));
      res = await remoteRegistry.invoke(buildInvokeRequest(REMOTE_BIN_PROBE_RETRY_TIMEOUT_MS));
      if (res.ok) {
        log.info(`remote bin probe recovered after timeout (${describeNode(params.nodeId)})`);
      }
    }
    if (!res.ok) {
...
export async function refreshRemoteBinsForConnectedNodes(cfg: ArgentConfig) {
  if (!remoteRegistry) {
    return;
  }
