...
export function buildPortHints(listeners: PortListener[], port: number): string[] {
  ...
  if (kinds.has("gateway")) {
    hints.push(
      `Gateway already running locally. Stop it (${formatCliCommand("argent gateway stop")}) or use a different port.`,
    );
  }
  if (kinds.has("ssh")) {
    hints.push(
...
export function formatPortListener(listener: PortListener): string {
  ...
  const ppid = listener.ppid ? ` (ppid ${listener.ppid})` : "";
  ...
  return `${pid}${ppid}${user}: ${command}${address}`;
}
