...
export function triggerArgentRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.ARGENT_SYSTEMD_UNIT,
        process.env.ARGENT_PROFILE,
      );
  ...
  const label =
    process.env.ARGENT_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.ARGENT_PROFILE);
