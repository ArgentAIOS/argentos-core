...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ARGENT_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "ArgentOS";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(ArgentOS\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.ARGENT_MDNS_HOSTNAME?.trim() ||
    // CLAWDBOT_MDNS_HOSTNAME: backward-compat fallback for pre-ArgentOS installs
