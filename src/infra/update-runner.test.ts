...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-update-"));
  });
  ...
  it("skips preflight when already at upstream sha", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "argentos", version: "1.0.0" }),
    ...
    const { runner, calls } = createRunner({
      ...
      [`git -C ${tempDir} fetch --all --prune --tags`]: { stdout: "" },
      [`git -C ${tempDir} rev-parse @{upstream}`]: { stdout: "abc123" },
    });
  ...
  it("falls back when git root is not an argent checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
