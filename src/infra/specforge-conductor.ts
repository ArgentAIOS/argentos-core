...
function hasDevelopmentKickoffSignal(message: string): boolean {
  ...
  if (starterMatch && domainHits > 0) return true;
...
function parseProjectType(message: string): SpecforgeProjectType {
  ...
  if (brownfield && !greenfield) return "brownfield";
  if (greenfield && !brownfield) return "greenfield";
  return "unknown";
}
...
export async function shouldInvokeSpecforgeToolForMessage(params: {
  ...
  if (!text || text.startsWith("/")) return false;
  if (hasDevelopmentKickoffSignal(params.message)) return true;
