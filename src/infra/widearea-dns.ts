...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ARGENT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*argent-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "argent";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "argent");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "argent-gw");
  ...
  records.push(`_argent-gw._tcp IN PTR ${instanceLabel}._argent-gw._tcp`);
  records.push(`${instanceLabel}._argent-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._argent-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; argent-content-hash: ${contentHash}\n${contentBody}`;
}
