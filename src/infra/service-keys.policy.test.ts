...
const TEST_KEYCHAIN_ENV = { ARGENT_KEYCHAIN_DISABLE_WRITE: "1" };
...
describe("service key policy enforcement", () => {
  it("falls back to process env when a stored encrypted key is unreadable", async () => {
    await withTempHome(
      ...
      { env: { ...TEST_KEYCHAIN_ENV, BRAVE_API_KEY: "env-brave-secret" } },
    );
  });
