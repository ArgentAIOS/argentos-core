...
async function checkInstallSourceStatus(params: {
  root: string | null;
  installKind: "git" | "package" | "unknown";
}): Promise<InstallSourceStatus> {
  if (!params.root) {
    return {
      kind: "unknown",
      manifestPath: null,
  ...
  const artifactCandidates = [path.join(root, "dist", "entry.js"), path.join(root, "argent.mjs")];
  const manifestReady = await exists(manifestPath);
  ...
  if (!artifactPath) {
    artifactPath = artifactCandidates[0] ?? null;
  }

  const ready = manifestReady && artifactReady;
  let reason: string | undefined;
  if (!manifestReady && !artifactReady) {
    reason = "manifest-and-artifact-missing";
...
export async function fetchNpmTagVersion(params: {
  ...
  try {
    const res = await fetchWithTimeout(
      `https://registry.npmjs.org/argent/${encodeURIComponent(tag)}`,
      timeoutMs,
    );
    if (!res.ok) {
...
export async function checkUpdateStatus(params: {
  ...
  if (!root) {
    return {
      ...
      source: await checkInstallSourceStatus({ root, installKind: "unknown" }),
  ...
  const source = await checkInstallSourceStatus({ root, installKind });
