...
export function startJobOrchestratorRunner(opts: {
  ...
  // Guard: job orchestrator requires PostgreSQL — silently disable on SQLite
  const storageCfg = (cfg as unknown as { storage?: { backend?: string; readFrom?: string } })
    .storage;
  if (storageCfg?.backend === "sqlite" || storageCfg?.readFrom === "sqlite") {
    log.info(
      "job orchestrator disabled (requires PostgreSQL, current backend: " +
        (storageCfg?.backend ?? "sqlite") +
        ")",
    );
    const noopStatus: JobOrchestratorStatus = {
      enabled: false,
      pollMs: 0,
      running: false,
      lastCycleAt: null,
    ...
    return {
      ...
      enqueueEvent: async () => ({ accepted: false }),
    };
  }
  ...
  const hookHandler = (event: InternalHookEvent) => {
    ...
    void enqueueEvent({
      ...
      log.error(`hook event enqueue failed: ${err instanceof Error ? err.message : String(err)}`);
    });
  };
    ...
    try {
      ...
      if (!storageProbed) {
        ...
        if (storage instanceof SQLiteAdapter) {
          // Runtime guard for default installs without an explicit storage
          // config: jobs operations are unsupported on the SQLite adapter.
          log.info("job orchestrator disabled (requires PostgreSQL-backed storage)");
          stopRunner();
          return;
        }
      }
