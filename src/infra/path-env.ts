...
function candidateBinDirs(opts: EnsureArgentPathOpts): string[] {
  ...
  // Bundled macOS app: `argent` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "argent");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/argent` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "argent"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `argent` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureArgentCliOnPath(opts: EnsureArgentPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ARGENT_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.ARGENT_PATH_BOOTSTRAPPED = "1";
