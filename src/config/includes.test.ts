...
const ETC_ARGENT_DIR = path.join(ROOT_DIR, "etc", "argent");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "argent.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcArgentPath(...parts: string[]) {
  return path.join(ETC_ARGENT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcArgentPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "argent.json"))).toEqual({
      shared: true,
    });
  });
});
