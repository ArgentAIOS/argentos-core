import type { ArgentConfig } from "./config.js";
...
function isTelegramConfigured(cfg: ArgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: ArgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: ArgentConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: ArgentConfig): boolean {
...
function isIMessageConfigured(cfg: ArgentConfig): boolean {
...
function isWhatsAppConfigured(cfg: ArgentConfig): boolean {
...
function isGenericChannelConfigured(cfg: ArgentConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: ArgentConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: ArgentConfig, providerId: string): boolean {
...
function resolveConfiguredPlugins(cfg: ArgentConfig, env: NodeJS.ProcessEnv): PluginEnableChange[] {
...
function isPluginExplicitlyDisabled(cfg: ArgentConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: ArgentConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: ArgentConfig, pluginId: string): ArgentConfig {
...
function enablePluginEntry(cfg: ArgentConfig, pluginId: string): ArgentConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
