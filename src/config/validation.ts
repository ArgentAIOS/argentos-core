...
function validateIdentityAvatar(config: ArgentConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  ...
  const validated = ArgentSchema.safeParse(raw);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ArgentConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayAuthIssues = validateGatewayAuthConfig(validated.data as ArgentConfig);
  if (gatewayAuthIssues.length > 0) {
    return {
      ok: false,
      issues: gatewayAuthIssues.map((issue) => ({
        path: issue.path,
        message: issue.message,
      })),
    };
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as ArgentConfig)),
    ),
  };
}
