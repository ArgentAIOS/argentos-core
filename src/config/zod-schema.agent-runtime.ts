...
function enforceMinimumInterval(
  ...
  try {
    const ms = parseDurationMs(value, { defaultUnit: "m" });
    if (ms < minimumMs) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path,
        message: `${label} must be at least 30m`,
      });
    }
  } catch {
    ctx.addIssue({
...
export const HeartbeatSchema = z
  ...
  .strict()
  .superRefine((val, ctx) => {
    enforceMinimumInterval(val.every, ctx, ["every"], 30 * 60 * 1000, "heartbeat interval");
...
export const ExecutionWorkerSchema = z
  .object({
    ...
    maxTasksPerCycle: z.number().int().positive().optional(),
    scope: z
      .union([z.literal("assigned"), z.literal("agent-visible"), z.literal("all")])
      .optional(),
    requireEvidence: z.boolean().optional(),
    maxNoProgressAttempts: z.number().int().positive().optional(),
  })
  .strict()
  .superRefine((val, ctx) => {
    if (!val.every) {
...
export const ContemplationIntervalSchema = z
  .object({
    enabled: z.boolean().optional(),
    every: z.string().optional(),
    familyEvery: z.string().optional(),
    ...
    discoveryPhase: z
      .object({
        enabled: z.boolean().optional(),
        everyEpisodes: z.number().int().positive().optional(),
  ...
  .strict()
  .superRefine((val, ctx) => {
    enforceMinimumInterval(val.every, ctx, ["every"], 30 * 60 * 1000, "contemplation interval");
    if (val.familyEvery) {
      try {
        parseDurationMs(val.familyEvery, { defaultUnit: "m" });
      } catch {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["familyEvery"],
...
export const SisSchema = z
  .object({
    enabled: z.boolean().optional(),
    every: z.string().optional(),
    model: z.string().optional(),
    episodesPerConsolidation: z.number().int().positive().optional(),
    maxLessonsInPrompt: z.number().int().positive().optional(),
    lessonDecayDays: z.number().int().positive().optional(),
    lessonMinConfidence: z.number().min(0).max(1).optional(),
  })
  .strict()
  .superRefine((val, ctx) => {
    enforceMinimumInterval(val.every, ctx, ["every"], 30 * 60 * 1000, "SIS interval");
  })
  .optional();
...
export const ToolsWebSearchSchema = z
  .object({
    enabled: z.boolean().optional(),
    provider: z
      .union([z.literal("brave"), z.literal("perplexity"), z.literal("tinyfish")])
      .optional(),
  ...
  .object({
    ...
    tinyfish: z
      .object({
        ...
        apiKey: z.string().optional(),
        baseUrl: z.string().optional(),
        format: z.union([z.literal("markdown"), z.literal("html"), z.literal("json")]).optional(),
        timeoutSeconds: z.number().int().positive().optional(),
      })
      .strict()
      .optional(),
...
export const AgentEntrySchema = z
  .object({
    ...
    sis: SisSchema,
    executionWorker: ExecutionWorkerSchema,
...
export const ToolsSchema = z
  .object({
    ...
    alsoAllow: z.array(z.string()).optional(),
    deny: z.array(z.string()).optional(),
    departments: z.record(z.string(), ToolPolicyWithProfileSchema).optional(),
    byProvider: z.record(z.string(), ToolPolicyWithProfileSchema).optional(),
    web: ToolsWebSchema,
