...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-argent",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-argent#general");
  });
  ...
  it("derives session transcripts dir from ARGENT_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ARGENT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.ARGENT_STATE_DIR = "/custom/state";
    try {
