...
export function buildAgentSessionKey(params: {
  ...
  /** Channel/guild session scope. */
  channelScope?: "main" | "per-channel";
...
function listAgents(cfg: ArgentConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: ArgentConfig, agentId: string): string {
...
export function resolveAgentRoute(input: ResolveAgentRouteInput): ResolvedAgentRoute {
  ...
  const channelScope = input.cfg.session?.channelScope ?? "per-channel";
