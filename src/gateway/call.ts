...
export function buildGatewayConnectionDetails(
  options: { config?: ArgentConfig; url?: string; configPath?: string } = {},
...
export async function callGateway<T = Record<string, unknown>>(
  ...
  const timeoutMsRaw =
    typeof opts.timeoutMs === "number" && Number.isFinite(opts.timeoutMs)
      ...
      : 10_000;
  const timeoutMs = Math.min(MAX_NODE_TIMEOUT_MS, Math.max(1, timeoutMsRaw));
    ...
    (!urlOverride
      ? isRemoteMode
        ? typeof remote?.token === "string" && remote.token.trim().length > 0
          ? remote.token.trim()
          : undefined
        : // CLAWDBOT_GATEWAY_TOKEN: backward-compat fallback for pre-ArgentOS installs
  ...
  const password =
    ...
    (!urlOverride
      ? // CLAWDBOT_GATEWAY_PASSWORD: backward-compat fallback for pre-ArgentOS installs
