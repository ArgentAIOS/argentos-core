...
import type { ArgentConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export type GatewayReloadPlan = {
  ...
  restartSis: boolean;
  restartKnowledgeObservations: boolean;
...
type ReloadAction =
  ...
  | "restart-sis"
  | "restart-knowledge-observations"
  | "restart-kernel"
...
const BASE_RELOAD_RULES: ReloadRule[] = [
  ...
  {
    prefix: "agents.defaults.contemplation",
    kind: "hot",
    actions: ["restart-contemplation"],
  },
  {
    prefix: "agents.defaults.executionWorker",
    kind: "hot",
    actions: ["restart-execution-worker"],
  },
  {
    prefix: "agents.list",
    kind: "hot",
  ...
  {
    prefix: "memory.observations",
    kind: "hot",
    actions: ["restart-knowledge-observations"],
  },
...
export function resolveGatewayReloadSettings(cfg: ArgentConfig): GatewayReloadSettings {
...
export function buildGatewayReloadPlan(changedPaths: string[]): GatewayReloadPlan {
  const plan: GatewayReloadPlan = {
    ...
    restartSis: false,
    restartKnowledgeObservations: false,
    restartKernel: false,
  ...
  const applyAction = (action: ReloadAction) => {
    ...
    switch (action) {
      ...
      case "restart-heartbeat":
        ...
        break;
      case "restart-contemplation":
        plan.restartContemplation = true;
        break;
      case "restart-execution-worker":
        plan.restartExecutionWorker = true;
        break;
      case "restart-sis":
        plan.restartSis = true;
        break;
      case "restart-knowledge-observations":
        plan.restartKnowledgeObservations = true;
        break;
      case "restart-kernel":
...
export function startGatewayConfigReloader(opts: {
  initialConfig: ArgentConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: ArgentConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: ArgentConfig) => void;
  log: {
