...
describe("buildGatewayReloadPlan", () => {
  ...
  it("hot reloads execution worker for worker config changes", () => {
    const plan = buildGatewayReloadPlan(["agents.defaults.executionWorker.every"]);
    expect(plan.restartGateway).toBe(false);
    expect(plan.restartExecutionWorker).toBe(true);
  });
