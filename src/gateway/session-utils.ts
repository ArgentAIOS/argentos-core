...
export function resolveSessionSurface(key: string): string | undefined {
  if (!key) {
    return undefined;
  }
  ...
  const raw = (parsed?.rest ?? key).trim();
  if (!raw) {
    return undefined;
  }
...
function listConfiguredAgentIds(cfg: ArgentConfig): string[] {
...
export function listAgentsForGateway(cfg: ArgentConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
export function resolveSessionStoreKey(params: { cfg: ArgentConfig; sessionKey: string }): string {
  const raw = params.sessionKey.trim();
  if (!raw) {
    return raw;
  }
  if (raw === "global" || raw === "unknown" || raw === "__lastUserMessage") {
    return raw;
  }
...
function resolveSessionStoreAgentId(cfg: ArgentConfig, canonicalKey: string): string {
...
export function resolveGatewaySessionStoreTarget(params: { cfg: ArgentConfig; key: string }): {
  agentId: string;
  storePath: string;
  canonicalKey: string;
  storeKeys: string[];
...
export function loadCombinedSessionStoreForGateway(cfg: ArgentConfig): {
  storePath: string;
...
export function getSessionDefaults(cfg: ArgentConfig): GatewaySessionsDefaults {
...
export function listSessionsFromStore(params: {
  ...
  const kinds =
    Array.isArray(opts.kinds) && opts.kinds.length > 0
      ? new Set(opts.kinds.map((kind) => kind.toLowerCase()))
      : undefined;
  const surfaceFilter = typeof opts.surface === "string" ? opts.surface.trim().toLowerCase() : "";
  const channelFilter = typeof opts.channel === "string" ? opts.channel.trim().toLowerCase() : "";
  ...
  let sessions = Object.entries(store)
    ...
    .map(([key, entry]) => {
      ...
      const surface = resolveSessionSurface(key);
  ...
  if (kinds && kinds.size > 0) {
    sessions = sessions.filter((session) => kinds.has(session.kind.toLowerCase()));
  }

  if (surfaceFilter) {
    sessions = sessions.filter(
      (session) => (session.surface ?? "").toLowerCase() === surfaceFilter,
    );
  }

  if (channelFilter) {
    sessions = sessions.filter(
      (session) => (session.channel ?? "").toLowerCase() === channelFilter,
    );
  }
