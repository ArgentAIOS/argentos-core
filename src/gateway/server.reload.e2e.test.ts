...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.ARGENT_SKIP_CHANNELS;
    prevSkipGmail = process.env.ARGENT_SKIP_GMAIL_WATCHER;
    process.env.ARGENT_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.ARGENT_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.ARGENT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ARGENT_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
