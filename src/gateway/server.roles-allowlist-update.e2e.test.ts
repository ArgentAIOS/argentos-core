...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".argent", "restart-sentinel.json");
  ...
  test("does not restart when update fails", async () => {
    const sigusr1 = vi.fn();
    process.on("SIGUSR1", sigusr1);
    ...
    try {
      ...
      ws.send(
        JSON.stringify({
          ...
          method: "update.run",
          params: {
            sessionKey: "agent:main:webchat:test-failed-update",
            restartDelayMs: 0,
          },
        }),
      );
      const res = await onceMessage<{ ok: boolean; payload?: { ok?: boolean; restart?: unknown } }>(
        ws,
        (o) => o.type === "res" && o.id === id,
      );
      expect(res.ok).toBe(true);
      expect(res.payload?.ok).toBe(false);
      expect(res.payload?.restart ?? null).toBeNull();
