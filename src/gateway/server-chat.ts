...
export function createAgentEventHandler({
  ...
  return (evt: AgentEventPayload) => {
    ...
    const shouldSanitizeAssistantText =
      evt.stream === "assistant" ||
      (evt.stream === "lifecycle" && lifecyclePhase === "run_complete");
    const sanitizedData =
      shouldSanitizeAssistantText && evt.data
        ? (() => {
            const next = { ...evt.data };
            if (typeof next.text === "string") {
              next.text = sanitizeAssistantDisplayText(next.text);
            }
            if (typeof next.content === "string") {
              next.content = sanitizeAssistantDisplayText(next.content);
            }
            return next;
          })()
        : evt.data;
    ...
    const agentEvent =
      sanitizedData !== evt.data
        ? ({
            ...evt,
            data: sanitizedData,
          } satisfies AgentEventPayload)
        : evt;
    const agentPayload = sessionKey ? { ...agentEvent, sessionKey } : agentEvent;
    ...
    if (isToolEvent) {
      ...
      // AEVP: emit fine-grained activity state for tool events
      if (evt.data?.phase === "start") {
        const toolName = typeof evt.data.name === "string" ? evt.data.name : "unknown";
        const toolCategory = classifyToolActivity(
          toolName,
          evt.data.args as Record<string, unknown> | undefined,
        );
        console.log(`[AEVP] activity → working (${toolName} → ${toolCategory})`);
        broadcast(
          "aevp_activity",
          {
            type: "activity_state",
        ...
        console.log("[AEVP] activity → thinking");
        broadcast(
          "aevp_activity",
          {
            type: "activity_state",
