...
describe("agent event handler", () => {
  ...
  it("strips leaked tool json from assistant events", () => {
    const broadcast = vi.fn();
    const broadcastToConnIds = vi.fn();
    const nodeSendToSession = vi.fn();
    const agentRunSeq = new Map<string, number>();
    const chatRunState = createChatRunState();
    const toolEventRecipients = createToolEventRecipientRegistry();
    chatRunState.registry.add("run-json", { sessionKey: "session-1", clientRunId: "client-1" });
    ...
    const agentPayload = agentCalls[0]?.[1] as { data?: { text?: string } };
    expect(agentPayload.data?.text).toBe("");
    expect(nodeSendToSession.mock.calls.filter(([, event]) => event === "chat")).toHaveLength(0);
  });
  ...
  it("routes tool events only to registered recipients when verbose is enabled", () => {
    ...
    // broadcast may fire for aevp_activity, but NOT for chat/tool payloads
    const chatBroadcasts = broadcast.mock.calls.filter(([event]: [string]) => event === "chat");
