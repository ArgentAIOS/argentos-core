...
describe("listSessionsFromStore filters", () => {
  const baseCfg = {
    session: { mainKey: "main" },
    agents: { list: [{ id: "main", default: true }] },
  } as ArgentConfig;
  ...
  test("filters by kind", () => {
    const result = listSessionsFromStore({
      ...
      storePath: "/tmp/sessions.json",
      store,
      opts: { includeGlobal: true, kinds: ["group"] },
    });
    expect(result.sessions.length).toBe(1);
    expect(result.sessions[0]?.key).toBe("agent:main:discord:group:dev-team");
  });
  ...
  test("filters by surface", () => {
    const result = listSessionsFromStore({
      ...
      storePath: "/tmp/sessions.json",
      store,
      opts: { includeGlobal: true, surface: "webchat" },
    });
    expect(result.sessions.length).toBe(1);
    expect(result.sessions[0]?.key).toBe("agent:main:webchat-111");
  });
  ...
  test("filters by channel", () => {
    const result = listSessionsFromStore({
      ...
      storePath: "/tmp/sessions.json",
      store,
      opts: { includeGlobal: true, channel: "discord" },
    });
    expect(result.sessions.length).toBe(1);
    expect(result.sessions[0]?.key).toBe("agent:main:discord:group:dev-team");
  });

  test("combines kind and surface filters", () => {
    const result = listSessionsFromStore({
      cfg: baseCfg,
      storePath: "/tmp/sessions.json",
      store,
      opts: { includeGlobal: true, kinds: ["direct"], surface: "webchat" },
    });
    expect(result.sessions.length).toBe(1);
    expect(result.sessions[0]?.key).toBe("agent:main:webchat-111");
  });
