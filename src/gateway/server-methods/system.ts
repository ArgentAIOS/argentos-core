...
export const systemHandlers: GatewayRequestHandlers = {
  ...
  "contemplation.runOnce": async ({ params, respond, context }) => {
    const agentId =
      typeof params.agentId === "string" && params.agentId.trim().length > 0
        ? params.agentId.trim()
        : undefined;
    if (!context.contemplationRunner) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INTERNAL_ERROR, "contemplation runner unavailable"),
      );
      return;
    }
    const result = await context.contemplationRunner.runNow(agentId);
