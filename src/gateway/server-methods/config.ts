...
function invalidConfigMessage(issues: Array<{ path: string; message: string }>): string {
  const first = issues[0];
  if (!first) {
    return "invalid config";
  }
  const path = first.path?.trim() || "<root>";
  return `invalid config: ${path}: ${first.message}`;
}
  ...
  "config.set": async ({ params, respond }) => {
    ...
    if (!validated.ok) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, invalidConfigMessage(validated.issues), {
