...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.ARGENT_STATE_DIR;
  previousConfigPath = process.env.ARGENT_CONFIG_PATH;
  previousSkipBrowserControl = process.env.ARGENT_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.ARGENT_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.ARGENT_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "argent-gateway-home-"));
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "argent-test-"))
    : path.join(tempHome, ".argent-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.ARGENT_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.ARGENT_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.ARGENT_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.ARGENT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ARGENT_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.ARGENT_SKIP_CANVAS_HOST;
    } else {
      process.env.ARGENT_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.ARGENT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ARGENT_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.ARGENT_GATEWAY_PASSWORD;
