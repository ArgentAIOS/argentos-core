...
export function sanitizeChatMessageForDisplay(message: unknown): unknown | null {
  if (!message || typeof message !== "object") {
    return null;
  }
  ...
  if (role === "assistant") {
    ...
    const text =
      Array.isArray(content) || typeof content === "string"
        ? extractAssistantText(message as AssistantMessage)
  ...
  return stripEnvelopeFromMessage(message);
}

export function sanitizeChatHistoryMessages(messages: unknown[]): unknown[] {
  const stripped = stripEnvelopeFromMessages(messages);
  return stripped
    .map((message) => sanitizeChatMessageForDisplay(message))
