...
export function resolveGatewayAuth(params: {
  ...
  // CLAWDBOT_GATEWAY_TOKEN/PASSWORD: backward-compat fallback for pre-ArgentOS installs
  const token =
    authConfig.token ?? env.ARGENT_GATEWAY_TOKEN ?? env.CLAWDBOT_GATEWAY_TOKEN ?? undefined;
  const password =
    authConfig.password ??
    env.ARGENT_GATEWAY_PASSWORD ??
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or ARGENT_GATEWAY_TOKEN)",
    );
  }
