...
export async function handleToolsInvokeHttpRequest(
      ...
      sendJson(res, 400, {
        ...
        error: {
          ...
          message:
            ...
            "Enable by configuring a memory plugin slot (and ensure plugins.enabled is not false).",
        },
      });
      return true;
    }
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-argent-message-channel") ?? "");
  const accountId = getHeader(req, "x-argent-account-id")?.trim() || undefined;
  ...
  const allTools = createArgentTools({
