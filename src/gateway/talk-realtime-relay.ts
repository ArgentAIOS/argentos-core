...
import type { RealtimeVoiceProvider } from "../realtime-voice/provider-types.js";
...
export const TALK_REALTIME_RELAY_EVENT = "talk.realtime.relay";
...
export type TalkRealtimeRelayEvent =
  ...
  | { relaySessionId: string; type: "close"; reason: RealtimeVoiceCloseReason };
...
function broadcastToOwner(
  ...
  context.broadcastToConnIds(TALK_REALTIME_RELAY_EVENT, event, new Set([connId]), {
    dropIfSlow: true,
  });
}

function closeRelaySession(session: RelaySession, reason: RealtimeVoiceCloseReason): void {
  relaySessions.delete(session.id);
  clearTimeout(session.cleanupTimer);
  session.bridge.close(reason);
...
export function clearTalkRealtimeRelaySessionsForTest(): void {
  for (const session of relaySessions.values()) {
    clearTimeout(session.cleanupTimer);
    session.bridge.close("cancelled");
  }
  relaySessions.clear();
}
