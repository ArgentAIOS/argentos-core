...
export function createGatewayReloadHandlers(params: {
  ...
  const applyHotReload = async (
    ...
    if (plan.restartContemplation) {
      nextState.contemplationRunner.updateConfig(nextConfig);
      nextState.sisRunner.updateConfig(nextConfig);
    }

    if (plan.restartExecutionWorker) {
      nextState.executionWorkerRunner.updateConfig(nextConfig);
      nextState.jobOrchestratorRunner.updateConfig(nextConfig);
    }
    ...
    if (plan.restartGmailWatcher) {
      ...
      if (!isTruthyEnvValue(process.env.ARGENT_SKIP_GMAIL_WATCHER)) {
        ...
        params.logHooks.info("skipping gmail watcher restart (ARGENT_SKIP_GMAIL_WATCHER=1)");
      }
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (ARGENT_SKIP_CHANNELS=1 or ARGENT_SKIP_PROVIDERS=1)",
        );
      } else {
    ...
    setCommandLaneConcurrency(CommandLane.Interactive, resolveAgentMaxConcurrent(nextConfig));
    setCommandLaneConcurrency(CommandLane.Subagent, resolveSubagentMaxConcurrent(nextConfig));
    setCommandLaneConcurrency(CommandLane.Background, resolveBackgroundMaxConcurrent(nextConfig));
