...
function normalizeSessionToolList(raw: unknown): string[] {
...
export async function applySessionsPatchToStore(params: {
  ...
  if ("toolsAllow" in patch) {
    const raw = patch.toolsAllow;
    if (raw === null) {
      delete next.toolsAllow;
    } else if (raw !== undefined) {
      next.toolsAllow = normalizeSessionToolList(raw);
    }
  }

  if ("toolsDeny" in patch) {
    const raw = patch.toolsDeny;
    if (raw === null) {
      delete next.toolsDeny;
    } else if (raw !== undefined) {
      next.toolsDeny = normalizeSessionToolList(raw);
    }
  }
