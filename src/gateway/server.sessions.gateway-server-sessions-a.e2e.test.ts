...
beforeAll(async () => {
  previousToken = process.env.ARGENT_GATEWAY_TOKEN;
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.ARGENT_GATEWAY_TOKEN = previousToken;
  }
});
...
describe("gateway server sessions", () => {
  ...
  test("lists and patches session store via sessions.* RPC", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-sessions-"));
    ...
    const resetStoreRaw = await fs.readFile(storePath, "utf-8");
    const resetStore = JSON.parse(resetStoreRaw) as Record<string, { [key: string]: unknown }>;
    const globalAfterReset = resetStore["__lastUserMessage"];
    expect(globalAfterReset).toBeTruthy();
    expect(typeof globalAfterReset?.sessionClearedAt).toBe("number");
    expect(globalAfterReset?.sessionClearedFromKey).toBe("agent:main:main");
    expect(globalAfterReset?.sessionClearedReason).toBe("sessions.reset");
  ...
  test("sessions.preview returns transcript previews", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-sessions-preview-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
