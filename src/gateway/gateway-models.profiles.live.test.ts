...
import type { ArgentConfig, ModelProviderConfig } from "../config/types.js";
...
import { ensureArgentModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.ARGENT_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ARGENT_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.ARGENT_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<ArgentConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.ARGENT_AGENT_DIR,
  ...
  const hostAgentDir = resolveArgentAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-live-state-"));
  ...
  const toolProbePath = path.join(workspaceDir, `.argent-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveArgentAgentDir();
  const sanitizedCfg: ArgentConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Argent live tool probe (local, safe): " +
    ...
    process.env.ARGENT_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ARGENT_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureArgentModelsJson(cfg);
      ...
      const rawModels = process.env.ARGENT_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.argent-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
