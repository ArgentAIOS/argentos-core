...
const ANTHROPIC_PREFIXES = ["claude-opus-4-6", "claude-sonnet-4-6", "claude-haiku-4-5"];
...
const ZAI_PREFIXES = ["glm-5.1", "glm-5", "glm-4.7"];
...
export function isModernModelRef(ref: ModelRef): boolean {
  ...
  if (provider === "nvidia") {
    return matchesPrefix(id, NVIDIA_PREFIXES);
  }
