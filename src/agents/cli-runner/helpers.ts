...
/**
 * Cleanup suspended Argent CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: ArgentConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-cli-images-"));
  const paths: string[] = [];
...
export async function prepareCliMcpConfig(params: {
  backend: CliBackendConfig;
  workspaceDir: string;
}): Promise<{ configPath: string; cleanup?: () => Promise<void> } | null> {
  const configuredPath = params.backend.mcpConfigPath?.trim();
  if (configuredPath) {
    const resolvedPath = path.isAbsolute(configuredPath)
      ? configuredPath
      : path.resolve(params.workspaceDir, configuredPath);
    let stat;
    try {
      stat = await fs.stat(resolvedPath);
    } catch {
      throw new Error(`CLI backend MCP config not found: ${resolvedPath}`);
    }
    if (!stat.isFile()) {
      throw new Error(`CLI backend MCP config is not a file: ${resolvedPath}`);
    }
    return { configPath: resolvedPath };
  }

  const servers = params.backend.mcpServers;
  if (!servers || typeof servers !== "object" || Object.keys(servers).length === 0) {
    return null;
  }

  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-cli-mcp-"));
...
export function appendCliMcpArgs(params: {
  args: string[];
  backend: CliBackendConfig;
  mcpConfigPath?: string;
}): string[] {
  const mcpConfigPath = params.mcpConfigPath?.trim();
  if (!mcpConfigPath) {
    return [...params.args];
  }

  const args = [...params.args];
  const mcpConfigArg = params.backend.mcpConfigArg?.trim();
  if (mcpConfigArg && !args.includes(mcpConfigArg)) {
    args.push(mcpConfigArg, mcpConfigPath);
  }

  const strictArg = params.backend.strictMcpConfigArg?.trim();
  const strictEnabled = params.backend.strictMcpConfig !== false;
  if (strictEnabled && strictArg && !args.includes(strictArg)) {
    args.push(strictArg);
  }
