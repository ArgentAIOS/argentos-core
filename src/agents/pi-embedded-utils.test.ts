...
{"status": "ok", "data": "some result"}`,
  ...
  it("strips standalone tool-action json text", () => {
    const msg: AssistantMessage = {
      role: "assistant",
      content: [
        {
          type: "text",
          text: '{"command":"sed -n \\"2440,2585p\\" /tmp/file","workdir":"/tmp","yieldMs":12000,"timeout":25}',
        },
      ],
      timestamp: Date.now(),
    };
...
describe("stripGptThinkingTitles", () => {
  it("strips a typical GPT thinking title", () => {
    expect(stripGptThinkingTitles("Composing affirmative warm response\nHere is my reply.")).toBe(
      "Here is my reply.",
    );
  });

  it("strips various gerund-phrase titles", () => {
    expect(stripGptThinkingTitles("Crafting concise narrative response\nDone.")).toBe("Done.");
    expect(stripGptThinkingTitles("Summarizing test results concisely\nResults below.")).toBe(
      "Results below.",
    );
    expect(stripGptThinkingTitles("Drafting helpful reply\nSure!")).toBe("Sure!");
  });

  it("does not strip lines with punctuation", () => {
    const text = "Creating a new project involves steps.\nLet me explain.";
    expect(stripGptThinkingTitles(text)).toBe(text);
  });

  it("does not strip lines longer than 60 characters", () => {
    const longTitle =
      "Generating extremely detailed comprehensive elaborate extended response content\nActual reply.";
    expect(stripGptThinkingTitles(longTitle)).toBe(longTitle);
  });

  it("does not strip when first word is not a gerund", () => {
    const text = "Hello world\nActual content.";
    expect(stripGptThinkingTitles(text)).toBe(text);
  });

  it("does not strip when there is no newline", () => {
    const text = "Composing affirmative warm response";
    expect(stripGptThinkingTitles(text)).toBe(text);
  });

  it("does not strip when text contains uppercase words after gerund", () => {
    const text = "Creating New Project\nContent here.";
    expect(stripGptThinkingTitles(text)).toBe(text);
  });

  it("returns empty string for empty input", () => {
    expect(stripGptThinkingTitles("")).toBe("");
  });

  it("preserves legitimate gerund-starting paragraphs", () => {
    // Real content that starts with a gerund but has punctuation or is long
    const text = "Running the tests showed 3 failures.\nHere are the details.";
    expect(stripGptThinkingTitles(text)).toBe(text);
  });

  it("strips title from extractAssistantText pipeline", () => {
    const msg: AssistantMessage = {
      role: "assistant",
      content: [
        {
          type: "text",
          text: "Crafting concise narrative response\nHere is what I found.",
        },
      ],
      timestamp: Date.now(),
    };
