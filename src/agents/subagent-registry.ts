...
import { MAX_NODE_TIMEOUT_MS, resolveAgentTimeoutMs } from "./timeout.js";
...
async function waitForSubagentCompletion(runId: string, waitTimeoutMs: number) {
  try {
    const timeoutMs = Math.min(MAX_NODE_TIMEOUT_MS, Math.max(1, Math.floor(waitTimeoutMs)));
    const callTimeoutMs = Math.min(MAX_NODE_TIMEOUT_MS, timeoutMs + 10_000);
