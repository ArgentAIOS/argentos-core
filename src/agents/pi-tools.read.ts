...
import { createEditTool, createReadTool, createWriteTool } from "../agent-core/coding.js";
...
export function createSandboxedReadTool(root: string) {
  ...
  return wrapSandboxPathGuard(createArgentReadTool(base), root);
}
...
/**
 * Try to resolve a workspace-skill path when the model constructs an incorrect
 * skills path. The LLM often pattern-matches bundled skill paths (repo dir)
 * for workspace-only skills. When the requested path doesn't exist but matches
 * `<something>/skills/<name>/SKILL.md`, check the workspace skills dir.
 */
function tryResolveSkillFallbackPath(
...
export function createArgentReadTool(
  base: AnyAgentTool,
  ...
  return {
    ...
    execute: async (toolCallId, params, signal) => {
      ...
      try {
        ...
        // Skill path fallback: if the read failed with ENOENT and the path looks
        // like a skill path, try the workspace skills dir instead.
        const code =
          err && typeof err === "object" && "code" in err
            ? (err as { code: string }).code
            : undefined;
        if (code === "ENOENT" || (err instanceof Error && err.message.includes("ENOENT"))) {
          const fallback = tryResolveSkillFallbackPath(filePath, opts?.workspaceDir);
          if (fallback) {
            const fallbackParams = {
              ...(normalized ?? (params as Record<string, unknown>)),
              path: fallback,
            };
            const result = await base.execute(toolCallId, fallbackParams, signal);
            const normalizedResult = await normalizeReadImageResult(result, fallback);
            return sanitizeToolResultImages(normalizedResult, `read:${fallback}`);
          }
        }
        throw err;
      }
