...
import { SessionManager } from "../agent-core/coding.js";
...
describe("tool_result_persist hook", () => {
  it("sanitizes oversized toolResult payloads before persistence", () => {
    ...
    sm.appendMessage({
      role: "assistant",
      content: [{ type: "toolCall", id: "call_oversized", name: "heygen_video", arguments: {} }],
    } as AgentMessage);
  ...
  it("composes transforms in priority order and allows stripping toolResult.details", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "argent-toolpersist-"));
