...
async function mirrorSubagentReportToRequesterSession(params: {
  ...
  const conciseReply = (params.reply || "").trim();
  const report = [
    `Family agent report (${childAgentId || "subagent"})`,
    `Task: ${params.taskLabel}`,
    `Outcome: ${params.outcomeLabel}`,
    conciseReply ? `Findings: ${conciseReply}` : "Findings: (no output)",
    params.statsLine,
  ].join("\n");
  ...
  await callGateway({
    ...
    params: {
      ...
      label: `Family:${childAgentId || "subagent"}`,
      message: report,
    },
    timeoutMs: 15_000,
  });
...
export async function runSubagentAnnounceFlow(params: {
  ...
  try {
    ...
    // Auto-complete team tasks assigned to this subagent and update member status.
    // This fires BEFORE announce delivery so task state is current regardless of
    // which delivery path is taken (direct, steered, queued).
    try {
      const storage = await getStorageAdapter();
      const assignedTasks = await storage.tasks.list({
        assignee: params.childSessionKey,
        status: ["pending", "in_progress", "blocked"],
      });
      const completedTaskIds: string[] = [];
      for (const task of assignedTasks) {
        if (outcome.status === "ok" || outcome.status === "unknown") {
          await storage.tasks.complete(task.id);
          completedTaskIds.push(task.id);
        } else {
          await storage.tasks.fail(task.id, `subagent outcome: ${outcome.status}`);
        }
      }

      // Resolve blocked dependents when one of their dependencies just completed.
      if (completedTaskIds.length > 0) {
        ...
        for (const blocked of blockedTasks) {
          ...
          for (const depId of blocked.dependsOn) {
            const depTask = await storage.tasks.get(depId);
          ...
          if (allDepsCompleted) {
            await storage.tasks.update(blocked.id, { status: "pending" });
          }
        }
      }
      ...
      if (teamIds.size === 0) {
        ...
        for (const team of teams) {
          ...
          if (full?.members.some((m) => m.sessionKey === params.childSessionKey)) {
            teamIds.add(team.id);
          }
        }
      }
      if (teamIds.size > 0) {
        const memberStatus =
          outcome.status === "error" || outcome.status === "timeout" ? "failed" : "completed";
        for (const teamId of teamIds) {
          await storage.teams.updateMemberStatus(teamId, params.childSessionKey, memberStatus);
        }
      }
    } catch {
      // Best-effort — don't break the announce flow
    }
    ...
    // Mirror raw family-agent completion context into the requester's transcript.
    // This keeps the orchestrator context aware of parallel lane outcomes even when
    // the user-facing announce is queued/steered/delivered separately.
    try {
      await mirrorSubagentReportToRequesterSession({
      ...
      // Best-effort — announcement flow must continue.
    }
  ...
  // ── Post-task reflection for family agents ──
  // After a family agent completes a task, fire a short reflection session
  // so the agent can extract lessons and publish high-confidence ones.
  try {
    void maybeFireFamilyReflection({
      childSessionKey: params.childSessionKey,
      task: params.task,
    ...
    // Best-effort — never break the announce flow
  }
...
/**
 * If the completed subagent belongs to a registered family agent,
 * fire a reflection session in the cron lane so it can learn from the task.
 */
async function maybeFireFamilyReflection(params: {
  childSessionKey: string;
  task: string;
  outcome?: SubagentRunOutcome;
  reply?: string;
  ...
  // to catch any family agent regardless of how they were spawned.
  let isFamilyAgent = false;
  try {
    const { getAgentFamily } = await import("../data/agent-family.js");
    const family = await getAgentFamily();
    const agent = await family.getAgent(agentId);
    isFamilyAgent = agent !== null && agent.status === "active";
  } catch {
  ...
  const reflectionPrompt = [
    ...
    "",
    `You just completed a task: ${params.task.slice(0, 300)}`,
    `Outcome: ${status}`,
    "",
    "Brief output:",
    replySnippet,
    ...
    "3. If you discovered a high-confidence lesson or pattern, store it via memory_store and publish it to the family library via the family tool (action: publish).",
    "",
    "Keep it concise. Focus on actionable lessons.",
  ].join("\n");

  try {
    const reflectionSessionKey = `${params.childSessionKey}:reflection`;
    await callGateway({
      method: "agent",
      params: {
        ...
        lane: "cron",
        idempotencyKey: `family-reflection-${Date.now()}`,
        extraSystemPrompt:
          "You are reflecting on a completed task. Extract lessons and store valuable ones. Be brief.",
      },
      timeoutMs: 120_000,
    });
  } catch {
    // Best-effort — reflection failure is non-critical
  }
}
