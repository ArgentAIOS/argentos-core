...
function resolveProviderEnvVarCandidates(normalizedProvider: string): string[] | null {
  ...
  if (normalizedProvider === "qwen-portal") {
    return ["QWEN_OAUTH_TOKEN", "QWEN_PORTAL_API_KEY"];
  }

  if (normalizedProvider === "minimax-portal") {
  ...
  const envVar = PROVIDER_ENV_KEY_MAP[normalizedProvider];
  return envVar ? [envVar] : null;
}

async function resolveServiceKeyForProvider(
  provider: string,
  cfg?: ArgentConfig,
  ...
  try {
    ...
    const resolveServiceKeyAsync = mod.resolveServiceKeyAsync as (
      envName: string,
      cfg?: ArgentConfig,
    ) => Promise<string | undefined>;

    for (const envVar of candidates) {
      const value = (await resolveServiceKeyAsync(envVar, cfg))?.trim();
      if (value) {
...
export async function resolveApiKeyForProvider(params: {
  ...
  const serviceKeyResolved = await resolveServiceKeyForProvider(provider, cfg);
  if (serviceKeyResolved) {
    return {
      apiKey: serviceKeyResolved.apiKey,
      source: serviceKeyResolved.source,
      mode: serviceKeyResolved.source.includes("OAUTH_TOKEN") ? "oauth" : "api-key",
    };
  }
