...
import { __testing, createArgentCodingTools } from "./pi-tools.js";
...
describe("createArgentCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createArgentCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "argent-outside.txt");
