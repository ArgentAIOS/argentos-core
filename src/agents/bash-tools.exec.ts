...
const DEFAULT_PENDING_MAX_OUTPUT = clampNumber(
  readEnvInt("ARGENT_BASH_PENDING_MAX_OUTPUT_CHARS"),
  200_000,
  1_000,
  200_000,
);
        ...
        run.promise
          .then((outcome) => {
            ...
            if (outcome.status === "failed") {
              // Return non-zero exits as a resolved result instead of rejecting.
              // The agent sees the stderr/stdout and exit code; rejecting would
              // trigger logError in the tool adapter for every failed command
              // (e.g. jq receiving null input), spamming logs during autonomous
              // search cycles.
              resolve({
                content: [
                  {
                    type: "text",
                    text: `${getWarningText()}${outcome.reason ?? "Command failed."}`,
                  },
                ],
                details: {
                  status: "failed",
