...
export function resolveSubagentToolPolicy(cfg?: ArgentConfig): SandboxToolPolicy {
...
function resolveDepartmentIdForAgent(
  ...
  const raw = config.intent?.agents?.[normalizedAgentId]?.departmentId;
...
function normalizeSessionToolList(raw: unknown): string[] | undefined {
...
function normalizePolicyToolList(raw: unknown): string[] | undefined {
  return normalizeSessionToolList(raw);
}

function resolveSessionToolPolicy(params: {
  config?: ArgentConfig;
  sessionKey?: string;
  ...
  const entry =
    store[sessionKey] ??
    (sessionKey === "main" ? store[`agent:${agentId}:main`] : undefined) ??
    undefined;
  if (!entry) {
    return undefined;
  }
  const allow = normalizeSessionToolList(entry.toolsAllow);
  const deny = normalizeSessionToolList(entry.toolsDeny);
  if (!allow && !deny) {
    return undefined;
  }
  return { allow, deny };
...
export function resolveEffectiveToolPolicy(params: {
  ...
  const departmentId = resolveDepartmentIdForAgent(params.config, agentId);
  const departmentTools =
    departmentId && globalTools?.departments ? globalTools.departments[departmentId] : undefined;

  const profile = agentTools?.profile ?? departmentTools?.profile ?? globalTools?.profile;
  ...
  return {
    ...
    globalAsk: normalizePolicyToolList(globalTools?.ask),
    departmentPolicy: pickToolPolicy(departmentTools),
    departmentAsk: normalizePolicyToolList(departmentTools?.ask),
    globalProviderPolicy: pickToolPolicy(providerPolicy),
    agentPolicy: pickToolPolicy(agentTools),
    agentAsk: normalizePolicyToolList(agentTools?.ask),
    agentProviderPolicy: pickToolPolicy(agentProviderPolicy),
    sessionPolicy: resolveSessionToolPolicy({
    ...
    profileAlsoAllow: Array.isArray(agentTools?.alsoAllow)
      ...
      : Array.isArray(departmentTools?.alsoAllow)
        ? departmentTools?.alsoAllow
...
export function isToolMatchedByPolicyList(name: string, patterns?: string[]): boolean {
  if (!Array.isArray(patterns) || patterns.length === 0) {
    return false;
  }
  const compiled = compilePatterns(patterns);
  return matchesAny(normalizeToolName(name), compiled);
}
