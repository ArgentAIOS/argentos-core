import type { AgentEvent } from "../agent-core/core.js";
...
export function handleToolExecutionEnd(
  ...
  const taskMutationEvidence = extractTaskMutationEvidence(toolName, toolArgs, sanitizedResult);
  if (taskMutationEvidence) {
    ctx.state.taskMutationEvidence.push(taskMutationEvidence);
  }
  ...
  // Observability: warn when the same tool appears 3+ times in the last 10 outcomes
  const recentOutcomes = ctx.state.toolOutcomes.slice(-10);
  const sameToolCount = recentOutcomes.filter((o) => o.toolName === toolName).length;
  if (sameToolCount >= 3) {
    ctx.log.warn(
      `tool repetition detected: "${toolName}" called ${sameToolCount} times in last ${recentOutcomes.length} tool outcomes (runId=${ctx.params.runId})`,
    );
  }
