...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "argent.sandbox=1");
  args.push("--label", `argent.sessionKey=${params.scopeKey}`);
  args.push("--label", `argent.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `argent.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("argent.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`argent sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`argent sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("argent sandbox recreate --all");
}
