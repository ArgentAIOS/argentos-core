...
vi.mock("../models-config.js", () => ({
  ensureArgentModelsJson: vi.fn(async () => {}),
}));
...
vi.mock("../agent-paths.js", () => ({
  resolveArgentAgentDir: vi.fn(() => "/tmp/agent-dir"),
}));
...
vi.mock("../failover-error.js", () => ({
  ...
  resolveRetryAfterMs: vi.fn(() => undefined),
}));
...
vi.mock("../pi-embedded-helpers.js", async () => {
  const isContextOverflowError = (msg?: string) => {
    if (!msg) {
      return false;
    }
  ...
  return {
    ...
    isLikelyContextOverflowError: (msg?: string) => {
      ...
      return isContextOverflowError(msg);
    },
    ...
    isModelUnavailableErrorMessage: vi.fn(() => false),
  };
});
...
describe("overflow compaction in run loop", () => {
  ...
  it("returns user-facing fallback payload when interactive run has no content", async () => {
    ...
    expect(result.payloads?.[0]?.isError).toBe(true);
    expect(result.payloads?.[0]?.text).toContain("I couldn't generate a reply");
  });
});
