...
export function modelSupportsImages(model: {
  ...
  // Some provider catalogs may lag capability metadata for newer OpenAI models.
  // Keep image input enabled for known vision-capable GPT families.
  if (
    (provider === "openai" || provider === "openai-codex") &&
    (/^gpt-5(?:[.-]|$)/.test(id) || /^gpt-4\.1(?:[.-]|$)/.test(id) || /^gpt-4o(?:[.-]|$)/.test(id))
  ) {
    return true;
  }
