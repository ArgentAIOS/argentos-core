...
[media attached 1/2: /Users/tyleryust/.argentos/media/IMG_6430.jpeg (image/jpeg)]
[media attached 2/2: /Users/tyleryust/.argentos/media/IMG_6431.jpeg (image/jpeg)]
...
what is this?`;
  ...
  it("handles paths with spaces in filename", () => {
    ...
    const prompt = `[media attached: /Users/test/.argentos/media/ChatGPT Image Apr 21, 2025.png (image/png) | https://example.com/same.png]
what is this?`;
    const refs = detectImageReferences(prompt);
...
describe("modelSupportsImages", () => {
  ...
  it("returns true for GPT-5 family even if input metadata is text-only", () => {
    const model = { input: ["text"], provider: "openai", id: "gpt-5.3-codex" };
    expect(modelSupportsImages(model)).toBe(true);
  });
...
describe("detectAndLoadPromptImages", () => {
  it("preserves existing prompt images even when the active model lacks native vision", async () => {
