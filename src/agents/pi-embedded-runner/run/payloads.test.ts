...
describe("buildEmbeddedRunPayloads", () => {
  ...
  it("suppresses internal tool-loop budget abort errors", () => {
    const payloads = buildEmbeddedRunPayloads({
      assistantTexts: [],
      toolMetas: [],
      lastAssistant: undefined,
      lastToolError: {
        toolName: "exec",
        error:
          '[tool-loop-budget] Tool "exec" hit its per-turn budget after 6 calls. Do not call this tool again in this turn.',
      },
      sessionKey: "session:telegram",
      inlineToolResultsAllowed: false,
      verboseLevel: "off",
