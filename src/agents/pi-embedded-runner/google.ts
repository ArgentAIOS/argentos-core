...
import type { AgentMessage, AgentTool } from "../../agent-core/core.js";
...
import {
  ...
  stripSimulatedToolCallText,
  stripToolBlocksForNonAnthropicProvider,
...
export async function sanitizeSessionHistory(params: {
  ...
  const modelChanged = priorSnapshot
    ...
    : false;
  // Strip tool blocks when switching providers so the new provider doesn't
  // choke on Anthropic-format toolu_01... IDs in the history.
  const didStripToolBlocks = policy.stripToolBlocks && modelChanged;
  const strippedTools = didStripToolBlocks
    ? stripToolBlocksForNonAnthropicProvider(repairedTools)
    : repairedTools;
  // Strip legacy simulated tool text ("[Used tool: X]", "[Performed tool operations]")
  // ONLY for the actual Anthropic provider — these patterns cause Anthropic models
  // to imitate the text instead of making real tool_use blocks.
  // Do NOT strip when we just ran stripToolBlocksForNonAnthropicProvider in this
  // same pipeline invocation (would double-strip, leaving empty messages).
  // Also do NOT match on modelApi === "anthropic-messages" — other providers
  // (e.g. MiniMax) use that API format but are not Anthropic.
  const isActualAnthropicProvider = params.provider === "anthropic";
  const decontaminated =
    isActualAnthropicProvider && !didStripToolBlocks
      ? stripSimulatedToolCallText(strippedTools)
      : strippedTools;

  const sanitizedOpenAI =
    isOpenAIResponsesApi && modelChanged
      ? downgradeOpenAIReasoningBlocks(decontaminated)
      : decontaminated;
