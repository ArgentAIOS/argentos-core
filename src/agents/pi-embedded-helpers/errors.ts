...
export function isLikelyContextOverflowError(errorMessage?: string): boolean {
  ...
  // Rate limit messages contain "request would exceed ... rate limit" which
  // falsely matches the broad CONTEXT_OVERFLOW_HINT_RE regex. Exclude them
  // before checking the heuristic pattern.
...
export function formatAssistantErrorText(
  ...
  if (isContextOverflowError(raw)) {
    const normalized = raw
      ...
      .replace(/\s+/g, " ")
      .trim();
    if (/^context overflow:/i.test(normalized) && !/[{}]/.test(normalized)) {
      return normalized;
    }
    return (
...
export function sanitizeUserFacingText(text: string): string {
  ...
  if (ERROR_PREFIX_RE.test(trimmed)) {
    if (isModelUnavailableErrorMessage(trimmed)) {
      return "The selected model is unavailable or retired. Please choose another model.";
    }
...
export function classifyFailoverReason(raw: string): FailoverReason | null {
  ...
  if (isServerErrorMessage(raw)) {
    return "rate_limit"; // treat upstream failures as transient — trigger failover
  }
  return null;
}
