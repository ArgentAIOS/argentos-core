...
function makeCfg(overrides: Partial<ArgentConfig> = {}): ArgentConfig {
  return {
    agents: {
      defaults: {
        model: {
...
describe("runWithModelFallback", () => {
  ...
  it("skips provider when provider-level cooldown is active", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-auth-"));
    const provider = `provider-cooldown-${crypto.randomUUID()}`;
    const profileId = `${provider}:default`;
