...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): ArgentConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCodexConfig = (opts?: { apiKey?: string }): ArgentConfig =>
  ({
    models: {
      providers: {
        "openai-codex": {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-workspace-"));
    try {
  ...
  it("falls back from codex spark on tool-call contract mismatch", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-workspace-"));
    try {
      runEmbeddedAttemptMock
        .mockResolvedValueOnce(
          makeAttempt({
            assistantTexts: ['{"action":"open","request":{"kind":"navigate"}}'],
            lastAssistant: buildAssistant({
              api: "openai-codex-responses",
              provider: "openai-codex",
              model: "gpt-5.3-codex-spark",
              stopReason: "stop",
              content: [{ type: "text", text: '{"action":"open","request":{"kind":"navigate"}}' }],
            }),
            toolMetas: [],
          }),
        )
        .mockResolvedValueOnce(
          makeAttempt({
            ...
            toolMetas: [{ toolName: "browser" }],
          }),
        );

      await runEmbeddedPiAgent({
        sessionId: "session:test",
        sessionKey: "agent:test:codex-spark-tool-fallback",
        sessionFile: path.join(workspaceDir, "session.jsonl"),
        workspaceDir,
        agentDir,
        config: makeCodexConfig(),
        prompt: "navigate to the docs and summarize",
        provider: "openai-codex",
        model: "gpt-5.3-codex-spark",
        authProfileIdSource: "auto",
        timeoutMs: 5_000,
        runId: "run:codex-spark-tool-fallback",
      });
      ...
      const secondCall = runEmbeddedAttemptMock.mock.calls[1]?.[0] as
        | { modelId?: string }
        | undefined;
      expect(firstCall?.modelId).toBe("gpt-5.3-codex-spark");
      expect(secondCall?.modelId).toBe("gpt-5.3-codex");
    } finally {
      await fs.rm(agentDir, { recursive: true, force: true });
      await fs.rm(workspaceDir, { recursive: true, force: true });
    }
