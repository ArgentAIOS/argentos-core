...
describe("runCliAgent resume cleanup", () => {
  ...
  it("injects MCP config flags for claude-cli when inline mcpServers are configured", async () => {
    ...
    const firstCall = runCommandWithTimeoutMock.mock.calls[0];
    expect(firstCall?.[0]?.[0]).toBe("claude");
    const argv = firstCall?.[0] as string[];
    const mcpIndex = argv.indexOf("--mcp-config");
    expect(mcpIndex).toBeGreaterThan(-1);
    expect(argv[mcpIndex + 1]).toContain("argent-cli-mcp-");
    expect(argv).toContain("--strict-mcp-config");
  });

  it("uses explicit mcpConfigPath and honors strictMcpConfig=false", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-cli-runner-mcp-test-"));
    const mcpPath = path.join(tempDir, "mcp.json");
    await fs.writeFile(mcpPath, '{"mcpServers":{"demo":{"command":"echo"}}}\n', "utf8");
    ...
    try {
      ...
      const mcpIndex = argv.indexOf("--mcp-config");
      expect(mcpIndex).toBeGreaterThan(-1);
      expect(argv[mcpIndex + 1]).toBe(mcpPath);
  ...
  it("throws clear error when explicit mcpConfigPath does not exist", async () => {
    await expect(
      runCliAgent({
        sessionId: "s4",
        sessionFile: "/tmp/session.jsonl",
          ...
          agents: {
            defaults: {
              cliBackends: {
                "claude-cli": {
                  command: "claude",
                  mcpConfigPath: "/definitely/missing/mcp-config.json",
                },
              },
            },
          },
    ...
    ).rejects.toThrow(/MCP config not found/i);
  });
});
