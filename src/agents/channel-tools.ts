...
import {
  isPublicCorePluginAllowed,
  isPublicCorePluginToolAllowed,
...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as ArgentConfig);
...
export function listChannelAgentTools(params: { cfg?: ArgentConfig }): ChannelAgentTool[] {
  // Channel docking: aggregate channel-owned tools (login, etc.).
  const tools: ChannelAgentTool[] = [];
  const publicCoreGate = resolvePublicCorePluginRuntimeGate(params.cfg);
  for (const plugin of listChannelPlugins()) {
    if (!isPublicCorePluginAllowed(plugin.id, publicCoreGate)) {
      continue;
    }
    ...
    if (Array.isArray(resolved)) {
      tools.push(
        ...resolved.filter((tool) => isPublicCorePluginToolAllowed(tool.name, publicCoreGate)),
      );
    }
  }
  return tools;
