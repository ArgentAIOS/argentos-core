...
describe("resolveApiKeyForProfile fallback to main agent", () => {
  ...
  beforeEach(async () => {
    ...
    // Set environment variables so resolveArgentAgentDir() returns mainAgentDir
    process.env.ARGENT_STATE_DIR = tmpDir;
    process.env.ARGENT_AGENT_DIR = mainAgentDir;
  ...
  it("falls back to main agent credentials when secondary agent token is expired and refresh fails", async () => {
    ...
    // The merge in ensureAuthProfileStore now picks the fresher credential
    // from main, so the fallback path is never reached and the secondary
    // agent's on-disk store is not rewritten.  Verify the on-disk store
    // was NOT overwritten with the stale expired token (it keeps whatever
    // was there before — the original expired cred or a minimal stub).
    const updatedSecondaryStore = JSON.parse(
      await fs.readFile(path.join(secondaryAgentDir, "auth-profiles.json"), "utf8"),
    ) as AuthProfileStore;
    // On-disk secondary store was not updated by the fallback path,
    // but the in-memory merged store correctly used the fresh main token.
    expect(updatedSecondaryStore.profiles[profileId]).toBeDefined();
  });
