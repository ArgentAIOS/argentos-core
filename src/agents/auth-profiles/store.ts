...
function _syncAuthProfileStore(target: AuthProfileStore, source: AuthProfileStore): void {
  ...
  target.providerStats = source.providerStats;
}
  ...
  return {
    ...
    usageStats:
      record.usageStats && typeof record.usageStats === "object"
        ...
        : undefined,
    providerStats:
      record.providerStats && typeof record.providerStats === "object"
        ? (record.providerStats as Record<string, ProviderUsageStats>)
        : undefined,
  };
}
...
function mergeProfiles(
  base: Record<string, AuthProfileCredential>,
  override: Record<string, AuthProfileCredential>,
): Record<string, AuthProfileCredential> {
  const merged = { ...base };
  for (const [key, overrideCred] of Object.entries(override)) {
    const baseCred = base[key];
    if (!baseCred) {
      merged[key] = overrideCred;
      continue;
    }
    // For OAuth/token credentials present in both stores, keep the fresher one
    if (
      baseCred.type === overrideCred.type &&
      (baseCred.type === "oauth" || baseCred.type === "token")
    ) {
      merged[key] =
        credentialExpiry(overrideCred) > credentialExpiry(baseCred) ? overrideCred : baseCred;
    } else {
      // Different types or api_key: override wins (preserves existing behavior)
      merged[key] = overrideCred;
    }
  }
  return merged;
...
function mergeAuthProfileStores(
  ...
  return {
    ...
    providerStats: mergeRecord(base.providerStats, override.providerStats),
  };
}
...
function loadAuthProfileStoreForAgent(
  ...
  if (agentDir) {
    ...
    if (mainStore && Object.keys(mainStore.profiles).length > 0) {
      // Save minimal stub; credentials come from main via merge at runtime
      saveJsonFile(authPath, { version: AUTH_STORE_VERSION, profiles: {} });
...
export function saveAuthProfileStore(store: AuthProfileStore, agentDir?: string): void {
  ...
  const payload = {
    ...
    providerStats: store.providerStats ?? undefined,
