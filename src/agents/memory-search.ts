...
import type { ArgentConfig, MemorySearchConfig } from "../config/config.js";
...
const DEFAULT_HYBRID_MMR_ENABLED = false;
const DEFAULT_HYBRID_MMR_LAMBDA = 0.7;
const DEFAULT_HYBRID_TEMPORAL_DECAY_ENABLED = false;
...
function mergeConfig(
  ...
  const mmrLambda = clampNumber(hybrid.mmr.lambda, 0, 1);
  const temporalHalfLifeDays = Math.max(
    0.0001,
    clampNumber(hybrid.temporalDecay.halfLifeDays, 0.0001, Number.MAX_SAFE_INTEGER),
  );
    ...
    query: {
      ...
      hybrid: {
        ...
        temporalDecay: {
          ...
          halfLifeDays: temporalHalfLifeDays,
        },
      },
    },
    cache: {
