...
function wrapWebFetchContent(
  ...
  if (WEB_FETCH_WRAPPER_OVERHEAD > maxChars) {
    const minimal = wrapWebContent("", "web_fetch");
  ...
  let wrappedText = wrapWebContent(truncated.text, "web_fetch");
...
export async function fetchFirecrawlContent(params: {
  ...
  if (dashboardApi) {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Firecrawl proxy error (${res.status}): ${detail || res.statusText}`);
    }
    ...
    if (payload?.success === false) {
      throw new Error(`Firecrawl proxy error: ${payload.error || "unknown"}`);
    }
...
async function runWebFetch(params: {
  ...
  try {
    const result = await fetchWithSsrFGuard({
      ...
      init: {
        headers: {
          // Prefer markdown when available (Cloudflare Markdown-for-Agents).
          // Falls back gracefully to HTML → Readability extraction.
  ...
  try {
    ...
    // Capture Cloudflare Markdown-for-Agents metadata when present.
    ...
    if (contentType.includes("text/markdown")) {
      // Server returned native markdown (Cloudflare Markdown-for-Agents or similar).
      // Use directly — no Readability extraction needed.
    ...
    // Cloudflare Markdown-for-Agents metadata.
    if (markdownTokens) {
      payload.markdownTokens = Number(markdownTokens) || markdownTokens;
    }
    if (contentSignal) {
      payload.contentSignal = contentSignal;
    }
    writeCache(FETCH_CACHE, cacheKey, payload, params.cacheTtlMs);
    return payload;
  } finally {
