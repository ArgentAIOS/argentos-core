...
export function createSessionStatusTool(opts?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      const previousProvider = resolved.entry.providerOverride?.trim() || configured.provider;
      const previousModel = resolved.entry.modelOverride?.trim() || configured.model;
      ...
      const currentLabel = `${previousProvider}/${previousModel}`;
      const nextLabel = `${providerForCard}/${resolved.entry.modelOverride?.trim() || configured.model}`;
      const statusTextWithChangeNote = changedModel
        ? `${statusText}\n\nℹ️ Model override saved for next run (${currentLabel} → ${nextLabel}).`
        : statusText;

      return {
        content: [{ type: "text", text: statusTextWithChangeNote }],
        details: {
          ok: true,
          sessionKey: resolved.key,
          changedModel,
          statusText: statusTextWithChangeNote,
        },
      };
    },
  };
