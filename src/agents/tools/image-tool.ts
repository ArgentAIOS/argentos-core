...
import { type Api, type Context, complete, type Model } from "../../agent-core/ai.js";
...
function resolveDefaultModelRef(cfg?: ArgentConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
function pickMaxBytes(cfg?: ArgentConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: ArgentConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureArgentModelsJson(effectiveCfg, params.agentDir);
