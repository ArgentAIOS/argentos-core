...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-oauth-"));

    try {
      process.env.ARGENT_STATE_DIR = tempDir;
      process.env.ARGENT_AGENT_DIR = path.join(tempDir, "agent");
      ...
      const store = ensureAuthProfileStore(process.env.ARGENT_AGENT_DIR, {
  ...
  it("suggests openai-codex when only Codex OAuth is configured", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-auth-"));
  ...
  it("resolves Inception API key from env", async () => {
    const previousInceptionKey = process.env.INCEPTION_API_KEY;

    try {
      process.env.INCEPTION_API_KEY = "inception-test-key";
      ...
      expect(resolved.apiKey).toBe("inception-test-key");
      expect(resolved.source).toContain("INCEPTION_API_KEY");
    } finally {
      if (previousInceptionKey === undefined) {
        delete process.env.INCEPTION_API_KEY;
      } else {
        process.env.INCEPTION_API_KEY = previousInceptionKey;
      }
    }
  });

  it("resolves Inception API key from service-keys when env is missing", async () => {
    const previousInceptionKey = process.env.INCEPTION_API_KEY;
    ...
    try {
      ...
      expect(resolved.apiKey).toBe("inception-service-key");
      expect(resolved.source).toContain("INCEPTION_API_KEY");
