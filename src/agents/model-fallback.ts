...
export async function runWithModelFallback<T>(params: {
  ...
  for (let i = 0; i < candidates.length; i += 1) {
    ...
    if (authStore) {
      // Clear any cooldowns that have already expired so stale state
      // doesn't block profiles that are ready to retry.
      clearExpiredCooldowns(authStore);

      if (isProviderInCooldown(authStore, candidate.provider)) {
        const until = resolveProviderUnusableUntilForDisplay(authStore, candidate.provider);
        attempts.push({
          provider: candidate.provider,
          model: candidate.model,
          error:
            typeof until === "number"
              ? `Provider ${candidate.provider} is in cooldown until ${new Date(until).toISOString()}`
