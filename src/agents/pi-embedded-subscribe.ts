...
export function subscribeEmbeddedPiSession(params: SubscribeEmbeddedPiSessionParams) {
  ...
  const finalizeAssistantTexts = (args: {
    ...
    // When block replies are disabled, final message text should be authoritative.
    // Replace any interim streamed snippets with the final cleaned text.
  ...
  const resetForCompactionRetry = () => {
    ...
    state.taskMutationEvidence.length = 0;
  ...
  return {
    ...
    getTaskMutationEvidence: () =>
      state.taskMutationEvidence.map((entry) => ({
        ...entry,
