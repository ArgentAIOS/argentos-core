...
describe("sanitizeSessionHistory", () => {
  ...
  it("downgrades openai reasoning only when the model changes", async () => {
    ...
    // When models change, stripToolBlocksForNonAnthropicProvider converts thinking
    // blocks to "[Performed tool operations]" text before downgradeOpenAIReasoningBlocks
    // runs.  stripSimulatedToolCallText then tries to remove the placeholder but keeps
    // the original message when all blocks would be stripped.
    expect(result).toHaveLength(1);
    expect(result[0]?.role).toBe("assistant");
  });
});
