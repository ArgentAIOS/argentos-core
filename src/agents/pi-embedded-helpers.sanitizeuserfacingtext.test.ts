...
describe("sanitizeUserFacingText", () => {
  ...
  it("preserves specific context overflow guidance when already user-facing", () => {
    const result = sanitizeUserFacingText(
      "Context overflow: auto-compaction could not reduce context enough. Try /compact, then resend a shorter request.",
    );
    expect(result).toContain("auto-compaction could not reduce context enough");
  });
