...
export function isCliProvider(provider: string, cfg?: ArgentConfig): boolean {
...
function normalizeAnthropicModelId(model: string): string {
  ...
  if (lower === "sonnet-4.5" || lower === "sonnet-4.6") {
...
export function resolveConfiguredModelRef(params: {
  ...
  if (rawModel) {
    ...
    if (!trimmed.includes("/")) {
      ...
      console.warn(
        `[argent] Model "${trimmed}" specified without provider. Falling back to "anthropic/${trimmed}". Please use "anthropic/${trimmed}" in your config.`,
      );
...
export function selectionDiffersFromConfiguredPrimary(params: {
  configPrimary?: string;
  provider?: string;
  model?: string;
  defaultProvider: string;
  defaultModel: string;
  ...
  const selectedProvider = normalizeProviderId(
    (params.provider ?? "").trim() || normalizeProviderId(params.defaultProvider),
  );
  const selectedModel = (params.model ?? "").trim() || params.defaultModel.trim();
  if (!selectedProvider || !selectedModel) {
    return false;
  }

  const rawPrimary = params.configPrimary?.trim();
  if (!rawPrimary) {
    return true;
  }
  const configured = parseModelRef(rawPrimary, params.defaultProvider);
  if (!configured) {
    return true;
  }

  const configuredProvider = normalizeProviderId(configured.provider);
  const configuredModel = configured.model.trim();
  return configuredProvider !== selectedProvider || configuredModel !== selectedModel;
}
...
/**
 * Resolve the best reasoning-capable model from the catalog.
 * Used by the gateway to auto-upgrade to a powerful model when deep-think mode is activated.
 * Returns null if no reasoning model is available.
 */
export function resolveBestReasoningModel(params: {
  catalog: ModelCatalogEntry[];
  allowedKeys: Set<string>;
  allowAny: boolean;
