...
export async function runCliAgent(params: {
  ...
  let preparedMcpConfig: Awaited<ReturnType<typeof prepareCliMcpConfig>> = null;
  try {
    preparedMcpConfig = await prepareCliMcpConfig({
      backend,
      workspaceDir,
    });
    cleanupMcpConfig = preparedMcpConfig?.cleanup;
  } catch (err) {
    if (cleanupImages) {
      await cleanupImages();
  ...
  const resolvedArgs = useResume
    ...
    : baseArgs;
  const resolvedArgsWithMcp = appendCliMcpArgs({
    args: resolvedArgs,
    backend,
    mcpConfigPath: preparedMcpConfig?.configPath,
  });
  const args = buildCliArgs({
    backend,
    baseArgs: resolvedArgsWithMcp,
  ...
  try {
    const output = await enqueueCliRun(queueKey, async () => {
      ...
      const logOutputText = isTruthyEnvValue(process.env.ARGENT_CLAUDE_CLI_LOG_OUTPUT);
      if (logOutputText) {
