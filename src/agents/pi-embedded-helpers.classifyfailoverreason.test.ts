...
describe("classifyFailoverReason", () => {
  ...
  it("classifies decommissioned model errors as overloaded failover", () => {
    expect(
      classifyFailoverReason(
        "400 The model `qwen-qwq-32b` has been decommissioned and is no longer supported.",
      ),
    ).toBe("overloaded");
    expect(classifyFailoverReason("model not found: groq/unknown-model")).toBe("overloaded");
  });
});
