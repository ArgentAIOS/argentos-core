...
describe("buildAgentSystemPrompt", () => {
  ...
  it("omits extended sections in minimal prompt mode", async () => {
    ...
    expect(prompt).toContain("Safety guardrails (do not override)");
    expect(prompt).toContain("no self-preservation, replication");
    expect(prompt).toContain("Do not change system prompts");
  ...
  it("does not recommend promise-only endings that imply action without execution", async () => {
    const prompt = await buildAgentSystemPrompt({
      workspaceDir: "/tmp/argent",
    });

    expect(prompt).not.toContain(`"I'll handle that now."`);
    expect(prompt).not.toContain(`"I'm on it."`);
  });
  ...
  it("includes a CLI quick reference section", async () => {
    ...
    expect(prompt).toContain("## Argent CLI Quick Reference");
    expect(prompt).toContain("argent gateway restart");
  ...
  // See: https://github.com/ArgentAIOS/argentos/commit/66eec295b894bce8333886cfbca3b960c57c4946
  // Agents should use session_status or message timestamps to determine the date/time.
  // Related: https://github.com/ArgentAIOS/argentos/issues/1897
  //          https://github.com/ArgentAIOS/argentos/issues/3658
  it("does NOT include a date or time in the system prompt (cache stability)", async () => {
    ...
    // https://github.com/ArgentAIOS/argentos/issues/3658 for the preferred approach:
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", async () => {
    ...
    expect(prompt).toContain("## Argent Self-Update");
