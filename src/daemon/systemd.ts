...
function resolveSystemdServiceName(env: Record<string, string | undefined>): string {
  const override = env.ARGENT_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
  }
  return resolveGatewaySystemdServiceName(env.ARGENT_PROFILE);
}
...
function resolveSystemctlMachineScopeUser(env: Record<string, string | undefined>): string | null {
...
async function execSystemctlUser(
  ...
  // Under sudo, prefer invoking non-root user scope directly.
...
export async function installSystemdService({
  ...
  const serviceDescription =
    description ??
    formatGatewayServiceDescription({
      profile: env.ARGENT_PROFILE,
      version: environment?.ARGENT_SERVICE_VERSION ?? env.ARGENT_SERVICE_VERSION,
    });
  ...
  const serviceName = resolveGatewaySystemdServiceName(env.ARGENT_PROFILE);
...
export async function stopSystemdService({
  ...
  await assertSystemdAvailable(resolvedEnv);
  const serviceName = resolveSystemdServiceName(resolvedEnv);
  const unitName = `${serviceName}.service`;
  const res = await execSystemctlUser(resolvedEnv, ["stop", unitName]);
  if (res.code !== 0) {
...
export async function restartSystemdService({
  ...
  const serviceName = resolveSystemdServiceName(resolvedEnv);
  const unitName = `${serviceName}.service`;
  const res = await execSystemctlUser(resolvedEnv, ["restart", unitName]);
  if (res.code !== 0) {
...
async function isSystemctlAvailable(env: Record<string, string | undefined>): Promise<boolean> {
  const res = await execSystemctlUser(env, ["status"]);
  if (res.code === 0) {
    return true;
  }
