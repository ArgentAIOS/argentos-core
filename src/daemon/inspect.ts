...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "argent" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["argent_service_marker"];
  const kindKeys = ["argent_service_kind"];
...
function isArgentGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.argent.");
}

function isArgentGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("argent-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isArgentGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("argent gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    results.push({
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "argent" || isLegacyLabel(label),
    });
  }
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isArgentGatewayTaskName(name)) {
        continue;
      }
