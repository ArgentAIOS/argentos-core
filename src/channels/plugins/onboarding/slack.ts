...
function setSlackDmPolicy(cfg: ArgentConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Argent";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for ArgentOS`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/argent",
          description: "Send a message to Argent",
          should_escape: false,
        },
      ],
    },
...
function setSlackAllowFrom(cfg: ArgentConfig, allowFrom: string[]): ArgentConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
