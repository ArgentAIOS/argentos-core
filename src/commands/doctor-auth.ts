...
function pruneAuthProfiles(
  cfg: ArgentConfig,
  profileIds: Set<string>,
): { next: ArgentConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("argent models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("argent models auth setup-token")} or ${formatCliCommand(
      "argent configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "argent models auth login --provider openai-codex",
    )} or ${formatCliCommand("argent configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("argent configure")}\` or \`${formatCliCommand("argent onboard")}\`.`;
}
