...
import type { ArgentConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: ArgentConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("argent security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("argent configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("argent config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("argent doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("argent config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
