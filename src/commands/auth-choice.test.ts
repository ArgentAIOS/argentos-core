...
describe("applyAuthChoice", () => {
  ...
  it("prompts and writes Mistral API key when selecting mistral-api-key", async () => {
    ...
    const text = vi.fn().mockResolvedValue("sk-mistral-test");
    const select: WizardPrompter["select"] = vi.fn(
      async (params) => params.options[0]?.value as never,
    );
    const multiselect: WizardPrompter["multiselect"] = vi.fn(async () => []);
    ...
    expect(text).toHaveBeenCalledWith(
      expect.objectContaining({ message: "Enter Mistral API key" }),
    );
    expect(result.config.auth?.profiles?.["mistral:default"]).toMatchObject({
      provider: "mistral",
      mode: "api_key",
    });
    ...
    expect(parsed.profiles?.["mistral:default"]?.key).toBe("sk-mistral-test");
    expect(result.config.agents?.defaults?.model?.primary).toBe("mistral/mistral-large-latest");
  });
