...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveArgentAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("argent models auth setup-token")}\` or \`${formatCliCommand("argent configure")}\`.`
          : `Run \`${formatCliCommand("argent configure")}\` or set an API key env var.`;
  ...
  if (oauthProfiles.length === 0) {
    ...
    for (const [provider, profiles] of profilesByProvider) {
      ...
      for (const profile of profiles) {
        ...
        const profileLabel = colorize(rich, theme.accent, labelText);
        ...
        const expiry =
          profile.status === "static"
            ...
            : profile.expiresAt
              ...
              : " expires unknown";
        runtime.log(`  - ${profileLabel} ${status}${expiry}`);
      }
    }
  }
