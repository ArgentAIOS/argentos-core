...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveArgentAgentDir: vi.fn().mockReturnValue("/tmp/argent-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/argent-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/argent-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveArgentAgentDir: mocks.resolveArgentAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveArgentAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/argent-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/argent-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/argent-agent-custom");
