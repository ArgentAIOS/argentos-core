...
import { type ArgentConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function updateConfig(
  mutator: (cfg: ArgentConfig) => ArgentConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: ArgentConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ArgentConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("argent agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
