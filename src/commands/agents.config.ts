...
export function listAgentEntries(cfg: ArgentConfig): AgentEntry[] {
...
function resolveAgentName(cfg: ArgentConfig, agentId: string) {
...
function resolveAgentModel(cfg: ArgentConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ArgentConfig): AgentSummary[] {
