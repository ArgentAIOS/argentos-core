...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Argent status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("argent security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("argent security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.argent.ai/faq");
  runtime.log("Troubleshooting: https://docs.argent.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("argent status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("argent logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("argent status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("argent gateway probe")}`);
  }
}
