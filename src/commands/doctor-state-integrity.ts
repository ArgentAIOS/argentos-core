...
function findOtherStateDirs(stateDir: string): string[] {
  const resolvedState = realPath(stateDir);
  const roots =
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".argentos", ".argent"].map((dir) => path.resolve(root, entry.name, dir));
      for (const candidate of candidates) {
        if (realPath(candidate) === resolvedState) {
          continue;
        }
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".argentos");
  ...
  if (realPath(stateDir) !== realPath(defaultStateDir)) {
