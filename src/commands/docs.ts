...
const SEARCH_TOOL = "https://docs.argent.ai/mcp.SearchArgent";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.argent.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('argent docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.argent.ai/");
      runtime.log(`Search: ${formatCliCommand('argent docs "your query"')}`);
    }
    return;
  }
