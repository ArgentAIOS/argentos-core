...
vi.mock("../memory/health.js", () => ({
  getMemoryHealthSummary: vi.fn(() => ({
    generatedAt: "2026-03-05T12:00:00.000Z",
    lanes: {
      memuExtraction: {
        status: "green",
        lastSuccessAt: "2026-03-05T11:00:00.000Z",
        staleHours: 1,
        failureCount24h: 0,
      },
      contemplation: {
      ...
      sisConsolidation: {
        status: "yellow",
        lastSuccessAt: "2026-03-04T22:00:00.000Z",
        staleHours: 14,
        failureCount24h: 1,
      },
      ragIngestion: {
        status: "green",
        lastSuccessAt: "2026-03-04T18:00:00.000Z",
        staleHours: 18,
        failureCount24h: 0,
      },
    },
    failures24h: { memuParse: 0, sisParse: 1 },
    activeModels: {
...
describe("getHealthSnapshot", () => {
  ...
  it("skips telegram probe when not configured", async () => {
    ...
    expect(snap.memoryHealth?.activeModels.embeddings.model).toBe("nomic-embed-text");
  });
  ...
  it("treats telegram.tokenFile as configured", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "argent-health-"));
