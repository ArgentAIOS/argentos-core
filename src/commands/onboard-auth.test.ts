...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.ARGENT_OAUTH_DIR;
  });

  it("writes auth-profiles.json under ARGENT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-oauth-"));
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ARGENT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-minimax-"));
    process.env.ARGENT_STATE_DIR = tempStateDir;
    process.env.ARGENT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ARGENT_AGENT_DIR;
