...
beforeEach(() => {
  ...
  resolveArgentPackageRoot.mockReset().mockResolvedValue(null);
  ...
  originalUpdateInProgress = process.env.ARGENT_UPDATE_IN_PROGRESS;
  process.env.ARGENT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "argent-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.ARGENT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveArgentPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../terminal/note.js", () => ({
  note,
  wrapNoteMessage: (message: string) => message,
}));
...
vi.mock("../plugins/loader.js", () => ({
  loadArgentPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "argent-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.ARGENT_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when ARGENT_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/argent.json",
      exists: true,
      raw: "{}",
    ...
    process.env.ARGENT_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
