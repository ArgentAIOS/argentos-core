...
const SENSITIVE_ASSISTANT_MARKER_RE = /\[(?:MOOD|TTS(?:_NOW)?)\s*:/i;
...
function shouldRedactPayloadLog(opts: AgentCommandOpts): boolean {
  const channel = opts.messageChannel ?? opts.runContext?.messageChannel ?? opts.channel;
  return Boolean(channel);
}

function formatRedactedPayloadLog(opts: AgentCommandOpts, output: string): string {
  const channel =
    opts.messageChannel ?? opts.runContext?.messageChannel ?? opts.channel ?? "unknown";
  const marker = SENSITIVE_ASSISTANT_MARKER_RE.test(output)
    ? "sensitive-markers"
    : "channel-context";
  return `[agent-output] redacted channel=${channel} reason=${marker} chars=${output.length}`;
}
...
export async function deliverAgentCommandResult(params: {
  ...
  const logPayload = (payload: NormalizedOutboundPayload) => {
    ...
    const safeOutput = shouldRedactPayloadLog(opts)
      ? formatRedactedPayloadLog(opts, output)
      : output;
    if (opts.lane === AGENT_LANE_NESTED) {
      logNestedOutput(runtime, opts, safeOutput);
      return;
    }
    runtime.log(safeOutput);
  };
  if (!deliver) {
