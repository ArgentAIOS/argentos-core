...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "argent-commands-"));
...
function buildParams(commandBody: string, cfg: ArgentConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
