...
import { scheduleGatewaySigusr1Restart, triggerArgentRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Argent in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerArgentRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Argent via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
