...
function withSessionCliMcpServers(
  config: ArgentConfig,
  provider: string,
  cliMcpServers?: Record<string, Record<string, unknown>>,
): ArgentConfig {
  if (!cliMcpServers || Object.keys(cliMcpServers).length === 0) {
    return config;
  }
  const defaults = config.agents?.defaults;
  const cliBackends = defaults?.cliBackends;
  const backend = cliBackends?.[provider] ?? resolveCliBackendConfig(provider, config)?.config;
  if (!backend) {
    return config;
  }
  return {
...
export async function runAgentTurnWithFallback(params: {
  ...
  if (params.sessionKey) {
    registerAgentRunContext(runId, {
      ...
      timings: { startedAt: runStartedAt },
    });
  }
  ...
  while (true) {
    try {
      ...
      const fallbackResult = await runWithModelFallback({
        ...
        run: (provider, model) => {
          const runConfig = withSessionCliMcpServers(
            params.followupRun.run.config,
            provider,
            params.opts?.cliMcpServers,
          );
          ...
          if (isCliProvider(provider, runConfig)) {
