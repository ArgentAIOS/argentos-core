...
import { saveSessionSnapshot } from "../../agents/session-snapshot.js";
...
export const handleCompactCommand: CommandHandler = async (params) => {
  ...
  if (result.ok && result.compacted) {
    ...
    // Secondary snapshot save point (backup in case compact.ts path missed it)
    if (result.result?.summary) {
      try {
        ...
        // Snapshot failure must never break /compact
      }
    }
  }
