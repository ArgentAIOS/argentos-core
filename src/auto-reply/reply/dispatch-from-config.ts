...
const resolveSessionTtsAuto = (ctx: FinalizedMsgContext, cfg: ArgentConfig): string | undefined => {
...
export async function dispatchReplyFromConfig(params: {
  ...
  // ── Live Inbox: capture user-side candidates (fire-and-forget) ──
  {
    ...
    if (captureContent.trim()) {
      try {
        const liveInboxCfg = cfg.agents?.defaults?.liveInbox;
        captureAndPromote({
        ...
        logVerbose(`dispatch-from-config: live-inbox user capture error: ${String(err)}`);
      }
    }
  }
  ...
  try {
    ...
    // ── Live Inbox: capture assistant-side candidates (fire-and-forget) ──
    {
      const liveInboxCfg = cfg.agents?.defaults?.liveInbox;
      for (const reply of replies) {
        if (reply.text) {
          try {
            ...
            logVerbose(`dispatch-from-config: live-inbox assistant capture error: ${String(err)}`);
          }
        }
      }
    }
