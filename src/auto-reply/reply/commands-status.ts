...
function resolveProviderCooldownLine(
  ...
  const state = resolveProviderCircuitState(store, providerKey);
  if (state === "half_open") {
    return `⚠ Provider circuit: ${providerKey} half-open (probe)`;
  }
  if (!isProviderInCooldown(store, providerKey)) {
    return undefined;
  }
  const until = resolveProviderUnusableUntilForDisplay(store, providerKey);
  if (typeof until === "number") {
    return `⛔ Provider cooldown: ${providerKey} until ${new Date(until).toLocaleTimeString()}`;
  }
  return `⛔ Provider cooldown: ${providerKey}`;
}
...
export async function buildStatusReply(params: {
  ...
  const statusText = buildStatusMessage({
    ...
    providerCooldownLine: resolveProviderCooldownLine(provider, statusAgentDir),
