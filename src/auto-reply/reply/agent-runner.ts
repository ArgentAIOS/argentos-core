...
import { saveEmergencySnapshot } from "../../agents/session-snapshot.js";
...
export async function runReplyAgent(params: {
  ...
  const resetSession = async ({
    ...
    // Emergency snapshot: preserve context from the dying session before reset.
    // Only for compaction failures (not role-ordering resets which cleanup transcripts).
    if (!cleanupTranscripts && prevEntry.sessionId) {
      const agentIdForSnapshot = resolveAgentIdFromSessionKey(sessionKey);
      const oldTranscript = resolveSessionFilePath(prevEntry.sessionId, prevEntry, {
        agentId: agentIdForSnapshot,
      });
      try {
        const saved = await saveEmergencySnapshot({
        ...
        if (saved) {
          defaultRuntime.error(
            `Emergency snapshot saved from ${prevEntry.sessionId} before session reset (${failureLabel})`,
          );
        }
      } catch {
        // Non-fatal — don't block the reset.
      }
    }
