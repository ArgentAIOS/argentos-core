...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(
    ...
    {
      env: {
        ARGENT_AGENT_DIR: (home) => path.join(home, ".argentos", "agent"),
        PI_CODING_AGENT_DIR: (home) => path.join(home, ".argentos", "agent"),
      },
      prefix: "argent-reply-",
    },
  );
...
describe("directive behavior", () => {
        ...
        {},
        {
          agents: {
            defaults: {
              ...
              workspace: path.join(home, "argent"),
            },
          },
  ...
  it("accepts /thinking xhigh for any model (router selects powerful tier)", async () => {
    await withTempHome(async (home) => {
      const storePath = path.join(home, "sessions.json");
  ...
  it("treats skill commands as reserved for model aliases", async () => {
    await withTempHome(async (home) => {
      vi.mocked(runEmbeddedPiAgent).mockReset();
      const workspace = path.join(home, "argent");
      await writeSkill({
