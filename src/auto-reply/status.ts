...
type AgentConfig = Partial<NonNullable<NonNullable<ArgentConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.argentos/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
...
const formatVoiceModeLine = (config?: ArgentConfig, sessionEntry?: SessionEntry): string | null => {
  if (!config) {
    return null;
  }
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🤖 Argent ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: ArgentConfig): string {
  const lines = ["ℹ️ Help", ""];
