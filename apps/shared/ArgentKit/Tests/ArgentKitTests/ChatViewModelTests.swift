...
private final class TestChatTransport: @unchecked Sendable, ArgentChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ArgentChatHistoryPayload]
    private let sessionsResponses: [ArgentChatSessionsListResponse]

    private let stream: AsyncStream<ArgentChatTransportEvent>
    private let continuation: AsyncStream<ArgentChatTransportEvent>.Continuation

    init(
        historyResponses: [ArgentChatHistoryPayload],
        sessionsResponses: [ArgentChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ArgentChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ArgentChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ArgentChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [ArgentChatAttachmentPayload]) async throws -> ArgentChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return ArgentChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ArgentChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ArgentChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ArgentChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ArgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ArgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ArgentChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
