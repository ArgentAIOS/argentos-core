...
import "@a2ui/lit/ui";
import { themeContext } from "@argent/a2ui-theme-context";
...
class ArgentA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.argentA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["argent:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.argentCanvasA2UIAction ??
      globalThis.argentCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.argentCanvasA2UIAction) {
...
if (!customElements.get("argent-a2ui-host")) {
  customElements.define("argent-a2ui-host", ArgentA2UIHost);
}
