...
@MainActor
struct ArgentChatComposer: View {
    @Bindable var viewModel: ArgentChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ArgentChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ArgentChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ArgentPendingAttachment.id)
                { (att: ArgentPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ArgentPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ArgentChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ArgentChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ArgentChatTheme.subtleCard)
        .clipShape(Capsule())
    }
