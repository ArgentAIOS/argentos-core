...
enum AssistantTextParser {
    static func segments(from raw: String) -> [AssistantTextSegment] {
        let normalized = self.normalizeVisibleAssistantText(raw)
        ...
        while let match = self.nextTag(in: normalized, from: cursor) {
            matchedTag = true
            if match.range.lowerBound > cursor {
                self.appendSegment(kind: currentKind, text: normalized[cursor..<match.range.lowerBound], to: &segments)
            }

            guard let tagEnd = normalized.range(of: ">", range: match.range.upperBound..<normalized.endIndex) else {
                cursor = normalized.endIndex
                break
            }

            let isSelfClosing = self.isSelfClosingTag(in: normalized, tagEnd: tagEnd)
        ...
        if cursor < normalized.endIndex {
            self.appendSegment(kind: currentKind, text: normalized[cursor..<normalized.endIndex], to: &segments)
        }
    ...
    private static func normalizeVisibleAssistantText(_ raw: String) -> String {
        var text = raw
        let markerPatterns = [
            #"\[MOOD:[^\]]+\]\s*"#,
            #"\[TTS_NOW:[^\]]*\]\s*"#,
        ...
        // Strip lightweight bracketed delivery cues that often sit at the front
        // of spoken-summary text, e.g. "[warm laugh]" or "[calm and steady]".
        text = text.replacingOccurrences(
            of: #"(?m)(?:^|\s)\[(?!/?(?:think|final)\b)[^\[\]\n]{1,80}\]\s*"#,
            with: " ",
            options: [.regularExpression])

        // Collapse excessive blank lines left by marker stripping.
        text = text.replacingOccurrences(
            of: #"\n{3,}"#,
            with: "\n\n",
            options: [.regularExpression])
