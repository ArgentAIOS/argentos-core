...
private let chatUILogger = Logger(subsystem: "ai.argent", category: "ArgentChatUI")

@MainActor
@Observable
public final class ArgentChatViewModel {
    public private(set) var messages: [ArgentChatMessage] = []
    ...
    public var attachments: [ArgentPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ArgentChatPendingToolCall] = []
    public private(set) var sessions: [ArgentChatSessionEntry] = []
    private let transport: any ArgentChatTransport
    ...
    private var pendingToolCallsById: [String: ArgentChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any ArgentChatTransport) {
    ...
    public var sessionChoices: [ArgentChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [ArgentChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [ArgentChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ArgentPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ArgentChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ArgentChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [ArgentChatMessage]) -> [ArgentChatMessage] {
        var result: [ArgentChatMessage] = []
    ...
    private static func dedupeKey(for message: ArgentChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ArgentChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> ArgentChatAttachmentPayload in
            ArgentChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ArgentChatSessionEntry {
    ...
    private func handleTransportEvent(_ evt: ArgentChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ArgentChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ArgentAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ArgentChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ArgentPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
