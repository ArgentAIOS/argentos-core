...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = ArgentCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = ArgentCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = ArgentCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = ArgentCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = ArgentCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__argent__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: ArgentLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for argent:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case ArgentLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case ArgentCanvasCommand.present.rawValue,
                 ArgentCanvasCommand.hide.rawValue,
                 ArgentCanvasCommand.navigate.rawValue,
                 ArgentCanvasCommand.evalJS.rawValue,
                 ArgentCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case ArgentCanvasA2UICommand.reset.rawValue,
                 ArgentCanvasA2UICommand.push.rawValue,
                 ArgentCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case ArgentCameraCommand.list.rawValue,
                 ArgentCameraCommand.snap.rawValue,
                 ArgentCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case ArgentScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: ArgentNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ArgentNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(ArgentLocationGetParams.self, from: req.paramsJSON)) ??
            ArgentLocationGetParams()
        ...
        let payload = ArgentLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ArgentCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(ArgentCanvasPresentParams.self, from: req.paramsJSON)) ??
                ArgentCanvasPresentParams()
        ...
        case ArgentCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ArgentCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(ArgentCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ArgentCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(ArgentCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ArgentCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(ArgentCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case ArgentCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing argentA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ArgentCanvasA2UICommand.push.rawValue, ArgentCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == ArgentCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(ArgentCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try ArgentCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(ArgentCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try ArgentCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.argentA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case ArgentCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ArgentCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(ArgentCameraSnapParams.self, from: req.paramsJSON)) ??
                ArgentCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ArgentCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(ArgentCameraClipParams.self, from: req.paramsJSON)) ??
                ArgentCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(ArgentScreenRecordParams.self, from: req.paramsJSON)) ??
            ArgentScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> ArgentLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return ArgentLocationMode(rawValue: raw) ?? .off
    }
