...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [ArgentCapability.canvas.rawValue, ArgentCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(ArgentCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(ArgentCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = ArgentLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(ArgentCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        ...
        if caps.contains(ArgentCapability.camera.rawValue) {
        ...
        if caps.contains(ArgentCapability.location.rawValue) {
