...
struct IOSGatewayChatTransport: ArgentChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ArgentChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ArgentChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ArgentChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ArgentChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ArgentChatAttachmentPayload]) async throws -> ArgentChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [ArgentChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(ArgentChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(ArgentGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ArgentGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ArgentChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ArgentAgentEventPayload.self)
                        {
