...
@Suite(.serialized) struct VoicePushToTalkHotkeyTests {
    ...
    @Test func beginEndFiresOncePerHold() async {
        ...
        await MainActor.run {
            // Left Option down.
            hotkey._testUpdateModifierState(keyCode: 58, modifierFlags: [.option])
            hotkey._testUpdateModifierState(keyCode: 58, modifierFlags: [.option])
            // Option release.
            hotkey._testUpdateModifierState(keyCode: 58, modifierFlags: [])
        }
    ...
    @Test func optionWithOtherModifiersDoesNotStartCapture() async {
        let counter = Counter()
        let hotkey = VoicePushToTalkHotkey(
            beginAction: { await counter.incBegin() },
            endAction: { await counter.incEnd() })
        ...
        await MainActor.run {
            hotkey._testUpdateModifierState(keyCode: 61, modifierFlags: [.option, .command])
            hotkey._testUpdateModifierState(keyCode: 61, modifierFlags: [.option, .shift])
            hotkey._testUpdateModifierState(keyCode: 61, modifierFlags: [.option, .control])
            hotkey._testUpdateModifierState(keyCode: 61, modifierFlags: [])
        }

        for _ in 0..<50 {
            let snap = await counter.snapshot()
            if snap.began == 0, snap.ended == 0 { break }
            try? await Task.sleep(nanoseconds: 10_000_000)
        }
