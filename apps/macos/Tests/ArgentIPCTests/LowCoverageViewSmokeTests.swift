...
struct LowCoverageViewSmokeTests {
    ...
    @Test func dockVisibilityWindowClassifierIncludesTitledDashboardWindow() {
        let dashboardWindow = NSWindow(
            contentRect: NSRect(x: 0, y: 0, width: 1200, height: 800),
            styleMask: [.titled, .closable, .resizable, .miniaturizable],
            backing: .buffered,
            defer: false)
        #expect(DockIconManager.isUserFacingWindowKind(dashboardWindow))
        ...
        let panel = NSPanel(
            ...
            backing: .buffered,
            defer: false)
        #expect(!DockIconManager.isUserFacingWindowKind(panel))
        ...
        let borderless = NSWindow(
            ...
            backing: .buffered,
            defer: false)
        #expect(!DockIconManager.isUserFacingWindowKind(borderless))
    }
