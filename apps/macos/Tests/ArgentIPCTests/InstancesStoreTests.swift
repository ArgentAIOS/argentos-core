...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: ArgentProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ArgentProtocol.AnyCodable] = [
            "presence": .init([ArgentProtocol.AnyCodable(entry)]),
        ]
        let payload = ArgentProtocol.AnyCodable(payloadMap)
