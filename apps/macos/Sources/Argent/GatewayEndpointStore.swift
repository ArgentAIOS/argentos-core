...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.argent", category: "gateway-endpoint")
    ...
    private let logger = Logger(subsystem: "ai.argent", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "argent.onboardingSeen")
            initialMode = seen ? .local : .unconfigured
        }

        let port = deps.localPort()
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            root: ArgentConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: ArgentConfigFile.loadDict())
    ...
    {
        if let envBind = env["ARGENT_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["ARGENT_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
...
extension GatewayEndpointStore {
    ...
    static func dashboardURL(for config: GatewayConnection.Config) throws -> URL {
        ...
        if self.isLoopbackHost(config.url.host) {
            ...
            guard let local = URLComponents(string: "http://localhost:8080/") else {
                throw NSError(domain: "Dashboard", code: 1, userInfo: [
                    NSLocalizedDescriptionKey: "Invalid local dashboard URL",
                ])
            }
            components = local
        } else {
            guard var remote = URLComponents(url: config.url, resolvingAgainstBaseURL: false) else {
                throw NSError(domain: "Dashboard", code: 1, userInfo: [
                    NSLocalizedDescriptionKey: "Invalid gateway URL",
                ])
            }
            switch remote.scheme?.lowercased() {
            case "ws":
                remote.scheme = "http"
            case "wss":
                remote.scheme = "https"
