...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: ArgentOAuthStore.AnthropicOAuthStatus = ArgentOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: ArgentOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = ArgentOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = ArgentOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try ArgentOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Argent can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: ArgentOAuthStore.AnthropicOAuthStatus,
