...
enum CommandResolver {
    private static let projectRootDefaultsKey = "argent.gatewayProjectRootPath"
    private static let helperName = "argent"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let argentEntry = root.appendingPathComponent("argent.mjs").path
        if FileManager().isReadableFile(atPath: argentEntry) { return argentEntry }
        let binEntry = root.appendingPathComponent("bin/argent.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/argent")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let argentPaths = self.argentManagedPaths(home: home)
        if !argentPaths.isEmpty {
            extras.insert(contentsOf: argentPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + argentPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func argentManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".argent"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("argent.mjs").path,
            root.appendingPathComponent("bin/argent.js").path,
        ]
    ...
    static func hasAnyArgentInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.argentExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let argentPath = self.projectArgentExecutable(projectRoot: root) {
                return [argentPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "argent", subcommand] + extraArgs
            }
            if let argentPath = self.argentExecutable(searchPaths: searchPaths) {
                return [argentPath, subcommand] + extraArgs
            }

            let missingEntry = """
            argent entrypoint missing (looked for dist/index.js or argent.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real argent CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/argent"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/argent.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/argent.mjs"
            node "$PRJ/argent.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/argent.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/argent.js"
            node "$PRJ/bin/argent.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent argent"
          pnpm --silent argent \(quotedArgs);
        else
          echo "argent CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ArgentConfigFile.loadDict()
