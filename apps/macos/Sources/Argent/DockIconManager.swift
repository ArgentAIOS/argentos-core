...
final class DockIconManager: NSObject, @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "ai.argent", category: "DockIconManager")
        ...
        Task { @MainActor in
            ...
            let visibleWindows = NSApp?.windows.filter { window in
                window.isVisible &&
                    ...
                    Self.isUserFacingWindowKind(window)
            } ?? []
    ...
    @MainActor
    static func isUserFacingWindowKind(_ window: NSWindow) -> Bool {
        if window.isKind(of: NSPanel.self) { return false }

        let className = "\(type(of: window))"
        if className == "NSPopupMenuWindow" || className == "NSStatusBarWindow" {
            return false
        }

        // Titled windows are the main user-facing surfaces (Dashboard, Settings, etc).
        // Do not require contentViewController, because WKWebView-backed windows set contentView directly.
