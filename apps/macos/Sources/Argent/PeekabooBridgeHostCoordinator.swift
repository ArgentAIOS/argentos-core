...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.argent", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: ArgentPeekabooBridgeServices?
    private static var argentSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Argent", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = ArgentPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.argentSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.argentSocketPath, privacy: .public)")
    }
...
@MainActor
private final class ArgentPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.argent.peekaboo")
