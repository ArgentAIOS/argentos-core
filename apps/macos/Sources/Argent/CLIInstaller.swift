...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("argent").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing argent CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed argent \($0)." } ?? "Installed argent."
