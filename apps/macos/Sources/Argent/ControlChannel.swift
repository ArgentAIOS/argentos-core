...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ArgentProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.argent", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ArgentKit.AnyCodable]()) {
                $0[$1.key] = ArgentKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ArgentProtocol.AnyCodable?) -> [String: ArgentProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ArgentProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ArgentKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ArgentProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("argent.control.heartbeat")
    static let controlAgentEvent = Notification.Name("argent.control.agent")
}
