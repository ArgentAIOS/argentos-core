...
final class VoiceSessionCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.argent", category: "voicewake.coordinator")
    ...
    func sendNow(token: UUID, reason: String = "explicit") {
        ...
        let finalText = text
        Task.detached {
            // Always route through TalkModeRuntime so the message goes to the main
            // session (dashboard chat when open, gateway chatSend when closed).
            // Both paths produce TTS responses. VoiceWakeForwarder bypasses this
            // and sends to a different channel, so we never use it here.
            Self.debugLog("sendNow: source=\(source) text=\(finalText.prefix(60))")
            Self.debugLog("→ routing through TalkModeRuntime (submitPushToTalkTranscript)")
            await TalkModeRuntime.shared.submitPushToTalkTranscript(finalText)
        }
    }
