...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ARGENT_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/argent")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("argent-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("argent-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/argent/ (rolling or stdout), or nil if none exist.
