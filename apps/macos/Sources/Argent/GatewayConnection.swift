...
private let gatewayConnectionLogger = Logger(subsystem: "ai.argent", category: "gateway.connection")
...
struct TtsConvertResponse: Decodable, Sendable {
    let audio: String?
    let audioPath: String?
    let provider: String?
    let outputFormat: String?
    let voiceCompatible: Bool?
}

private func unwrapGatewayAnyCodable(_ value: Any) -> Any {
    if let wrapped = value as? AnyCodable {
        return unwrapGatewayAnyCodable(wrapped.value)
    }
    if let dict = value as? [String: AnyCodable] {
        return dict.mapValues { unwrapGatewayAnyCodable($0) }
    }
    if let array = value as? [AnyCodable] {
        return array.map { unwrapGatewayAnyCodable($0) }
    }
    return value
}
...
actor GatewayConnection {
    ...
    enum Method: String, Sendable {
        ...
        case ttsStatus = "tts.status"
        case ttsConvert = "tts.convert"
    }
        ...
        timeoutMs: Double? = nil) async throws -> Data
    {
        ...
        do {
            ...
            if Self.isGatewayAuthTokenMismatch(error) {
                gatewayConnectionLogger
                    .info(
                        "gateway auth mismatch detected; refreshing endpoint config and reconnecting")
                do {
                    let refreshed = try await self.configProvider()
                    await self.configure(
                        url: refreshed.url,
                        token: refreshed.token,
                        password: refreshed.password)
                    if let refreshedClient = self.client {
                        try await refreshedClient.connect()
                        return try await refreshedClient.request(
                            method: method,
                            params: params,
                            timeoutMs: timeoutMs)
                    }
                    ...
                    gatewayConnectionLogger.error(
                        "gateway auth refresh/reconnect failed \(error.localizedDescription, privacy: .public)")
                }
            }
    ...
    func ttsConvert(
        text: String,
        params: [String: AnyCodable]? = nil,
        timeoutMs: Double? = nil) async throws -> TtsConvertResponse
    {
        var merged: [String: AnyCodable] = ["text": AnyCodable(text)]
        if let params {
            for (key, value) in params {
                merged[key] = AnyCodable(unwrapGatewayAnyCodable(value))
            }
        }
    ...
    private func sessionDefaultString(_ defaults: [String: ArgentProtocol.AnyCodable]?, key: String) -> String {
    ...
    private func handleDisconnect(reason: String) async {
        guard Self.isGatewayAuthFailureReason(reason) else { return }
        gatewayConnectionLogger.info(
            "gateway disconnect auth failure detected; refreshing token config reason=\(reason, privacy: .public)")
        do {
            let refreshed = try await self.configProvider()
            await self.configure(url: refreshed.url, token: refreshed.token, password: refreshed.password)
            if let client = self.client {
                try await client.connect()
                gatewayConnectionLogger.info("gateway reconnected after auth refresh")
            }
        } catch {
            gatewayConnectionLogger.error(
                "gateway auth refresh from disconnect failed \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    private static func isGatewayAuthTokenMismatch(_ error: Error) -> Bool {
        if let responseError = error as? GatewayResponseError {
            let combined = "\(responseError.message) \(responseError.code)".lowercased()
            if Self.isGatewayAuthFailureReason(combined) {
                return true
            }
        }
        let lowered = (error as NSError).localizedDescription.lowercased()
        return Self.isGatewayAuthFailureReason(lowered)
    }

    private static func isGatewayAuthFailureReason(_ raw: String) -> Bool {
        let lowered = raw.lowercased()
        return lowered.contains("token mismatch") ||
            lowered.contains("token missing") ||
            lowered.contains("gateway rejected token") ||
...
extension GatewayConnection {
    private struct ToolInvokeHttpRequest: Encodable {
        let tool: String
        let action: String
    ...
    private struct ToolInvokeHttpResponse: Decodable {
        let ok: Bool
        let result: AnyCodable?
        let error: ToolInvokeHttpError?
    }

    private struct ToolInvokeHttpError: Decodable {
        let type: String?
        let message: String?
    }
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(ArgentGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> ArgentSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return ArgentSessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        sessionKey: String,
        limit: Int? = nil,
        timeoutMs: Int? = nil) async throws -> ArgentChatHistoryPayload
    {
    ...
    func chatSend(
        ...
        attachments: [ArgentChatAttachmentPayload],
        timeoutMs: Int = 30000) async throws -> ArgentChatSendResponse
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        var params: [String: AnyCodable] = [
    ...
    // MARK: - Tool HTTP Invoke

    private static func toolsInvokeURL(for config: GatewayConnection.Config) throws -> URL {
        guard var components = URLComponents(url: config.url, resolvingAgainstBaseURL: false) else {
            throw NSError(
                domain: "Gateway",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Invalid gateway URL"])
        }
        ...
        guard let url = components.url else {
            throw NSError(
                domain: "Gateway",
                code: 2,
                userInfo: [NSLocalizedDescriptionKey: "Failed to build tools invoke URL"])
        }
        return url
    }
    ...
    private static func extractToolText(_ payload: AnyCodable?) -> String? {
        if let dict = payload?.dictionaryValue,
           let content = dict["content"]?.arrayValue
        {
            for item in content {
                guard let itemDict = item.dictionaryValue else { continue }
                if itemDict["type"]?.stringValue == "text",
                   let text = Self.trim(itemDict["text"]?.stringValue)
                {
                    return text
                }
            }
        }
        return Self.trim(payload?.stringValue)
    }

    private static func fallbackText(_ payload: AnyCodable?) -> String {
        guard let payload else { return "" }
        if let text = payload.stringValue {
            return text
        }
        if let data = try? JSONSerialization.data(withJSONObject: payload.foundationValue, options: [.prettyPrinted]),
           let json = String(data: data, encoding: .utf8)
        {
            return json
        }
        return "\(payload.foundationValue)"
    }
    ...
    func invokeToolHttp(
        ...
        timeoutMs: Int = 45_000) async throws -> String
    {
        let cfg = try await GatewayEndpointStore.shared.requireConfig()
        let url = try Self.toolsInvokeURL(for: cfg)
        let body = ToolInvokeHttpRequest(tool: tool, action: action, args: args, sessionKey: sessionKey)

        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.timeoutInterval = max(1.0, Double(timeoutMs) / 1000.0)
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        if let auth = Self.trim(cfg.token) ?? Self.trim(cfg.password) {
        ...
        guard let http = response as? HTTPURLResponse else {
            throw NSError(
                domain: "Gateway",
                code: 3,
                userInfo: [NSLocalizedDescriptionKey: "Tools invoke returned non-HTTP response"])
        }

        let decoded = try JSONDecoder().decode(ToolInvokeHttpResponse.self, from: data)
        if http.statusCode < 200 || http.statusCode >= 300 || !decoded.ok {
            let message = Self.trim(decoded.error?.message)
                ?? "Tool invoke failed (\(http.statusCode))"
            throw NSError(
                domain: "Gateway",
        ...
        if let text = Self.extractToolText(decoded.result) {
            return text
        }
        return Self.fallbackText(decoded.result)
    }

    // MARK: - Meeting Recorder

    func meetingRecorderStart(
        title: String? = nil,
        systemAudio: Bool = true,
        mic: Bool = true,
        liveTranscript: Bool = true,
        liveIntervalSec: Int? = nil,
        sessionKey: String? = nil) async throws -> String
    {
        var args: [String: AnyCodable] = [
            "systemAudio": AnyCodable(systemAudio),
            "mic": AnyCodable(mic),
            "liveTranscript": AnyCodable(liveTranscript),
        ]
        if let title = Self.trim(title) {
            args["title"] = AnyCodable(title)
        }
        if let liveIntervalSec, liveIntervalSec > 0 {
            args["liveIntervalSec"] = AnyCodable(liveIntervalSec)
        }
        return try await self.invokeToolHttp(
            tool: "meeting_record",
            action: "start",
            args: args,
            sessionKey: sessionKey,
    ...
    func meetingRecorderStop(sessionKey: String? = nil) async throws -> String {
        try await self.invokeToolHttp(
            tool: "meeting_record",
            action: "stop",
            sessionKey: sessionKey,
            timeoutMs: 90_000)
    ...
    func meetingRecorderStatus(sessionKey: String? = nil) async throws -> String {
        try await self.invokeToolHttp(
            tool: "meeting_record",
            action: "status",
            sessionKey: sessionKey,
    ...
    func meetingRecorderList(limit: Int = 10, sessionKey: String? = nil) async throws -> String {
        try await self.invokeToolHttp(
            tool: "meeting_record",
            action: "list",
        ...
        sessionKey: String? = nil) async throws -> String
    {
        try await self.invokeToolHttp(
            ...
            action: "process",
            args: [
                "meetingId": AnyCodable(meetingId),
                "createTasks": AnyCodable(createTasks),
            ],
            sessionKey: sessionKey,
            timeoutMs: 420_000)
    }
