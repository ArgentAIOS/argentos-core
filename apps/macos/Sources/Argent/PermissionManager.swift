...
enum PermissionManager {
    ...
    private static func ensureNotifications(interactive: Bool) async -> Bool {
        ...
        case .notDetermined:
            ...
            return granted &&
                (updated == .authorized || updated == .provisional)
        case .denied:
            if interactive {
    ...
    static func status(_ caps: [Capability] = Capability.allCases) async -> [Capability: Bool] {
        ...
        for cap in caps {
            ...
            case .notifications:
                let status = await self.notificationAuthorizationStatus()
                results[cap] = status == .authorized || status == .provisional
...
enum AppleScriptPermission {
    private static let logger = Logger(subsystem: "ai.argent", category: "AppleScriptPermission")
