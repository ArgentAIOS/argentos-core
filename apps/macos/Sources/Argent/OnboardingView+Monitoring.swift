...
extension OnboardingView {
    ...
    func updateMonitoring(for pageIndex: Int) {
        ...
        self.maybeCheckVoiceAvailability(for: pageIndex)
    }

    func maybeCheckVoiceAvailability(for pageIndex: Int) {
        guard pageIndex == self.onboardingChatPageIndex else { return }
        guard self.state.connectionMode == .local else { return }
        Task { await self.checkElevenLabsAvailability() }
    }
    ...
    func refreshLocalGatewayProbe() async {
        ...
        await MainActor.run {
            ...
            let expectedTokens = ["node", "argent", "tsx", "pnpm", "bun"]
    ...
    func refreshAnthropicOAuthStatus() {
        _ = ArgentOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        let previous = self.anthropicAuthDetectedStatus
        let status = ArgentOAuthStore.anthropicOAuthStatus()
    ...
    func verifyAnthropicOAuthIfNeeded(force: Bool = false) async {
        ...
        guard let refresh = ArgentOAuthStore.loadAnthropicOAuthRefreshToken(), !refresh.isEmpty else {
        ...
        do {
            ...
            try ArgentOAuthStore.saveAnthropicOAuth(updated)
            self.refreshAnthropicOAuthStatus()
