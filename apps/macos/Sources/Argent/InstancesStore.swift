...
final class InstancesStore {
    ...
    private let logger = Logger(subsystem: "ai.argent", category: "instances")
    ...
    func handlePresenceEventPayload(_ payload: ArgentProtocol.AnyCodable) {
        do {
    ...
    private func applyPresence(_ entries: [PresenceEntry]) {
        ...
        let deduped = self.deduplicateInstancesByID(withIDs)
        if deduped.count != withIDs.count {
            self.logger.warning(
                "presence payload had duplicate instance IDs: total=\(withIDs.count, privacy: .public) deduped=\(deduped.count, privacy: .public)")
        }

        self.notifyOnNodeLogin(deduped)
        self.lastPresenceById = Dictionary(deduped.map { ($0.id, $0) }, uniquingKeysWith: { _, latest in latest })
        self.instances = deduped
        self.statusMessage = nil
        self.lastError = nil
    }

    /// Presence payloads can occasionally include duplicate IDs; keep one entry per ID
    /// and prefer the newest timestamp for that ID.
    private func deduplicateInstancesByID(_ instances: [InstanceInfo]) -> [InstanceInfo] {
        var latestByID: [String: InstanceInfo] = [:]
        latestByID.reserveCapacity(instances.count)

        for instance in instances {
            if let existing = latestByID[instance.id], existing.ts > instance.ts {
                continue
            }
        ...
        var seen = Set<String>()
        var ordered: [InstanceInfo] = []
        ordered.reserveCapacity(latestByID.count)

        for instance in instances {
            guard seen.insert(instance.id).inserted else { continue }
            if let selected = latestByID[instance.id] {
