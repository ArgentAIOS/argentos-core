...
enum DebugActions {
    private static let verboseDefaultsKey = "argent.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "argent.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = ArgentPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Argent", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = ArgentPaths.configURL
        guard
