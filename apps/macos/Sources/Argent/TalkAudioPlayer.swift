...
final class TalkAudioPlayer: NSObject, @preconcurrency AVAudioPlayerDelegate {
    ...
    private let logger = Logger(subsystem: "ai.argent", category: "talk.tts")
    ...
    func play(fileURL: URL) async -> TalkPlaybackResult {
        self.stopInternal()
        ...
        return await withCheckedContinuation { continuation in
            ...
            do {
                ...
                let ok = player.play()
                if !ok {
                    self.logger.error("talk audio player refused to play file")
                    self.finish(playback: playback, result: TalkPlaybackResult(finished: false, interruptedAt: nil))
                }
            } catch {
                self.logger.error("talk audio player failed for file: \(error.localizedDescription, privacy: .public)")
                self.finish(playback: playback, result: TalkPlaybackResult(finished: false, interruptedAt: nil))
            }
        }
    }
