...
enum ArgentLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = ArgentOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ArgentFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ArgentLogging.bootstrapIfNeeded()
        let label = ArgentLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ArgentOSLogHandler: LogHandler {
...
struct ArgentFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ArgentLogging.parseLabel(self.label)
