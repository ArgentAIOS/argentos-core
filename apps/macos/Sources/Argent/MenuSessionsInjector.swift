...
extension MenuSessionsInjector {
    ...
    private func inject(into menu: NSMenu) {
        ...
        if let snapshot = self.cachedSnapshot {
            ...
            let compact = self.compactSessionRows(rows, mainKey: mainKey)
            let topRows = compact.visible
            let overflowRows = compact.hidden

            if !topRows.isEmpty {
                let previewKeys = topRows.prefix(20).map(\.key)
                let task = Task {
            ...
            let statusText: String? = {
                if let cached = self.cachedErrorText { return cached }
                if !isConnected { return self.controlChannelStatusText(for: channelState) }
                if !overflowRows.isEmpty { return "Showing active chat (\(overflowRows.count) more in submenu)" }
                return nil
            }()
            ...
            if !overflowRows.isEmpty {
                let more = NSMenuItem(
                    title: "Other Sessions (\(overflowRows.count))",
                    action: nil,
                    keyEquivalent: "")
                more.tag = self.tag
                more.image = NSImage(systemSymbolName: "list.bullet", accessibilityDescription: nil)
                more.submenu = self.buildSessionsOverflowMenu(
                    rows: overflowRows,
                    storePath: snapshot.storePath,
                    width: width)
                menu.insertItem(more, at: cursor)
                cursor += 1
            }

            let sessionsSettings = NSMenuItem(
                title: "Open Sessions Settings…",
                action: #selector(self.openSessionsSettings(_:)),
                keyEquivalent: "")
            sessionsSettings.tag = self.tag
            sessionsSettings.target = self
            menu.insertItem(sessionsSettings, at: cursor)
            cursor += 1
        } else {
            let headerItem = NSMenuItem()
            headerItem.tag = self.tag
        ...
        if let usageSubmenu = self.buildUsageAndCostSubmenu(width: width) {
            ...
            let usageItem = NSMenuItem(title: "Usage & Costs", action: nil, keyEquivalent: "")
            usageItem.tag = self.tag
            usageItem.isEnabled = true
            usageItem.image = NSImage(systemSymbolName: "chart.bar.xaxis", accessibilityDescription: nil)
            usageItem.submenu = usageSubmenu
            menu.insertItem(usageItem, at: cursor)
            cursor += 1
        }
    ...
    private func buildUsageAndCostSubmenu(width: CGFloat) -> NSMenu? {
        let menu = NSMenu()
        var cursor = 0
...
extension MenuSessionsInjector {
    ...
    private func compactSessionRows(_ rows: [SessionRow], mainKey: String) -> (visible: [SessionRow], hidden: [SessionRow]) {
        guard !rows.isEmpty else { return ([], []) }

        let primary: SessionRow? = rows.first(where: { $0.key == mainKey }) ?? rows.first
        guard let primary else { return ([], []) }

        let hidden = rows.filter { $0.key != primary.key }
        return ([primary], hidden)
    }

    private func buildSessionsOverflowMenu(rows: [SessionRow], storePath: String, width: CGFloat) -> NSMenu {
        let menu = NSMenu()
        for row in rows {
            let item = NSMenuItem()
            item.tag = self.tag
            item.isEnabled = true
            item.submenu = self.buildSubmenu(for: row, storePath: storePath)
            item.view = self.makeHostedView(
                rootView: AnyView(SessionMenuLabelView(row: row, width: width)),
                width: width,
                highlighted: true)
    ...
    @objc
    private func openSessionsSettings(_ sender: NSMenuItem) {
        SettingsTabRouter.request(.sessions)
        NSApp.activate(ignoringOtherApps: true)
        SettingsWindowOpener.shared.open()
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: .argentSelectSettingsTab, object: SettingsTab.sessions)
        }
    }
}
