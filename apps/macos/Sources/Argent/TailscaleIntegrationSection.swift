...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.argentos/argent.json. Prefer ARGENT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.argentos/argent.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.argentos/argent.json. Restart the gateway to apply."
        }
