...
/// Observes Option and starts a push-to-talk capture while it is held.
final class VoicePushToTalkHotkey: @unchecked Sendable {
    ...
    private func updateModifierState(keyCode: UInt16, modifierFlags: NSEvent.ModifierFlags) {
        ...
        let hasOtherModifiers = independentFlags.contains(.shift)
            || independentFlags.contains(.control)
            || independentFlags.contains(.command)
            || independentFlags.contains(.function)
        let chordActive = self.optionDown && !hasOtherModifiers
        if chordActive, !self.active {
            self.active = true
            Task {
                Logger(subsystem: "ai.argent", category: "voicewake.ptt")
...
actor VoicePushToTalk {
    ...
    private let logger = Logger(subsystem: "ai.argent", category: "voicewake.ptt")
