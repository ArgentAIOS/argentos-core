...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.argent", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Argent run already has an SSH listener on the expected port (common after restarts),
