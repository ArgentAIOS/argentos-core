...
actor VoiceWakeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.argent", category: "voicewake.runtime")
    ...
    func refresh(state: AppState) async {
        ...
        VoiceSessionCoordinator.debugLog("voicewake refresh: enabled=\(snapshot.0) supported=\(voiceWakeSupported) triggers=\(snapshot.1.triggers)")

        guard voiceWakeSupported, snapshot.0 else {
            VoiceSessionCoordinator.debugLog("voicewake refresh: STOPPED — supported=\(voiceWakeSupported) enabled=\(snapshot.0)")
            self.stop()
            return
        }
        ...
        guard PermissionManager.voiceWakePermissionsGranted() else {
            ...
            VoiceSessionCoordinator.debugLog("voicewake refresh: STOPPED — permissions missing")
            self.stop()
            return
        }
    ...
    private func start(with config: RuntimeConfig) async {
        if self.isStarting {
            VoiceSessionCoordinator.debugLog("voicewake start: already starting, skipping")
            return
        }
        ...
        do {
            ...
            guard let recognizer, recognizer.isAvailable else {
                ...
                VoiceSessionCoordinator.debugLog("voicewake start: FAILED — speech recognizer unavailable")
                return
            }
            VoiceSessionCoordinator.debugLog("voicewake start: gen=\(generation) recognizer available, setting up audio")
            ...
            let formatMismatch =
                ...
                (hardwareFormat.channelCount != outputFormat.channelCount ||
                    abs(hardwareFormat.sampleRate - outputFormat.sampleRate) > 0.5)
            let tapFormat: AVAudioFormat? = formatMismatch ? nil : outputFormat
            if formatMismatch {
                self.logger.warning(
                    "voicewake runtime input/output format mismatch; using default tap format " +
                        "(output=\(outputFormat.channelCount)ch@\(outputFormat.sampleRate, privacy: .public), " +
                        "hardware=\(hardwareFormat.channelCount)ch@\(hardwareFormat.sampleRate, privacy: .public))")
            }
            input.removeTap(onBus: 0)
            input.installTap(onBus: 0, bufferSize: 2048, format: tapFormat) { [weak self, weak request] buffer, _ in
            ...
            VoiceSessionCoordinator.debugLog("voicewake start: LISTENING gen=\(generation)")
            ...
            VoiceSessionCoordinator.debugLog("voicewake start: EXCEPTION — \(error.localizedDescription)")
            self.stop()
        }
    }
    ...
    private func handleRecognition(_ update: RecognitionUpdate, config: RuntimeConfig) async {
        ...
        // When the recognition task ends (isFinal or error that kills the task),
        // restart the recognizer so voice wake keeps listening. Apple's
        // SFSpeechRecognizer terminates continuous recognition after ~60 seconds.
        if update.isFinal || (update.error != nil && update.transcript == nil) {
            if !self.isCapturing {
                let errMsg = update.error?.localizedDescription ?? "none"
                self.logger.info("voicewake runtime recognition ended (isFinal=\(update.isFinal) error=\(update.error != nil)) — scheduling restart")
                VoiceSessionCoordinator.debugLog("voicewake recognition ENDED: isFinal=\(update.isFinal) error=\(errMsg) — scheduling restart")
                DiagnosticsFileLog.shared.log(category: "voicewake.runtime", event: "recognitionEnded", fields: [
                    "isFinal": "\(update.isFinal)",
                    "hasError": "\(update.error != nil)",
                ])
                self.scheduleRestartRecognizer(delay: 0.5)
                return
            }
        }
    ...
    private var lastFileLogTapAt: Date?

    private func noteAudioTap(rms: Double) {
        let now = Date()
        // Log to file every 15s so we can confirm audio is flowing
        if self.lastFileLogTapAt == nil || now.timeIntervalSince(self.lastFileLogTapAt!) >= 15.0 {
            self.lastFileLogTapAt = now
            let db = 20 * log10(max(rms, 1e-7))
            VoiceSessionCoordinator.debugLog("voicewake audioTap: rms=\(String(format: "%.6f", rms)) db=\(String(format: "%.1f", db)) capturing=\(self.isCapturing) hasRecTask=\(self.recognitionTask != nil)")
        }
    ...
    private func beginCapture(command: String, triggerEndTime: TimeInterval?, config: RuntimeConfig) async {
        ...
        VoiceSessionCoordinator.debugLog("voicewake beginCapture: command='\(command.prefix(60))' triggerEndTime=\(triggerEndTime ?? -1)")
    ...
    private func finalizeCapture(config: RuntimeConfig) async {
        ...
        if let token = self.overlayToken {
            ...
            Task.detached {
                // Always route through TalkModeRuntime for consistent session + TTS
                await TalkModeRuntime.shared.submitPushToTalkTranscript(finalTranscript)
            }
        }
        self.overlayToken = nil
        VoiceSessionCoordinator.debugLog("voicewake finalizeCapture: done, scheduling restart (0.7s)")
    ...
    private func restartRecognizer() {
        ...
        VoiceSessionCoordinator.debugLog("voicewake restartRecognizer: hasConfig=\(current != nil) isCapturing=\(self.isCapturing)")
        self.stop(dismissOverlay: false, cancelScheduledRestart: false)
        if let current {
            Task { await self.start(with: current) }
        } else {
            VoiceSessionCoordinator.debugLog("voicewake restartRecognizer: NO CONFIG — cannot restart!")
        }
    }
