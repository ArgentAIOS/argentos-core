...
enum UIStrings {
    static let welcomeTitle = "Welcome to Argent"
}
...
final class OnboardingController {
    ...
    func show() {
        if ProcessInfo.processInfo.isNixMode {
            ...
            UserDefaults.standard.set(true, forKey: "argent.onboardingSeen")
        ...
        window.minSize = NSSize(width: OnboardingView.windowMinWidth, height: OnboardingView.windowMinHeight)
        window.center()
        self.window = window
        self.present(window)
...
struct OnboardingView: View {
    ...
    @State var anthropicAuthDetectedStatus: ArgentOAuthStore.AnthropicOAuthStatus = .missingFile
    ...
    @State var onboardingChatModel: ArgentChatViewModel
    @State var onboardingSkillsModel = SkillsSettingsModel()
    @State var onboardingWizard = OnboardingWizardModel()
    @State var didLoadOnboardingSkills = false
    @State var localGatewayProbe: LocalGatewayProbe?
    @State var showVoiceChoice = false
    @State var selectedVoiceId: String = VoicePreset.presets[0].voiceId
    @State var previewingVoiceId: String?
    @State var voicePreviewPlayer: AVAudioPlayer?
    @Bindable var state: AppState
    var permissionMonitor: PermissionMonitor

    static let windowMinWidth: CGFloat = 630
    static let windowMinHeight: CGFloat = 752
    static let windowWidth: CGFloat = 700
    static let windowHeight: CGFloat = 860

    let pageWidth: CGFloat = Self.windowWidth
    let contentMinHeight: CGFloat = 520
    ...
    let voicePageIndex = 10
    static func pageOrder(
        for mode: AppState.ConnectionMode,
        showOnboardingChat: Bool,
        showVoiceChoice: Bool = false) -> [Int]
    {
        switch mode {
        case .remote:
        ...
        case .local:
            if showOnboardingChat && showVoiceChoice {
                [0, 1, 3, 5, 8, 10, 9]
            } else if showOnboardingChat {
                [0, 1, 3, 5, 8, 9]
            } else {
                [0, 1, 3, 5, 9]
            }
    ...
    var pageOrder: [Int] {
        Self.pageOrder(
            ...
            showVoiceChoice: self.showVoiceChoice)
    }
    ...
    var devLinkCommand: String {
        ...
        return "npm install -g argent@\(version)"
    }
            ...
            filterLocalGateways: false))
    {
        ...
        self._onboardingChatModel = State(
            initialValue: ArgentChatViewModel(
