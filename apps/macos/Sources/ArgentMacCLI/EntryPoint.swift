...
struct ArgentMacCLI {
    static func main() async {
        ...
        default:
            fputs("argent-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      argent-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      argent-mac discover [--timeout <ms>] [--json] [--include-local]
      argent-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      argent-mac connect
      argent-mac connect --url ws://127.0.0.1:18789 --json
      argent-mac discover --timeout 3000 --json
      argent-mac wizard --mode local
    """)
}
