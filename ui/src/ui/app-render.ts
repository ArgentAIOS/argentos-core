        ...
        <div class="topbar-left">
          ...
          <div class="brand">
            <div class="brand-logo">
              <img src="${logoHref}" alt="ArgentOS" />
            </div>
            <div class="brand-text">
              <div class="brand-title">ARGENTOS</div>
          ...
          state.tab === "overview"
            ? renderOverview({
                ...
                onSessionKeyChange: (next) => {
                  ...
                  state.chatMessage = "";
                  (state as unknown as ArgentApp).resetToolStream();
                  state.applySettings({
                    ...state.settings,
                    sessionKey: next,
          ...
          state.tab === "chat"
            ? renderChat({
                ...
                onSessionKeyChange: (next) => {
                  ...
                  (state as unknown as ArgentApp).chatStreamStartedAt = null;
                  state.chatQueue = [];
                  (state as unknown as ArgentApp).resetToolStream();
                  (state as unknown as ArgentApp).resetChatScroll();
                  state.applySettings({
                    ...state.settings,
                    sessionKey: next,
                    lastActiveSessionKey: next,
                ...
                onChatScroll: (event) => (state as unknown as ArgentApp).handleChatScroll(event),
                onDraftChange: (next) => (state.chatMessage = next),
                attachments: state.chatAttachments,
                onAttachmentsChange: (next) => (state.chatAttachments = next),
                onSend: () => (state as unknown as ArgentApp).handleSendChat(),
                canAbort: Boolean(state.chatRunId),
                onAbort: () => void (state as unknown as ArgentApp).handleAbortChat(),
                onQueueRemove: (id) => (state as unknown as ArgentApp).removeQueuedMessage(id),
                onNewSession: () =>
                  (state as unknown as ArgentApp).handleSendChat("/new", { restoreDraft: true }),
                showNewMessages: state.chatNewMessagesBelow,
                onScrollToBottom: () => state.scrollToBottom(),
                // Sidebar props for tool output viewing
                sidebarOpen: (state as unknown as ArgentApp).sidebarOpen,
                sidebarContent: (state as unknown as ArgentApp).sidebarContent,
                sidebarError: (state as unknown as ArgentApp).sidebarError,
                splitRatio: (state as unknown as ArgentApp).splitRatio,
                onOpenSidebar: (content: string) =>
                  (state as unknown as ArgentApp).handleOpenSidebar(content),
                onCloseSidebar: () => (state as unknown as ArgentApp).handleCloseSidebar(),
                onSplitRatioChange: (ratio: number) =>
                  (state as unknown as ArgentApp).handleSplitRatioChange(ratio),
        ...
        ${
          state.tab === "config"
            ? renderConfig({
                ...
                searchQuery: (state as unknown as ArgentApp).configSearchQuery,
                activeSection: (state as unknown as ArgentApp).configActiveSection,
                activeSubsection: (state as unknown as ArgentApp).configActiveSubsection,
                onRawChange: (next) => {
                  state.configRaw = next;
                },
                onFormModeChange: (mode) => (state.configFormMode = mode),
                onFormPatch: (path, value) =>
                  updateConfigFormValue(state as unknown as ArgentApp, path, value),
                onSearchChange: (query) =>
                  ((state as unknown as ArgentApp).configSearchQuery = query),
                onSectionChange: (section) => {
                  (state as unknown as ArgentApp).configActiveSection = section;
                  (state as unknown as ArgentApp).configActiveSubsection = null;
                },
                onSubsectionChange: (section) =>
                  ((state as unknown as ArgentApp).configActiveSubsection = section),
                onReload: () => loadConfig(state as unknown as ArgentApp),
                onSave: () => saveConfig(state as unknown as ArgentApp),
                onApply: () => applyConfig(state as unknown as ArgentApp),
                onUpdate: () => runUpdate(state as unknown as ArgentApp),
              })
            : nothing
        }
          ...
          state.tab === "logs"
            ? renderLogs({
                ...
                onExport: (lines, label) =>
                  (state as unknown as ArgentApp).exportLogs(lines, label),
                onScroll: (event) => (state as unknown as ArgentApp).handleLogsScroll(event),
              })
            : nothing
        }
      </main>
