import type { ArgentApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as ArgentApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as ArgentApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as ArgentApp);
  }
  if (host.tab === "agents") {
    const app = host as unknown as ArgentApp;
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as ArgentApp);
    await loadDevices(host as unknown as ArgentApp);
    await loadConfig(host as unknown as ArgentApp);
    await loadExecApprovals(host as unknown as ArgentApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as ArgentApp);
    await loadConfig(host as unknown as ArgentApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as ArgentApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as ArgentApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__ARGENT_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as ArgentApp, false),
    loadPresence(host as unknown as ArgentApp),
    loadSessions(host as unknown as ArgentApp),
    loadCronStatus(host as unknown as ArgentApp),
    loadDebug(host as unknown as ArgentApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as ArgentApp, true),
    loadConfigSchema(host as unknown as ArgentApp),
    loadConfig(host as unknown as ArgentApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as ArgentApp, false),
    loadCronStatus(host as unknown as ArgentApp),
    loadCronJobs(host as unknown as ArgentApp),
  ]);
}
