...
const originalConnect = ArgentApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("argent-app") as ArgentApp;
...
beforeEach(() => {
  ArgentApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__ARGENT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  ArgentApp.prototype.connect = originalConnect;
  window.__ARGENT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/argent/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/argent");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/argent/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ARGENT_CONTROL_UI_BASE_PATH__ = "/argent";
    const app = mountApp("/argent/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/argent");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/argent/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("argent.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
