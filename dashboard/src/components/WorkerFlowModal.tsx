...
function describeHubSpotScope(draft: FlowDraft): string {
  ...
  const portalId = draft.hubspot.portalId.trim();
  if (portalId) {
    parts.push(`portal ${portalId}`);
  }
  const pipelines = parseDelimitedList(draft.hubspot.pipelines);
  if (pipelines.length > 0) {
    parts.push(`pipelines ${pipelines.join(", ")}`);
  }
  const owners = parseDelimitedList(draft.hubspot.owners);
  if (owners.length > 0) {
    parts.push(`owners ${owners.join(", ")}`);
  }
  const teams = parseDelimitedList(draft.hubspot.teams);
  if (teams.length > 0) {
...
export function WorkerFlowModal({
  ...
  useEffect(() => {
    if (!isOpen || draft.mode !== "existing" || !draft.existingAgentId) {
      return;
    }
  ...
  const validateCurrentDraft = useCallback((): string | null => {
    if (!draft.identity.displayName.trim()) {
    ...
    if (!draft.purpose.workerTitle.trim()) {
      return "Worker title is required.";
    }
    if (!draft.purpose.mission.trim()) {
    ...
    if (!draft.rules.successDefinition.trim()) {
      return "Success definition is required.";
    }
