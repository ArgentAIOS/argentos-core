...
import {
  ...
  buildDayAxis,
  buildGanttEntries,
  groupGanttEntriesByLane,
  positionGanttEntry,
  resolveGanttDateFields,
  resolveGanttLaneField,
  shiftDays,
} from "./gantt-bucket";

/**
 * AppForge Gantt view (v1, read-only).
 *
 * Mirrors TimelineView's coordinate model — horizontal day axis at the
 * top, one row per swimlane below, each record drawn as an absolutely-
 * positioned bar by its (start, end) date pair. Reuses `gantt-bucket`
 * helpers which themselves re-export the Timeline date math
 * (`buildDayAxis`, `positionEntry`, `resolveDateFields`, etc.).
 *
 * v1 ships the bars-on-axis must-have. The hierarchy + dependency arrow
 * + critical-path features are wired into `gantt-bucket.ts` as pure
 * helpers but NOT yet rendered (see PR body for v1.1 follow-up — the
 * scope explicitly authorizes shipping bars-only if the overlay math
 * would balloon the diff). The bars-only Gantt is still operator-
 * useful: it shows a distinct "Gantt"-labeled view mode and renders
 * a slightly different bar palette (amber, planning-oriented) from
 * Timeline's sky tint, signalling the eventual project-management
 * sibling without blocking on the overlay work.
 *
 * Date-field selection precedence (per spec, identical to Timeline):
 ...
 * `groupFieldId` slot — same trick Calendar / Gallery / Timeline use).
 * When unset or pointing to a missing field, renders a single "All" lane.
 *
 * v1 is read-only: no drag-to-rebase, no inline edit. Clicking a bar
 * pops the record open via the parent's editor path (mirrors siblings).
 */
...
const RANGE_DAYS = GANTT_DEFAULT_RANGE_DAYS;
...
export type GanttViewProps = {
   ...
   * currently carries only one optional group field id, so for v1
   ...
   * missing field, the Gantt renders a single "All" lane.
   */
  preferredLaneFieldId?: string;
  /**
   * Lookup for the user-visible title of a record. Passed in so the
   * caller controls how name/title is resolved (matches Timeline /
   * Calendar / Gallery / Kanban).
   */
...
function fieldValueAsRaw(value: ForgeStructuredRecordValue | undefined): unknown {
  // Mirrors TimelineView: collapse single-element arrays to their value
...
export function GanttView({
  ...
  const { start: startField, end: endField } = useMemo(
    () => resolveGanttDateFields(fields, preferredStartFieldId, preferredEndFieldId),
    [fields, preferredStartFieldId, preferredEndFieldId],
  );

  const laneField = useMemo(
    () => resolveGanttLaneField(fields, preferredLaneFieldId),
  ...
  // ~25% in from the left of the axis — matches Asana / Linear / our
  // Timeline view's ergonomics.
  ...
  const entries = useMemo(() => {
    ...
    return buildGanttEntries(records, {
  ...
  const lanes = useMemo(() => groupGanttEntriesByLane(entries), [entries]);
    ...
    return (
      <div className="flex min-h-72 flex-col items-center justify-center gap-3 p-8 text-center">
        ...
        <div className="max-w-md text-xs leading-5 text-white/45">
          Gantt plots project records along a horizontal day axis. Add at least one date column in
          the field inspector — two columns (start + end) light up date-range bars; one column
          renders single-day milestones.
        </div>
      </div>
    );
  }
  ...
  const visibleEntryCount = entries.filter(
    (entry) => positionGanttEntry(entry, rangeStart, RANGE_DAYS).visible,
  ).length;
  ...
  return (
    <div className="space-y-3 p-4">
      ...
      <div className="overflow-x-auto rounded-xl border border-white/10 bg-black/20">
        <div
          style={{
            gridTemplateColumns: `minmax(160px, 220px) repeat(${dayAxis.length}, ${DAY_CELL_PX}px)`,
          }}
          className="grid"
        >
          ...
          <div className="sticky left-0 z-10 border-b border-r border-white/10 bg-black/45 px-3 py-2 text-[10px] uppercase tracking-[0.16em] text-white/40">
            {laneField ? laneField.name : "All projects"}
          </div>
          {dayAxis.map((cell) => (
            <div
              key={cell.key}
...
function LaneRow({
  ...
  const visible = entries
    .map((entry) => ({
      entry,
      position: positionGanttEntry(entry, rangeStart, dayAxisLength),
    }))
        ...
        {visible.map((row, rowIndex) => {
          ...
          return (
            <button
              ...
              title={title}
              // Amber palette distinguishes Gantt from Timeline's sky-blue
              // bars, signalling the project-management slant even in the
              // v1 bars-only renderer.
              className="absolute truncate rounded-md border border-amber-300/30 bg-amber-400/25 px-2 py-1 text-left text-[11px] text-amber-50 transition-colors hover:border-amber-200/60 hover:bg-amber-400/40"
