...
> - **D6 skip-reason log — DONE.** New `GateReason` type (`data/types.ts`) + bounded `appendGateReason` ring (`run-event-log.ts`, `GATE_REASON_LOG_MAX=25`, +2 tests). Runner records `{ts, gate, reason}` at both skip chokepoints (normal skip carries `result.reason` e.g. agent-busy/no-runnable-tasks/paused; cycle-error carries the message); surfaced per-agent via `getStatus().gateReasons` (last 10). Closes #445's silent no-op at the observability layer. tsc gate green.
> - **D9 proposed_action — DONE** (operator expanded budget for the shared-engine change). SIMULATE-mode write-capable tool calls are structurally diverted to recording stubs (`pi-embedded-runner/simulate-tool-stub.ts`, +4 tests) — they record `proposed_action {tool,args}` and never execute, so nothing external is touched. A `simulateWrites` flag threads `AgentCommandOpts → agent.ts → run.ts → attempt.ts` (wraps `effectiveTools` through both session paths, surfaces a `proposedActions[]` collector up through `EmbeddedPiRunMeta`); the runner appends each as a `proposed_action` run-event (D7 flush). Prevent-and-record replaces detect-and-block.
> - **Workforce Board read — DONE.** Thin additive render in `WorkforceBoard.tsx` (+159 lines): Runs tab shows the per-run lifecycle timeline (`JobRun.events`); Worker tab adds an `execution.worker.status` fetch and renders `gateReasons` ("why didn't this run") + in-flight `liveRuns` event chains. No new surfaces.
> - **P4 COMPLETE (5/5).** Commits: D7 `49a66567` · cross-check `20d9e414` · D6 `eed360be` · D9 `26e49c71` · board `9e0ac585`. tsc-since green throughout (189 baseline, zero net-new). Pre-existing env-dependent failures in the agent/overflow-compaction suites (4, identical on clean HEAD) are unrelated. Exit criterion met: run acceptance now cites the work_report × telemetry + run-events, not a `boardChanged`/`updatedAt` diff.
> - **DEPLOYED + LIVE-VERIFIED 2026-06-14 s2.** The live box runs the **installed** snapshot (`~/.argentos/lib/node_modules/argentos`), not the repo — so the commits above were inert until deployed via `scripts/out-of-sync-patch.sh` (`ARGENT_GIT_DIR=<repo> SKIP_FETCH=1 SKIP_UPDATE=1` → build my HEAD, atomic rsync to the package dir, gateway bounce). Post-deploy dist carries P4 (`report_telemetry_mismatch`/`proposed_action` in chunks) + the resend email default (5×, 0 sendgrid). **Live regression:** conductor run `9d42b3dd` (deployed code) persisted the D7 event log on the run record — `claimed → spawned → heartbeat → report(done)` — and the read-only role's `done` was accepted (no false `report_telemetry_mismatch`). gemma-4-12b-qat completed the conductor on the prior (old-code) run `c2fe65fd`; this run was gpt-5.5. **D9 not exercised live** — the conductor role grants no write-capable tool, so no `proposed_action` fires (unit-verified only; a write-granting role is needed to prove D9 live). _Same deploy made the resend podcast-email fix actually live (was repo-only before)._
