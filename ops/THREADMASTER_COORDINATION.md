...
### 2026-04-26 — AOS WooCommerce Connector Slice

Lane: `AOS connector Ralph loop`
Branch/status: `codex/aos-woocommerce-loop`, started from `origin/dev` at `4ad2a5a8`.
Owned files/directories: `tools/aos/aos-woocommerce/**`; coordination note in `ops/THREADMASTER_COORDINATION.md`.
Shared contract change in progress: WooCommerce connector capability metadata is being narrowed to live REST read commands only. Operator-controlled service keys remain the auth and linking source; local environment variables are development fallback only.
Workflows/AppForge/AOU reaction: consume the WooCommerce manifest/capabilities only. Do not assume WooCommerce order/product/customer/coupon write mutations exist unless a later handoff advertises them as live-ready with verified approval policy.
