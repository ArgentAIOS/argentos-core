...
### 2026-04-26 — AOS Box Connector Slice

Lane: `AOS Box connector`
Branch/status: `codex/aos-box-loop` in progress.
Owned files/directories: `tools/aos/aos-box/**` plus this coordination note.
Shared contract change: Box public AOS surface is being narrowed to truthful live read-only commands only: `file.list`, `file.get`, `file.download`, `folder.list`, `folder.get`, `collaboration.list`, `search.query`, `metadata.get`, and connector utility commands. Upload/copy/move/folder-create/shared-link/collaboration-create/metadata-write actions are not advertised until a write bridge and approval policy are verified.
Operator key contract: `BOX_ACCESS_TOKEN`, `BOX_CLIENT_ID`, `BOX_CLIENT_SECRET`, `BOX_JWT_CONFIG`, `BOX_FOLDER_ID`, `BOX_FILE_ID`, and `BOX_QUERY` resolve from operator service keys before local environment fallback.
Workflows/AppForge/AOU reaction: consume Box manifest/capabilities only; treat Box as live read-only and do not present it as an output/write destination.
