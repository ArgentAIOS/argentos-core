...
### 2026-04-26 — AOS Xero Connector Slice

Lane: `AOS connector Ralph loop`
Branch/status: `codex/aos-xero-loop`, started from `origin/dev` at `4ad2a5a8`.
Owned files/directories: `tools/aos/aos-xero/**`; coordination note in `ops/THREADMASTER_COORDINATION.md`.
Shared contract change in progress: Xero connector capability metadata is being narrowed to live Accounting API read commands only. Operator-controlled service keys remain the auth and linking source; local environment variables are development fallback only.
Workflows/AppForge/AOU reaction: consume the Xero manifest/capabilities only. Do not assume Xero invoice/contact/payment/bank-transaction/quote write mutations exist unless a later handoff advertises them as live-ready with verified approval policy.
