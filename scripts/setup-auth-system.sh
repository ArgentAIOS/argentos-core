#!/bin/bash
# Setup ArgentOS Auth Management System
...
echo "=== ArgentOS Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'argent-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  ArgentOS message: Send warning via ArgentOS itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/argent-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/argent-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/argent-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now argent-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status argent-auth-monitor.timer"
echo ""
echo "Quick commands:"
