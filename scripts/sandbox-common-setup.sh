...
BASE_IMAGE="${BASE_IMAGE:-argent-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-argent-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=argent.sandbox=1)
NOTE
