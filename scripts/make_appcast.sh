...
ZIP=${1:?"Usage: $0 ArgentOS-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/ArgentAIOS/argentos-core/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^ArgentOS-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/ArgentAIOS/argentos/releases/download/v${VERSION}/"}
