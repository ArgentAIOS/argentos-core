...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "ArgentOS")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "ArgentOS")
...
# Cleanup only exact DMG mount names from previous runs.
# Avoid broad prefix matches (e.g. "Argent*" would accidentally match "ArgentOS").
for pattern in "/Volumes/$DMG_VOLUME_NAME"* "/Volumes/$APP_NAME"*; do
  for vol in $pattern; do
    [[ -d "$vol" ]] || continue
    base="$(basename "$vol")"
    if [[ "$base" == "$DMG_VOLUME_NAME" || "$base" == "$DMG_VOLUME_NAME "* || "$base" == "$APP_NAME" || "$base" == "$APP_NAME "* ]]; then
...
DMG_TEMP="$(mktemp -d /tmp/argent-dmg.XXXXXX)"
...
# Add 10% + 100MB headroom for filesystem metadata (large bundles with many files need it)
HEADROOM_MB=$(( APP_SIZE_MB / 10 + 100 ))
DMG_SIZE_MB=$((APP_SIZE_MB + HEADROOM_MB))
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  # Volume icon: prefer ArgentOS icon, fall back to legacy ArgentOS icon.
  ICON_SRC="${DMG_VOLUME_ICON:-$ROOT_DIR/assets/argent-icon.icns}"
  if [[ ! -f "$ICON_SRC" ]]; then
    ICON_SRC="$ROOT_DIR/apps/macos/Sources/Argent/Resources/Argent.icns"
  fi
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/argent-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/argent-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/argent-dmg-limits.txt
...
hdiutil convert "$DMG_RW_PATH" -format UDZO -imagekey zlib-level=9 -o "$OUT_PATH" -ov
rm -f "$DMG_RW_PATH"
