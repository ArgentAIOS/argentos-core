...
// @ts-nocheck — standalone script, run with: bun scripts/migrate-mimo-to-memu-fixed.ts
...
async function main() {
  ...
  const mimo = new Database(MIMO_DB_PATH);
  ...
  function getOrCreateCategory(name: string): string {
    ...
    // FTS handled by trigger rebuild at the end
  ...
  // Disable triggers during bulk migration (avoid duplicate FTS + broken cats trigger)
  console.log("Disabling FTS triggers for bulk insert...");
  memu.exec("DROP TRIGGER IF EXISTS memu_items_ai");
  memu.exec("DROP TRIGGER IF EXISTS memu_cats_ai");
  ...
  // Rebuild FTS indexes from scratch (clean, no duplicates)
  console.log("Rebuilding FTS indexes...");
  memu.exec("DELETE FROM memory_items_fts");
  memu.exec(`INSERT INTO memory_items_fts(rowid, summary)
    SELECT rowid, summary FROM memory_items`);
  memu.exec("DELETE FROM memory_categories_fts");
  memu.exec(`INSERT INTO memory_categories_fts(rowid, name, summary)
    SELECT rowid, name, summary FROM memory_categories`);

  // Recreate triggers (fixed — categories trigger was inserting non-existent 'description' column)
  console.log("Recreating FTS triggers...");
  memu.exec(`CREATE TRIGGER memu_items_ai AFTER INSERT ON memory_items BEGIN
    INSERT INTO memory_items_fts(rowid, summary) VALUES (new.rowid, new.summary);
  END`);
  memu.exec(`CREATE TRIGGER memu_cats_ai AFTER INSERT ON memory_categories BEGIN
    INSERT INTO memory_categories_fts(rowid, name, summary)
      VALUES (new.rowid, new.name, new.summary);
  END`);

  // Fix broken cats update/delete triggers too
  memu.exec("DROP TRIGGER IF EXISTS memu_cats_ad");
  memu.exec("DROP TRIGGER IF EXISTS memu_cats_au");
  memu.exec(`CREATE TRIGGER memu_cats_ad AFTER DELETE ON memory_categories BEGIN
    INSERT INTO memory_categories_fts(memory_categories_fts, rowid, name, summary)
      VALUES('delete', old.rowid, old.name, old.summary);
  END`);
  memu.exec(`CREATE TRIGGER memu_cats_au AFTER UPDATE ON memory_categories BEGIN
    INSERT INTO memory_categories_fts(memory_categories_fts, rowid, name, summary)
      VALUES('delete', old.rowid, old.name, old.summary);
    INSERT INTO memory_categories_fts(rowid, name, summary)
      VALUES (new.rowid, new.name, new.summary);
