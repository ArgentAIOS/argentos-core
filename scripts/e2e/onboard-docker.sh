...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/argent-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/argent-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/argent-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/argent-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.argentos/workspace"
    config_path="$HOME/.ArgentAIOS/argentos.json"
    sessions_dir="$HOME/.argentos/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.ArgentAIOS/argentos.json" <<'"'"'JSON'"'"'
{
