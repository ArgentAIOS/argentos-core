...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/argent-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.argentos/extensions/demo-plugin"

  cat > "$HOME/.argentos/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.argentos/extensions/demo-plugin/argent.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/argent-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@argentos/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/argent.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/argent-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@argentos/demo-plugin-dir",
  "version": "0.0.1",
  "argent": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$dir_plugin/argent.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/argent-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@argentos/demo-plugin-file",
  "version": "0.0.1",
  "argent": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/argent.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
