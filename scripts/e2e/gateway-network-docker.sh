...
IMAGE_NAME="argent-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="argent-net-e2e-$$"
GW_NAME="argent-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "ARGENT_GATEWAY_TOKEN=$TOKEN" \
	  -e "ARGENT_SKIP_CHANNELS=1" \
	  -e "ARGENT_SKIP_GMAIL_WATCHER=1" \
	  -e "ARGENT_SKIP_CRON=1" \
	  -e "ARGENT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
