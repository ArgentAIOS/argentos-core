...
IMAGE_NAME="argent-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/argent-bin:$PATH"
  mkdir -p /tmp/argent-bin

  cat > /tmp/argent-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/argent-bin/systemctl

  cat > /tmp/argent-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/argent-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/argent"
  npm_entry="/tmp/npm-prefix/lib/node_modules/argent/dist/index.js"
  git_entry="/app/dist/index.js"
  git_cli="/app/argent.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/argent-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/argent-gateway.service"
    if [ ! -f "$unit_path" ]; then
