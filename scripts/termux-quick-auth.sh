...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/argent/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "ArgentOS Re-Auth" -c "After getting key, run: ssh $SERVER '~/argent/scripts/mobile-reauth.sh'" --id argent-auth
        ;;
    *)
