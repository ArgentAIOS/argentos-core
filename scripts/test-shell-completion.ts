 ...
 * `argent update`. Use it to verify the completion installation flow
...
\`argent update\`, \`argent doctor\`, and \`argent onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# ArgentOS Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`argent completion --install\` later to enable.`));
    return;
  }
