#!/usr/bin/env bash
# Reset ArgentOS like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${ARGENT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Argent.app/Contents/MacOS/Argent"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Argent"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Argent"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Argent"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.argentos.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/argent-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${ARGENT_RESTART_LOG:-/tmp/argent-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${ARGENT_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.argentos/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  ARGENT_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node argent.mjs daemon install --force --runtime node"
      log "  node argent.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.argentos/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.argentos.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing ArgentOS instances"
kill_all_argent
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product ArgentOS"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Argent.app" ]]; then
    APP_BUNDLE="/Applications/Argent.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Argent.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Argent.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Argent.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set ARGENT_APP_BUNDLE to your installed Argent.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node argent.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node argent.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".argentos", "argent.json"), "utf8");
