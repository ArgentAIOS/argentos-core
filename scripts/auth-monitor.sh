...
#
# Suggested cron: */30 * * * * /home/admin/argent/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send ArgentOS notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., argent-alerts)
...
STATE_FILE="$HOME/.argentos/auth-monitor-state"
...
send_notification() {
    ...
    # Send via ArgentOS if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use argent
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via ArgentOS to $NOTIFY_PHONE..."
            argent send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! ArgentOS is down. Run: ssh l36 '~/argent/scripts/mobile-reauth.sh'" "urgent"
    exit 1
