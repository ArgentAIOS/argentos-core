...
APP_BUNDLE="${1:-dist/Argent.app}"
IDENTITY="${SIGN_IDENTITY:-}"
TIMESTAMP_MODE="${CODESIGN_TIMESTAMP:-auto}"
DISABLE_LIBRARY_VALIDATION="${DISABLE_LIBRARY_VALIDATION:-0}"
SKIP_TEAM_ID_CHECK="${SKIP_TEAM_ID_CHECK:-0}"
ENT_TMP_BASE=$(mktemp -t argent-entitlements-base.XXXXXX)
ENT_TMP_APP_BASE=$(mktemp -t argent-entitlements-app-base.XXXXXX)
ENT_TMP_RUNTIME=$(mktemp -t argent-entitlements-runtime.XXXXXX)
...
if [ -f "$APP_BUNDLE/Contents/MacOS/argent-audio-capture" ]; then
  echo "Signing auxiliary binary: argent-audio-capture"; sign_item "$APP_BUNDLE/Contents/MacOS/argent-audio-capture" "$APP_ENTITLEMENTS"
fi

# Sign main binary (detect name from Info.plist)
MAIN_EXEC=$(/usr/libexec/PlistBuddy -c "Print CFBundleExecutable" "$APP_BUNDLE/Contents/Info.plist" 2>/dev/null || basename "$APP_BUNDLE" .app)
if [ -f "$APP_BUNDLE/Contents/MacOS/$MAIN_EXEC" ]; then
  echo "Signing main binary: $MAIN_EXEC"; sign_item "$APP_BUNDLE/Contents/MacOS/$MAIN_EXEC" "$APP_ENTITLEMENTS"
fi
...
# Deep-sign all native binaries in Resources (node_modules, bin, etc.)
# Apple notarization requires every Mach-O binary to be signed with Developer ID,
# hardened runtime, and secure timestamp — including third-party .node/.dylib files.
echo "Deep-signing native binaries in Resources..."
NATIVE_COUNT=0
while IFS= read -r -d '' f; do
  if /usr/bin/file "$f" | /usr/bin/grep -q "Mach-O"; then
...
done < <(find "$APP_BUNDLE/Contents/Resources" -type f \( -name "*.node" -o -name "*.dylib" -o -name "spawn-helper" -o -name "argent-audio-capture" -o -name "node" \) -print0 2>/dev/null)
echo "Signed ${NATIVE_COUNT} native binaries in Resources."
