...
export function getA2uiPaths(env = process.env) {
  const srcDir = env.ARGENT_A2UI_SRC_DIR ?? path.join(repoRoot, "src", "canvas-host", "a2ui");
  const outDir = env.ARGENT_A2UI_OUT_DIR ?? path.join(repoRoot, "dist", "canvas-host", "a2ui");
  return { srcDir, outDir };
}

async function canBuildA2uiInThisEnvironment() {
  const rendererDir = path.join(repoRoot, "vendor", "a2ui", "renderers", "lit");
  const appDir = path.join(repoRoot, "apps", "shared", "ArgentKit", "Tools", "CanvasA2UI");
  try {
    await fs.stat(rendererDir);
...
export async function copyA2uiAssets({ srcDir, outDir }: { srcDir: string; outDir: string }) {
  const skipMissing = process.env.ARGENT_A2UI_SKIP_MISSING === "1";
  const canBuild = await canBuildA2uiInThisEnvironment();
  try {
