...
INSTALL_URL="${ARGENT_INSTALL_URL:-https://argentos.ai/install.sh}"
SMOKE_PREVIOUS_VERSION="${ARGENT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ARGENT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="argent"
PACKAGE_NAME="${ARGENT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ARGENT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ARGENT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ARGENT_INSTALL_LATEST_OUT:-}"
fi
