...
const forwardedArgsRaw = process.argv.slice(2).filter(Boolean);
const forwardedArgs =
  forwardedArgsRaw.length > 0 && forwardedArgsRaw[0] === "--"
    ? forwardedArgsRaw.slice(1)
    : forwardedArgsRaw;

const withSuppressedNodeWarnings = (env = process.env) => {
...
if (forwardedArgs.length > 0) {
  const directArgs =
    forwardedArgs[0] === "run" || forwardedArgs[0] === "watch"
      ? ["vitest", ...forwardedArgs]
      : ["vitest", "run", ...forwardedArgs];
  const child = spawn(pnpm, directArgs, {
    stdio: "inherit",
    env: withSuppressedNodeWarnings(),
    shell: process.platform === "win32",
  });
  children.add(child);
  const code = await new Promise((resolve) => {
    child.on("exit", (nextCode, signal) => {
      children.delete(child);
      resolve(nextCode ?? (signal ? 1 : 0));
    });
  });
  process.exit(code);
}
...
const runOnce = (entry, extraArgs = []) =>
  new Promise((resolve) => {
    ...
    const child = spawn(pnpm, args, {
      stdio: "inherit",
      env: { ...withSuppressedNodeWarnings(), VITEST_GROUP: entry.name },
      shell: process.platform === "win32",
    });
    children.add(child);
