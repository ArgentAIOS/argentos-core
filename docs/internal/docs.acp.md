...
This document describes how the ArgentOS ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`argent acp` exposes an ACP agent over stdio and forwards prompts to a running
ArgentOS Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive an ArgentOS Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `argent acp` over stdio.

Example config:

```bash
argent config set gateway.remote.url wss://gateway-host:18789
argent config set gateway.remote.token <token>
```

Example run:

```bash
argent acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
argent acp --session agent:main:main
argent acp --session agent:design:main
argent acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select "ArgentOS ACP" to start a thread.

## Execution Model

- ACP client spawns `argent acp` and speaks ACP messages over stdio.
...
```bash
argent acp --session agent:main:main
argent acp --session-label "support inbox"
argent acp --reset-session
```
...
`argent acp` resolves the Gateway URL and auth from CLI flags or config:
