...
You can also set `ARGENTOS_VERBOSE=1` instead of the flag.
...
### `docs.argentos.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.argentos.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.argentos.ai` to the allowlist, then retry.
...
- Check the allowlist: `argent config get agents.defaults.models`
...
If you can, include the relevant log tail from `argent logs --follow`.
