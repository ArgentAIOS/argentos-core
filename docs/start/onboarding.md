---
summary: "First-run onboarding flow for ArgentOS (macOS app)"
read_when:
...
- Writes credentials to `~/.argentos/credentials/oauth.json`
...
The app can install the global `argent` CLI via npm/pnpm so terminal
...
On the first agent run, ArgentOS bootstraps a workspace (default `~/.argentos/workspace`):
...
```bash
argent webhooks gmail setup --account you@gmail.com
```
...
- `~/.argentos/credentials/oauth.json`
