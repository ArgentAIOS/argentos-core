...
- Publish as `argent/plugin-sdk` (or export from core under `argent/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ArgentOSPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ArgentOSConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ArgentOSConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ArgentOSConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ArgentOSConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ArgentOSConfig): string;
  };
};
```
...
- Introduce `argent/plugin-sdk`.
- Add `api.runtime` to `ArgentOSPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `argentRuntime: ">=2026.2.0"`).
