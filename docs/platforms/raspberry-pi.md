---
summary: "ArgentOS on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up ArgentOS on a Raspberry Pi
  - Running ArgentOS on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# ArgentOS on Raspberry Pi

## Goal

Run a persistent, always-on ArgentOS Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most ArgentOS features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled argent

# Enable if not
sudo systemctl enable argent

# Start on boot
sudo systemctl start argent
```
...
```bash
# Check logs
journalctl -u argent --no-pager -n 100

# Common fix: rebuild
cd ~/argentos  # if using hackable install
npm run build
sudo systemctl restart argent
```
