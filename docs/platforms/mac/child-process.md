...
service via the external `argent` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`ARGENTOS_PROFILE`; legacy `com.argentos.*` is supported).
