---
summary: "Quick troubleshooting guide for common ArgentOS failures"
read_when:
...
When ArgentOS misbehaves, here's how to fix it.
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `argent status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `argent status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `argent status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `argent gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `argent channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `argent gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `argent logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `argent status --all` (it redacts tokens). If you paste `argent status`, consider setting `ARGENTOS_SHOW_SECRETS=0` first (token previews).
...
```bash
argent models auth paste-token --provider anthropic
argent models status
```
...
- Preferred: `argent logs --follow`
- File logs (always): `/tmp/argentos/argent-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$ARGENTOS_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u argent-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  argent config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  argent config set gateway.mode remote
  argent config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `argent setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.argentos/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `argent doctor`
...
- then run `argent sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `argent gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `argent gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `argent gateway status` and `argent doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `ARGENTOS_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `argent gateway status` shows a config mismatch**
...
- Fix: rerun `argent gateway install --force` from the same `--profile` / `ARGENTOS_STATE_DIR` you want the service to use.

**If `argent gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `argent doctor` to update it (or `argent gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `ARGENTOS_GATEWAY_TOKEN`) and restart the service.

**If `argent gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/argentos` on disk.
...
Symptoms: `pwd` or file tools show `~/.argentos/sandboxes/...` even though you
...
ArgentOS intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `argent models list` or
  `argent models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${ARGENTOS_CONFIG_PATH:-$HOME/.argentos/argent.json}"
```
...
tail -f "$(ls -t /tmp/argentos/argent-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
argent logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.argentos/agents/<agentId>/sessions/
```
...
argent logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
argent gateway --verbose
```

If you’re logged out / unlinked:

```bash
argent channels logout
trash "${ARGENTOS_STATE_DIR:-$HOME/.argentos}/credentials" # if logout can't cleanly remove everything
argent channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/argentos/argent-*.log | head -1)" | tail -20
```

### High Memory Usage

ArgentOS keeps conversation history in memory.
...
ArgentOS now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `argent doctor` reports every invalid entry.
- `argent doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `argent logs`, `argent health`, `argent status`, `argent gateway status`, and `argent gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/argentos/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `argent models status` (CLI).
...
5. Run `argent channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. ArgentOS scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/ArgentAIOS/argentos/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
argent gateway status
argent gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.argentos.* still works)
```
...
Ensure the global `argent` CLI is installed and matches the app version:

```bash
argent --version
npm install -g argentos@<version>
```
...
#   ${ARGENTOS_CONFIG_PATH:-$HOME/.argentos/argent.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
argent gateway --verbose
argent channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                  |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/argentos/argent-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$ARGENTOS_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.argentos/logs/...`; profiles use `~/.argentos-<profile>/logs/...`)<br />Linux: `journalctl --user -u argent-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "ArgentOS Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$ARGENTOS_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                          |
| Media cache                       | `$ARGENTOS_STATE_DIR/media/`                                                                                                                                                                                                                                                                                              |
| Credentials                       | `$ARGENTOS_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                        |
...
argent gateway status --deep

# Is the gateway reachable?
argent health --json
# If it fails, rerun with connection details:
argent health --verbose
...
tail -20 /tmp/argentos/argent-*.log
```
...
trash "${ARGENTOS_STATE_DIR:-$HOME/.argentos}"
argent channels login         # re-pair WhatsApp
argent gateway restart           # or: argent gateway
```
...
1. Check logs first: `/tmp/argentos/` (default: `argent-YYYY-MM-DD.log`, or your configured `logging.file`)
