...
`argent doctor` is the repair + migration tool for ArgentOS. It fixes stale
...
```bash
argent doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
argent doctor --repair --force
```
...
- Extra workspace dir detection (`~/argentos`).
...
you to run `argent doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.argentos/argent.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.argentos/sessions/` to `~/.argentos/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.argentos/agent/` to `~/.argentos/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.argentos/credentials/*.json` (except `oauth.json`)
  - to `~/.argentos/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `argent doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.argentos` folders exist across
  home directories or when `ARGENTOS_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.argentos/argent.json` is
...
offers to remove them and install the ArgentOS service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named ArgentOS gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `argent doctor --generate-gateway-token` to force token
...
- `argent doctor` prompts before rewriting supervisor config.
- `argent doctor --yes` accepts the default repair prompts.
- `argent doctor --repair` applies recommended fixes without prompts.
- `argent doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `argent gateway install --force`.
