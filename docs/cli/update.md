---
summary: "CLI reference for `argent update` (safe-ish source update + gateway auto-restart)"
read_when:
...
`argent --update` rewrites to `argent update` (useful for shells and launcher scripts).

## See also

- `argent doctor` (offers to run update first on git checkouts)
