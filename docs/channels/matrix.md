...
Matrix is an open, decentralized messaging protocol. ArgentOS connects as a Matrix **user**
...
```bash
argent plugins install @argentos/matrix
```

Local checkout (when running from a git repo):

```bash
argent plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `argent plugins install @argentos/matrix`
   - From a local checkout: `argent plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: ArgentOS calls the same
     login endpoint, stores the access token in `~/.argentos/credentials/matrix/credentials.json`,
...
- On first connection, ArgentOS requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  ArgentOS logs a warning.
...
`~/.argentos/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `argent pairing list matrix`
  - `argent pairing approve matrix <CODE>`
...
- On startup, ArgentOS resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
