...
```bash
argent plugins install @argentos/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
argent plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "ArgentOS" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `argent pairing list nextcloud-talk`
  - `argent pairing approve nextcloud-talk <CODE>`
