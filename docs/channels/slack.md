...
3. Set tokens for ArgentOS and start the gateway.
...
7. Slash Commands → create `/argent` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
ArgentOS can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, ArgentOS falls back to the user token.
...
```json
{
  "display_information": {
    "name": "ArgentOS",
    "description": "Slack connector for ArgentOS"
  },
  "features": {
    "bot_user": {
      "display_name": "ArgentOS",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/argent",
        "description": "Send a message to ArgentOS",
        "should_escape": false
      }
    ]
  },
...
By default, ArgentOS replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, ArgentOS infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `argent pairing approve slack <code>`.
...
- On startup, ArgentOS resolves channel/user names in allowlists to IDs (when tokens allow)
