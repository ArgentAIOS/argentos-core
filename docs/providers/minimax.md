---
summary: "Use MiniMax M2.1 in ArgentOS"
read_when:
  - You want MiniMax models in ArgentOS
...
```bash
argent plugins enable minimax-portal-auth  # skip if already loaded.
argent gateway restart  # restart if gateway is already running
argent onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/ArgentAIOS/argentos/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `argent.json`:
...
- Use `argent models list` and `argent models set minimax/MiniMax-M2.1` to switch.
...
- Running `argent configure` and selecting **MiniMax M2.1**, or
