---
summary: "Use Venice AI privacy-focused models in ArgentOS"
read_when:
  - You want privacy-focused inference in ArgentOS
...
```bash
argent onboard --auth-choice venice-api-key
```
...
After setup, ArgentOS shows all available Venice models. Pick based on your needs:
...
```bash
argent models list | grep venice
```

## Configure via `argent configure`

1. Run `argent configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
ArgentOS automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
The Venice model catalog updates dynamically. Run `argent models list` to see currently available models. Some models may be temporarily offline.
