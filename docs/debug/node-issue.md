...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ArgentOS
...
Running ArgentOS via Node with `tsx` fails at startup with:

```
[argent] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `argent status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch argent.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node argent.mjs status` works on Node 25.
