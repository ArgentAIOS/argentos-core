---
summary: "ArgentOS on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up ArgentOS on DigitalOcean
  - Looking for cheap VPS hosting for ArgentOS
title: "DigitalOcean"
---

# ArgentOS on DigitalOcean

## Goal

Run a persistent ArgentOS Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status argent-gateway.service

# View logs
journalctl --user -u argent-gateway.service -f
```
...
```bash
argent config set gateway.bind tailnet
argent gateway restart
```
...
```bash
argent channels login whatsapp
# Scan QR code
```
...
- `~/.argentos/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u argent --no-pager -n 50
```
