...
This guide summarizes a sane workflow for working on the pi integration in ArgentOS.
...
- Trigger the agent directly:
  - `pnpm argent agent --message "Hello" --thinking low`
...
State lives under the ArgentOS state directory. Default is `~/.argentos`. If `ARGENTOS_STATE_DIR` is set, use that directory instead.

To reset everything:

- `argent.json` for config
...
- https://docs.argentos.ai/testing
- https://docs.argentos.ai/start/getting-started
