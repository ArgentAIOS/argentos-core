...
- During heartbeat runs, ArgentOS treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `argent sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, ArgentOS skips the heartbeat run entirely (no model call).
...
```bash
argent system event --text "Check for urgent follow-ups" --mode now
```
