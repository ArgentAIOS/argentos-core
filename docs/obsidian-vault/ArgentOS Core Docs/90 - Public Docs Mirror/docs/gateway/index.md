...
- Replaces the legacy `gateway` command. CLI entry point: `argent gateway`.
...
argent gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.argentos/argent.json` (or `ARGENTOS_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__argent__/canvas/` from `~/.argentos/workspace/canvas`. Disable with `canvasHost.enabled=false` or `ARGENTOS_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `ARGENTOS_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `ARGENTOS_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.argentos.*` may still exist)
- Linux: `argent-gateway-<profile>.service`
- Windows: `ArgentOS Gateway (<profile>)`

Install metadata is embedded in the service config:

- `ARGENTOS_SERVICE_MARKER=argent`
- `ARGENTOS_SERVICE_KIND=gateway`
- `ARGENTOS_SERVICE_VERSION=<version>`
...
```bash
argent --dev setup
argent --dev gateway --allow-unconfigured
...
- `ARGENTOS_STATE_DIR=~/.argent-dev`
- `ARGENTOS_CONFIG_PATH=~/.argent-dev/argent.json`
- `ARGENTOS_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.argentos/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `ARGENTOS_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `ARGENTOS_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `ARGENTOS_CONFIG_PATH`
- unique `ARGENTOS_STATE_DIR`
...
```bash
argent --profile main gateway install
argent --profile rescue gateway install
```

Example:

```bash
ARGENTOS_CONFIG_PATH=~/.argentos/a.json ARGENTOS_STATE_DIR=~/.argentos-a argent gateway --port 19001
ARGENTOS_CONFIG_PATH=~/.argentos/b.json ARGENTOS_STATE_DIR=~/.argentos-b argent gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `argent health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `argent`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `argent gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.argentos.*` is cleaned up).
  - `argent doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are ArgentOS profile services.
  ...
  - Cleanup: `argent gateway uninstall` (current service) and `argent doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `argent gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- ArgentOS.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.argentos.*` labels still unload cleanly).
- To stop it cleanly, use `argent gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `argent gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `argent gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

ArgentOS installs a **systemd user service** by default on Linux/WSL2. We
...
`argent gateway install` writes the user unit. `argent doctor` audits the
...
Restart=always
RestartSec=5
Environment=ARGENTOS_GATEWAY_TOKEN=
...
Create `/etc/systemd/system/argent-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now argent-gateway[-<profile>].service
```
...
- `argent gateway health|status` — request health/status over the Gateway WS.
- `argent message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `argent agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `argent gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `argent gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `argent gateway` and the legacy TCP control port.
