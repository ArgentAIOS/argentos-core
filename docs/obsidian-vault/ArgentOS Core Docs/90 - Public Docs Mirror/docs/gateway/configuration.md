---
summary: "All configuration options for ~/.argentos/argent.json with examples"
read_when:
...
ArgentOS reads an optional **JSON5** config from `~/.argentos/argent.json` (comments + trailing commas allowed).

If the file is missing, ArgentOS uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.argentos/workspace`). You usually only need a config to:
...
ArgentOS only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `argent doctor`, `argent logs`, `argent health`, `argent status`, `argent service`, `argent help`).
- Run `argent doctor` to see the exact issues.
- Run `argent doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `argent config set`. Keep a backup of `~/.argentos/argent.json`.
...
```bash
argent gateway call config.get --params '{}' # capture payload.hash
argent gateway call config.apply --params '{
...
```bash
argent gateway call config.get --params '{}' # capture payload.hash
argent gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.argentos/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@argent", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.argentos/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.argentos/workspace" }],
}
```
...
{ "$include": "/etc/argent/base.json5" } // absolute
...
```json5
// ~/.argentos/clients/mueller/agents.json5
[
...
```json5
// ~/.argentos/clients/mueller/broadcast.json5
{
...
ArgentOS reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, ArgentOS runs your login shell and imports only the missing expected keys (never overrides).
...
- `ARGENTOS_LOAD_SHELL_ENV=1`
- `ARGENTOS_SHELL_ENV_TIMEOUT_MS=15000`
...
ArgentOS stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.argentos/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.argentos/credentials/oauth.json` (or `$ARGENTOS_STATE_DIR/credentials/oauth.json`)
...
- `~/.argentos/agent/*` (migrated by `argent doctor` into `~/.argentos/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `ARGENTOS_OAUTH_DIR`
- Agent dir (default agent root override): `ARGENTOS_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, ArgentOS imports `oauth.json` entries into `auth-profiles.json`.
...
If set, ArgentOS derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/argentos/argent-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/argentos/argent.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.argentos/credentials/whatsapp/biz
          // authDir: "~/.argentos/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `argent doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@argent", "argent"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@argent"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.argentos/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.argentos/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.argentos/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.argentos/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.argentos/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.argentos/workspace-home" },
      { id: "work", workspace: "~/.argentos/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `argent.json`).
...
ArgentOS starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["argent-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-argent",
...
ArgentOS starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `argent plugins install @argentos/mattermost` (or `./extensions/mattermost` from a git checkout).
...
ArgentOS starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
ArgentOS spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, ArgentOS
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/argent" } },
}
```
...
When a file exceeds this limit, ArgentOS logs a warning and injects a truncated
...
message envelopes). If unset, ArgentOS uses the host timezone at runtime.
...
`[argent]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[argent]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[argent]"`, ArgentOS will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, ArgentOS generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.argentos/settings/tts.json",
      elevenlabs: {
...
`params` is also applied to streaming runs (embedded agent + compaction). Common keys:

- `temperature`, `maxTokens`
- `cacheRetention` (`none` | `short` | `long`, Anthropic)
- `reasoning` (`minimal` | `low` | `medium` | `high` | `xhigh`)
- `thinking` (`off` | `on` | `adaptive` | provider object)
- `effort` (`low` | `medium` | `high` | `max`, Anthropic adaptive thinking)

These merge with call-time options; caller-supplied values win. `temperature` is
an advanced knob—leave unset unless you know the model’s defaults and need a change.
    ...
    defaults: {
      models: {
        "anthropic/claude-sonnet-4-5-20250929": {
          params: { temperature: 0.6, thinking: "adaptive", effort: "medium" },
        },
        "openai/gpt-5.2": {
...
ArgentOS also ships a few built-in alias shorthands. Defaults only apply when the model
...
- For `claude-cli`, defaults are wired in. Override the command path if PATH is minimal
  (launchd/systemd).
- To expose MCP tools in CLI backends, configure `mcpServers` (inline) or
  `mcpConfigPath` (existing JSON file). ArgentOS passes this via `mcpConfigArg`
  (Claude default: `--mcp-config`).
      ...
      cliBackends: {
        "claude-cli": {
          ...
          mcpServers: {
            filesystem: {
              command: "npx",
              args: ["-y", "@modelcontextprotocol/server-filesystem", "/Users/sem/code"],
            },
          },
        },
        "my-cli": {
          ...
          imageArg: "--image",
          imageMode: "repeat",
          mcpConfigPath: "/Users/sem/.config/my-cli/mcp.json",
...
If you omit the provider, ArgentOS currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `argent configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:argent`: all built-in ArgentOS tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.argentos/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.argentos/sandboxes",
        docker: {
          image: "argent-sandbox:bookworm-slim",
          containerPrefix: "argent-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "argent-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "argent-sandbox-browser:bookworm-slim",
          containerPrefix: "argent-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
ArgentOS uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.argentos/agents/<agentId>/agent/models.json` or by defining the same schema inside your
ArgentOS config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, ArgentOS writes/merges a `models.json` into
`~/.argentos/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. ArgentOS uses
...
- Shortcut: `argent onboard --auth-choice opencode-zen`.
...
Shortcut: `argent onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `argent onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `argent onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `argent onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `argent onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `argent onboard --auth-choice minimax-api`.
...
- Override the agent config root with `ARGENTOS_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.argentos/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.argentos/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.argentos/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, ArgentOS stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.argentos/skills` (workspace skills
...
from `~/.argentos/extensions`, `<workspace>/.argentos/extensions`, plus any
...
### `browser` (argent-managed browser)

ArgentOS can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for argent and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (ArgentOS.app menubar, or `argent gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      argent: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/argent" }
...
- Examples: `"/ui"`, `"/argent"`, `"/apps/argent"`.
...
- When a connection comes from one of these IPs, ArgentOS uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `argent gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `ARGENTOS_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `ARGENTOS_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. ArgentOS
...
- ArgentOS.app watches `~/.argentos/argent.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.argentos/argent.json` (or `ARGENTOS_CONFIG_PATH`) and applies changes automatically.
...
- `~/.argentos/argent.json` (or `ARGENTOS_CONFIG_PATH`)
...
- `ARGENTOS_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `argent --dev …` → uses `~/.argent-dev` + shifts ports from base `19001`
- `argent --profile <name> …` → uses `~/.argentos-<name>` (port via config/env/flags)
...
Gmail helper config (used by `argent webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "argent@gmail.com",
...
- Set `ARGENTOS_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, ArgentOS defaults `serve.path` to `/` so
...
Default root: `~/.argentos/workspace/canvas`  
Default port: `18793` (chosen to avoid the argent browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__argent__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__argent__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.argentos/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- env: `ARGENTOS_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.argentos/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.argentos/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.argentos/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_argent-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `argent` (advertises `argent.local`). Override with `ARGENTOS_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_argent-gw._tcp` under `~/.argentos/dns/` using the configured discovery domain (example: `argent.internal.`).
