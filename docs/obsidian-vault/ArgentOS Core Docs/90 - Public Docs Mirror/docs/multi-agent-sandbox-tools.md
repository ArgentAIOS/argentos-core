...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `argent sandbox explain`.
...
Legacy `agent.*` configs are migrated by `argent doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=argent-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${ARGENTOS_STATE_DIR:-$HOME/.argentos}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
