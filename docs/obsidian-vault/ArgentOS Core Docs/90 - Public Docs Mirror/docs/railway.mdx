...
Deploy ArgentOS on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/argent-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/argent` — Control UI

## What you get

- Hosted ArgentOS Gateway + Control UI
...
- `ARGENTOS_STATE_DIR=/data/.argentos` (recommended)
- `ARGENTOS_WORKSPACE_DIR=/data/workspace` (recommended)
- `ARGENTOS_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your ArgentOS state + workspace so you can migrate to another host without losing config or memory.
