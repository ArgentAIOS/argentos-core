...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/argent`)
...
If the page fails to load, start the Gateway first: `argent gateway`.
...
you revoke it with `argent devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.argentos/argent.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. ArgentOS
...
ArgentOS **blocks** Control UI connections without device identity.
