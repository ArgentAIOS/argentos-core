...
```bash
argent tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `argent tui --deliver`
...
- Check the Gateway logs: `argent logs --follow`.
- Confirm the agent can run: `argent status` and `argent models status`.
...
- No agents in picker: check `argent agents list` and your routing config.
