...
- `/usage cost` in chats: local cost summary aggregated from ArgentOS session logs.
- CLI: `argent status --usage` prints a full per-provider breakdown.
- CLI: `argent channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
