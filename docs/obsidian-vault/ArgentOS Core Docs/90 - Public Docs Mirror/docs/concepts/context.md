...
read_when:
  - You want to understand what “context” means in ArgentOS
...
“Context” is **everything ArgentOS sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (ArgentOS-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How ArgentOS builds the system prompt

The system prompt is **ArgentOS-owned** and rebuilt each run. It includes:
...
By default, ArgentOS injects a fixed set of workspace files (if present):
