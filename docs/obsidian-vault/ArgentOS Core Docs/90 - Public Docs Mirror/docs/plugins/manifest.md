...
read_when:
  - You are building a ArgentOS plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (argent.plugin.json)

Every plugin **must** ship a `argent.plugin.json` file in the **plugin root**.
ArgentOS uses this manifest to validate configuration **without executing plugin
