...
Deploy ArgentOS on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/ArgentAIOS/argentos"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run ArgentOS:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: ARGENTOS_STATE_DIR
        value: /data/.argentos
      - key: ARGENTOS_WORKSPACE_DIR
        value: /data/workspace
      - key: ARGENTOS_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/argent`.
...
If you use the original ArgentOS repository, Render will not auto-deploy your ArgentOS. To update it, run a manual Blueprint sync from the dashboard.
...
For ArgentOS, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any ArgentOS host.
