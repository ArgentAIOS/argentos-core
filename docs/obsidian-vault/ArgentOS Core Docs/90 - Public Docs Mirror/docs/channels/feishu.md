...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects ArgentOS to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
argent plugins install @argentos/feishu
```

Local checkout (when running from a git repo):

```bash
argent plugins install ./extensions/feishu
```
...
If you just installed ArgentOS, run the wizard:
...
2. Configuring app credentials in ArgentOS
...
1. You already ran `argent channels add` for Feishu
2. The gateway is running (`argent gateway status`)
...
```bash
argent pairing approve feishu <CODE>
```
...
2. Run `argent logs --follow` and look for `chat_id`
...
2. Run `argent logs --follow` and look for `open_id`
...
| Command                  | Description                   |
| ------------------------ | ----------------------------- |
| `argent gateway status`  | Show gateway status           |
| `argent gateway install` | Install/start gateway service |
| `argent gateway stop`    | Stop gateway service          |
| `argent gateway restart` | Restart gateway service       |
| `argent logs --follow`   | Tail gateway logs             |
...
4. Check logs: `argent logs --follow`
...
5. Ensure the gateway is running: `argent gateway status`
6. Check logs: `argent logs --follow`
