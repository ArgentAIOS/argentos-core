...
read_when:
  - You want ArgentOS to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables ArgentOS to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`argent onboard`) and `argent channels add` list optional channel plugins.
...
```bash
argent plugins install @argentos/nostr
```

Use a local checkout (dev workflows):

```bash
argent plugins install --link <path-to-argent>/extensions/nostr
```
...
```json
{
  "channels": {
    "nostr": {
      ...
      "profile": {
        "name": "argent",
        "displayName": "ArgentOS",
        "about": "Personal assistant DM bot",
        "picture": "https://example.com/avatar.png",
        "banner": "https://example.com/banner.png",
        "website": "https://example.com",
        "nip05": "argent@example.com",
        "lud16": "argent@example.com"
      }
    }
  }
}
