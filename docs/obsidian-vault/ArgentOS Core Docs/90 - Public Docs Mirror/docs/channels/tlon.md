...
Tlon is a decentralized messenger built on Urbit. ArgentOS connects to your Urbit ship and can
...
```bash
argent plugins install @argentos/tlon
```

Local checkout (when running from a git repo):

```bash
argent plugins install ./extensions/tlon
```
...
Use these with `argent message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, ArgentOS replies in-thread.
