...
read_when:
  - Setting up Twitch chat integration for ArgentOS
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. ArgentOS connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
argent plugins install @argentos/twitch
```

Local checkout (when running from a git repo):

```bash
argent plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `ARGENTOS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "argent", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ARGENTOS_TWITCH_ACCESS_TOKEN env var)
...
```bash
ARGENTOS_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
