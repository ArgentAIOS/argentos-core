...
- [ ] Update CLI/version strings: [`src/cli/program.ts`](https://github.com/ArgentAIOS/argentos/blob/main/src/cli/program.ts) and the Baileys user agent in [`src/provider-web.ts`](https://github.com/ArgentAIOS/argentos/blob/main/src/provider-web.ts).
- [ ] Confirm package metadata (name, description, repository, keywords, license) and `bin` map points to [`argent.mjs`](https://github.com/ArgentAIOS/argentos/blob/main/argent.mjs) for `argent`.
...
We only publish **existing npm plugins** under the `@argentos/*` scope. Bundled
plugins that are not on npm stay **disk-tree only** (still shipped in
`extensions/**`).

Process to derive the list:

1. `npm search @argent --json` and capture the package names.
