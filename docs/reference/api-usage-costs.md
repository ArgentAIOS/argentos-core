...
ArgentOS features that can generate provider usage or paid API calls.
...
- `argent status --usage` and `argent channels list` show provider **usage windows**
...
ArgentOS can pick up credentials from:
...
`argent models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
