...
ArgentOS integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- ArgentOS owns lifecycle when `channels.signal.autoStart=true`.
...
- ArgentOS spawns `imsg rpc` as a child process (legacy iMessage integration).
