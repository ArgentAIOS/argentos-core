...
你可以在 `~/.argentos/argent.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/argent.log"
  }
}
```
...
```bash
argent channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.argentos/argent.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/argentos/argent-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- ArgentOS 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
ARGENTOS_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `argent plugins enable diagnostics-otel` 启用插件。
...
- `argent.tokens`（计数器，属性：`argent.token`、`argentos.channel`、`argent.provider`、`argent.model`）
- `argent.cost.usd`（计数器，属性：`argentos.channel`、`argent.provider`、`argent.model`）
- `argent.run.duration_ms`（直方图，属性：`argentos.channel`、`argent.provider`、`argent.model`）
- `argent.context.tokens`（直方图，属性：`argent.context`、`argentos.channel`、`argent.provider`、`argent.model`）

消息流：

- `argent.webhook.received`（计数器，属性：`argentos.channel`、`argent.webhook`）
- `argent.webhook.error`（计数器，属性：`argentos.channel`、`argent.webhook`）
- `argent.webhook.duration_ms`（直方图，属性：`argentos.channel`、`argent.webhook`）
- `argent.message.queued`（计数器，属性：`argentos.channel`、`argent.source`）
- `argent.message.processed`（计数器，属性：`argentos.channel`、`argent.outcome`）
- `argent.message.duration_ms`（直方图，属性：`argentos.channel`、`argent.outcome`）

队列 + 会话：

- `argent.queue.lane.enqueue`（计数器，属性：`argent.lane`）
- `argent.queue.lane.dequeue`（计数器，属性：`argent.lane`）
- `argent.queue.depth`（直方图，属性：`argent.lane` 或 `argentos.channel=heartbeat`）
- `argent.queue.wait_ms`（直方图，属性：`argent.lane`）
- `argent.session.state`（计数器，属性：`argent.state`、`argent.reason`）
- `argent.session.stuck`（计数器，属性：`argent.state`）
- `argent.session.stuck_age_ms`（直方图，属性：`argent.state`）
- `argent.run.attempt`（计数器，属性：`argent.attempt`）

### 导出的 span（名称 + 关键属性）

- `argent.model.usage`
  - `argentos.channel`、`argent.provider`、`argent.model`
  - `argent.sessionKey`、`argent.sessionId`
  - `argent.tokens.*`（input/output/cache_read/cache_write/total）
- `argent.webhook.processed`
  - `argentos.channel`、`argent.webhook`、`argent.chatId`
- `argent.webhook.error`
  - `argentos.channel`、`argent.webhook`、`argent.chatId`、`argent.error`
- `argent.message.processed`
  - `argentos.channel`、`argent.outcome`、`argent.chatId`、`argent.messageId`、`argent.sessionKey`、`argent.sessionId`、`argent.reason`
- `argent.session.stuck`
  - `argent.state`、`argent.ageMs`、`argent.queueDepth`、`argent.sessionKey`、`argent.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `argent doctor`。
