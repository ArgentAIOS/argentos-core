---
summary: "Agent tool surface for ArgentOS (browser, canvas, nodes, message, cron) replacing legacy `argent-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `argent-*` skills
title: "Tools"
---

# Tools (ArgentOS)

ArgentOS exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `argent-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `argent.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, ArgentOS logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`argent node run`).
...
- Requires a Brave API key (recommended: `argent configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated ArgentOS-managed browser.
...
- Quick smoke: `argent nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `argent gateway` restart in-place)
