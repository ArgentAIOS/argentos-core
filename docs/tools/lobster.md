---
title: Lobster
summary: "Typed workflow runtime for ArgentOS with resumable approval gates."
description: Typed workflow runtime for ArgentOS — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets ArgentOS run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: ArgentOS runs one Lobster tool call and gets a structured result.
...
ArgentOS launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | argent.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
argent.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In ArgentOS tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the ArgentOS Gateway (see the [Lobster repo](https://github.com/argent/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), ArgentOS keeps core tools enabled. To restrict core
...
→ argent calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ argent calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls ArgentOS tools that do.
