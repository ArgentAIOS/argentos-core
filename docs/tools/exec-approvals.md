...
- **gateway host** → `argent` process on the gateway machine
...
`~/.argentos/exec-approvals.json` directly.

CLI: `argent approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
