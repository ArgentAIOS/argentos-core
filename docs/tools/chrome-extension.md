---
summary: "Chrome extension: let ArgentOS drive your existing Chrome tab"
read_when:
...
The ArgentOS Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate argent-managed Chrome profile.
...
ArgentOS then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the ArgentOS release (npm package) as static files. There is no separate “build” step.

After upgrading ArgentOS:

- Re-run `argent browser extension install` to refresh the installed files under your ArgentOS state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

ArgentOS ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want ArgentOS to control.
...
- `ON`: attached; ArgentOS can drive that tab.
...
Debugging: `argent sandbox explain`
...
`argent browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `argent browser extension install` first to copy the extension to a stable location under your ArgentOS state directory.
...
- **This is not isolated** like the dedicated argent-managed profile.
