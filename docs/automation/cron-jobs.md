...
argent cron list
argent cron run <job-id> --force
argent cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.argentos/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `argent cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, ArgentOS defaults to `announce`.
...
- Job store: `~/.argentos/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.argentos/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
argent cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
argent cron edit <jobId> --agent ops
argent cron edit <jobId> --clear-agent
```
...
```bash
argent system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`argent system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `ARGENTOS_SKIP_CRON`.
